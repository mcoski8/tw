"""
Session 74 — strategy_v47_dt: v44 + 1 high_only_aug_v7 route-tradeoff
comparator (108 features total), trained at v44's saturating regime
(depth=36 ml=1).

H2 isolation: v47 = v44 + ho_v7 (NOT v46b + ho_v7). The 2 ho_v6 features
from v46b are EXCLUDED. This isolates the H2 (route-tradeoff comparator)
effect from H1 (SS+ms route quality). If both H1 and H2 partial-positive,
v48 = v44 + ho_v6 + ho_v7 will batch them.

See SESSION_74 design notes and CURRENT_PHASE.md S73→S74 rewrite.
"""
...
from high_only_aug_v7_features_gated import compute_high_only_v7_features_for_hand  # noqa: E402

_MODEL_CACHE: Optional[dict] = None
MODEL_PATH = ROOT / "data" / "v47_dt_model.npz"
...
_HO_V7_NAMES = ["ho_v7_route_tradeoff_joint_minus_nonjoint_g"]
...
def load_model(path: Optional[Path] = None) -> dict:
    ...
    _MODEL_CACHE = {
        ...
        "ho_v7_index_in_full":    [feature_columns.index(c) for c in _HO_V7_NAMES],
...
def strategy_v47_dt(hand: np.ndarray) -> int:
    model = load_model()
    ...
    ho_v7 = compute_high_only_v7_features_for_hand(h)
    ...
    for k, ci in enumerate(model["ho_v7_index_in_full"]):    full[ci] = ho_v7[k]
