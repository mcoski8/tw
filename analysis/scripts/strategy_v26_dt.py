"""
Session 33 — strategy_v26_dt: extends v25 with 6 two_pair-gated aug
features. At runtime computes all 5 gated families plus base vector,
...
def load_model(path: Optional[Path] = None) -> dict:
    ...
    base_columns = [
        ...
        if c not in suited_set and c not in tp_set and c not in comp_set
           and c not in pair_set and c not in twopair_set
    ]
...
def strategy_v26_dt(hand: np.ndarray) -> int:
    model = load_model()
