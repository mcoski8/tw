#!/usr/bin/env python3
"""Grade v19_gated (gated suited-broadway features) vs v18d on full + prefix."""
...
def main() -> int:
    ...
    # Build v18d strategy via the inline npz loader.
    from strategy_v18_dt import _walk_tree_to_leaf
    from strategy_v5_dt import compute_feature_vector
    from strategy_v19_gated_dt import strategy_v19_gated_dt

    arr = np.load(REPO / "data" / "v18d_dt_model.npz", allow_pickle=True)
    ...
    feat_meta_v18d = {"cat_map": cat_map, "feature_columns": feature_columns}
    def strategy_v18d(hand):
        x = compute_feature_vector(hand, feat_meta_v18d)
        return int(v18d_model["leaf_values"][_walk_tree_to_leaf(x, v18d_model)].argmax())

    strategies = [
        ("v18d        (d=28, ml=10, 193K leaves, 37 feat)", strategy_v18d),
        ("v19_gated   (d=28, ml=10, 216K leaves, 43 feat)", strategy_v19_gated_dt),
    ]
    ...
    print(f"\nv19_gated vs v18d: {results[0].mean_regret - results[1].mean_regret:+.4f}  ≈ ${(results[0].mean_regret - results[1].mean_regret) * 10 * 1000:+,.0f}/1000h")
    return 0
