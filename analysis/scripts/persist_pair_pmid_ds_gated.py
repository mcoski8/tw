"""
Session 78 — persist H6 (pair_pmid_ds_n_configs_g) to parquet.

Output: data/feature_table_pair_pmid_ds_gated.parquet
"""
...
from pair_pmid_ds_features_gated import compute_pair_pmid_ds_batch  # noqa: E402

OUT = ROOT / "data" / "feature_table_pair_pmid_ds_gated.parquet"
...
print("computing pair_pmid_ds_n_configs_g ...")
t0 = time.time()
augmented = compute_pair_pmid_ds_batch(hands)
print(f"  done ({time.time()-t0:.1f}s)")

vals = augmented["pair_pmid_ds_n_configs_g"]
table = pa.table({
    "canonical_id": np.arange(N, dtype=np.uint32),
    "pair_pmid_ds_n_configs_g": pa.array(vals, type=pa.int8()),
})
...
vv = back["pair_pmid_ds_n_configs_g"].values
uniq, counts = np.unique(vv, return_counts=True)
summary = ", ".join(f"{int(u)}:{int(c):,}" for u, c in zip(uniq, counts) if c > 0)
print(f"    pair_pmid_ds_n_configs_g  {summary}")

# Zero-on-non-pair verification.
