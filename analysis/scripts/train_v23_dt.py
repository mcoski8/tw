"""
Session 31 — train_v23_dt: extends train_v19_gated_dt with the 6
trips_pair-gated aug features (zeros for non-trips_pair hands). Total
feature count: 37 base + 6 suited-gated + 6 trips_pair-gated = 49.

The trips_pair gated features fire on ~8.7% of hands (much larger
population than the 20.4% high_only suited features fire on). The
gating ensures any DT split using these features applies ONLY to
trips_pair hands; non-trips_pair hands route via the existing 43-feature
v20 paths unchanged.

Saves data/v23_dt_model.npz with the same npz layout.
Inference path: strategy_v23_dt.py.

Run:
  PYTHONUNBUFFERED=1 python3 analysis/scripts/train_v23_dt.py \\
      --max-depth 30 --min-samples-leaf 5 --output data/v23_dt_model.npz
"""
...
SUITED_GATED_PATH = ROOT / "data" / "feature_table_suited_aug_gated.parquet"
TRIPS_PAIR_GATED_PATH = ROOT / "data" / "feature_table_trips_pair_aug_gated.parquet"

CATS = ["high_only", "pair", "quads", "three_pair", "trips", "trips_pair", "two_pair"]
ALPHA_MAP = {c: i for i, c in enumerate(CATS)}

# 37 base + 6 suited-gated + 6 trips_pair-gated = 49.
FEATURE_COLUMNS = [
    ...
    # Session 30 additions (gated to high_only):
    ...
    # Session 31 additions (gated to trips_pair):
    "tp_trip_rank_g",
    "tp_pair_rank_g",
...
def build_X():
    ...
    sg = pd.read_parquet(SUITED_GATED_PATH)
    tpg = pd.read_parquet(TRIPS_PAIR_GATED_PATH)
    ...
    cd = {
        ...
        "tp_trip_rank_g": tpg["tp_trip_rank_g"].to_numpy(),
        "tp_pair_rank_g": tpg["tp_pair_rank_g"].to_numpy(),
        "tp_high_singleton_rank_g": tpg["tp_high_singleton_rank_g"].to_numpy(),
        "tp_low_singleton_rank_g": tpg["tp_low_singleton_rank_g"].to_numpy(),
        "tp_singletons_suited_g": tpg["tp_singletons_suited_g"].to_numpy(),
        "tp_pair_routing_is_ds_g": tpg["tp_pair_routing_is_ds_g"].to_numpy(),
    }
    n = len(ft)
...
def main() -> int:
    ...
    ap.add_argument("--output", type=Path, default=ROOT / "data" / "v23_dt_model.npz")
    args = ap.parse_args()
    ...
    print(f"  v23 picked mean EV:    {picked_ev:+.4f}", flush=True)
