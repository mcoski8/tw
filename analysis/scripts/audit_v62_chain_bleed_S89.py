"""S89 chain bleed audit — identify which rule layer (v44->v47->v48->v52)
is responsible for the +$10.09/1000h bleed across the remaining prefix-silent
HIGH_ONLY cells (JOINT_MED × {J-A}, JOINT_LOW × {J-A}).
...
This is the same pattern S87/S88 used; v52 == v57 == v62 on these cells
(v62 gate-out fires on DS_NO_JOINT/DS_NO_MAXTOP/MS_ONLY/JOINT_HIGH × J-A,
not these).
...
def main() -> int:
    print("=" * 100)
    print("S89 chain bleed audit — per-layer attribution on JOINT_MED + JOINT_LOW × {J-A}")
    print("=" * 100)
    ...
    print(f"  filtered to {len(sub):,} hands across S89 target cells")
    print()
    ...
    print()
    print(f"  Σ Δ across all S89 cells: "
    ...
    # === Net bleed by mode (rolled up across all S89 cells) ===
    print("=" * 120)
    print("NET BLEED BY FIRING MODE (rolled up across all S89 cells)")
    print("=" * 120)
    ...
    print(f"  TOTAL BLEED (sum of v52 − v44 leak across S89 cells): "
