"""
Session 57 — Drill HO3: High_only zone v42_dt diagnostic (Phase 1, post-S56).

Goal: re-drill v42_dt's residual matrix in high_only after S56's ho_v2
features collapsed the SS->DS routing axis. The dominant tA_SS_mu →
tA_DS_ms class should be smaller now; new dominant classes will surface.
Three S57 axis hypotheses to look for:
  - Axis 1: defensive trigger at max ∈ {K, Q} (still picks K-on-top
            where oracle wants a defensive 2-on-top).
  - Axis 2: defensive top-card choice on T-9-8 hands (which low card
            goes on top).
  - Axis 3: broadway connectivity at non-Ace tops (mid Hold'em equity
            from connectors might outweigh DS-bot value).

Same classification scheme as Drill HO (Session 56):
  top_rank   : rank of the 1 top card (2..14)
  bot_suit   : DS / SS / RB / 31 / 4f
  mid_suited : "ms" if mid 2 cards share suit, "mu" otherwise

Run:
  PYTHONUNBUFFERED=1 python3 -u analysis/scripts/drill_high_only_zone_v42_diagnostic.py
"""
...
def main() -> int:
    ...
    print("=" * 88)
    print("Session 57 Drill HO3: High_only zone v42_dt diagnostic")
    print("=" * 88)
    ...
    cell_stats = defaultdict(lambda: {
        "n": 0,
        "sum_regret": 0.0,
        "v42_class_dist": Counter(),
        "oracle_class_dist": Counter(),
        "mismatch": Counter(),
        "mismatch_regret": defaultdict(float),
    })

    print("\n[3/4] per-hand v42_dt vs oracle classification ...", flush=True)
    t0 = time.time()
    n_processed = 0
    for cid in ho_idx:
        cid = int(cid)
        ...
        v42_class = classify_pick(h, feats, v42_idx)
        ...
        cell["v42_class_dist"][v42_class] += 1
        cell["oracle_class_dist"][oracle_class] += 1
        if v42_class != oracle_class:
            cell["mismatch"][(v42_class, oracle_class)] += 1
            cell["mismatch_regret"][(v42_class, oracle_class)] += regret
    ...
    print("=" * 110)
    print("TOP HIGH_ONLY (h1, h2) CELLS BY WHOLE-GRID REGRET CONTRIBUTION (v42_dt vs oracle)")
    print("=" * 110)
    ...
    print(f"  {'v42_pick':<22} {'oracle_pick':<22} {'n':>10} {'mean_reg':>10} {'wg_contrib':>12}")
    ...
    print(f"  {'v42_top':<10} {'oracle_top':<10} {'n':>10} {'mean_reg':>10} {'wg_contrib':>12}")
    ...
    print(f"  {'v42_bot':<10} {'oracle_bot':<10} {'n':>10} {'mean_reg':>10} {'wg_contrib':>12}")
    ...
    if ranked:
        top_k = ranked[0][0]
        top_v, top_o = top_k
        print(f"\n── DEEP DIVE: top mismatch  v42={top_v}  oracle={top_o} ──")
    ...
    print("\n" + "=" * 110)
    print("OVERALL CLASS DISTRIBUTION (v42 picks vs oracle picks)")
    print("=" * 110)
    agg_v42 = Counter()
    agg_oracle = Counter()
    for _, st in cell_stats.items():
        agg_v42.update(st["v42_class_dist"])
        agg_oracle.update(st["oracle_class_dist"])
    total = sum(agg_v42.values())
    all_classes = sorted(set(agg_v42.keys()) | set(agg_oracle.keys()),
                         key=lambda c: -(agg_v42[c] + agg_oracle[c]))
    print(f"  {'class':<22} {'v42_n':>10} {'v42_pct':>10} {'oracle_n':>10} {'oracle_pct':>11} {'v42-oracle':>12}")
    ...
    v42_top = Counter()
    oracle_top = Counter()
    for c, n in agg_v42.items():
        v42_top[c.split("_", 1)[0]] += n
    for c, n in agg_oracle.items():
        oracle_top[c.split("_", 1)[0]] += n
    keys = sorted(set(v42_top.keys()) | set(oracle_top.keys()))
    print(f"  {'top':<6} {'v42_n':>10} {'v42_pct':>10} {'oracle_n':>10} {'oracle_pct':>11} {'v42-oracle':>12}")
    for k in keys:
        vn = v42_top[k]; on = oracle_top[k]
        vp = 100*vn/total if total else 0
        op = 100*on/total if total else 0
        diff = vp - op
    ...
    oracle_suit = Counter()
    for c, n in agg_v42.items():
        v42_suit[c.split("_")[1] if "_" in c else "?"] += n
    for c, n in agg_oracle.items():
        oracle_suit[c.split("_")[1] if "_" in c else "?"] += n
    keys = sorted(set(v42_suit.keys()) | set(oracle_suit.keys()))
    print(f"  {'suit':<6} {'v42_n':>10} {'v42_pct':>10} {'oracle_n':>10} {'oracle_pct':>11} {'v42-oracle':>12}")
    for k in keys:
        vn = v42_suit[k]; on = oracle_suit[k]
        vp = 100*vn/total if total else 0
        op = 100*on/total if total else 0
        diff = vp - op
        print(f"  {k:<6} {vn:>10,} {vp:>9.2f}% {on:>10,} {op:>10.2f}% {diff:>+11.2f}%")
