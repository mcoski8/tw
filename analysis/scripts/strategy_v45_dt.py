"""
Session 59 — strategy_v45_dt: v44 + 4 high_only_aug_v5 rank-valued
features (111 total). Adds non-max-joint MID QUALITY + max-in-bot-pair
count + 4f topMax count on top of v44's max_top_rank/n_configs signal.
"""
...
from high_only_aug_v5_features_gated import compute_high_only_v5_features_for_hand  # noqa: E402

_MODEL_CACHE: Optional[dict] = None
MODEL_PATH = ROOT / "data" / "v45_dt_model.npz"
...
_HO_V5_NAMES = ["ho_v5_topNonMax_DS_ms_max_mid_high_g",
...
def load_model(path: Optional[Path] = None) -> dict:
    ...
    _MODEL_CACHE = {
        ...
        "ho_v5_index_in_full":    [feature_columns.index(c) for c in _HO_V5_NAMES],
...
def strategy_v45_dt(hand: np.ndarray) -> int:
    model = load_model()
    ...
    ho_v5 = compute_high_only_v5_features_for_hand(h)
    ...
    for k, ci in enumerate(model["ho_v5_index_in_full"]):    full[ci] = ho_v5[k]
