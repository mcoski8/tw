"""
Session 19 — depth-vs-agreement curve on the FULL 6M with all 36 features
(27 baseline + 3 pair-aug + 3 high_only-aug + 3 two_pair-aug).

Identical methodology to dt_phase1_aug2.py (3-fold CV on 1M subsample,
full-6M fit at each chosen depth, shape-agreement scoring).
...
References:
  ...
  Session 18 augmented (33 features = +pair-aug + high-aug):
    depth=None:        65.20% / 58.60%
    depth=15:          62.86% / 61.59% (new knee)
  Session 19 augmented (36 features = +pair-aug + high-aug + 2p-aug):
    target ceiling:    ~66.0%+ at depth=None per dt_two_pair_aug_ceiling.

Hypothesis: the +X.XXpp depth=None lift over Session 18 should propagate
...
# Load all augmented-feature parquets.
...
print(f"  done ({time.time()-t0:.1f}s)")
print("loading two_pair_aug parquet ...")
t0 = time.time()
...
# Baseline-feature engineering (identical to dt_phase1_aug2.py).
...
print()
print("Reference (Session 16/17/18):")
print("  Session 16 baseline (27)   depth=None:  61.74% / 57.24% (full / cv)")
print("  Session 17 aug-30          depth=None:  63.76% / 58.0%")
print("  Session 17 aug-30          depth=15:    61.96% / 60.71%")
print("  Session 18 aug-33          depth=None:  65.20% / 58.60%")
print("  Session 18 aug-33          depth=15:    62.86% / 61.59%   ← Session 18 knee")
print()
