"""
Session 59 — Drill HO11+HO12+HO13 (consolidated): exhaustive high_only deep-dive
on v44_dt's residuals (4th-pass on high_only).

PURPOSE
-------
After S58's ho_v4 collapse ($2,075 → $1,868 within-cat), the residual SHIFTED
inside the high_only zone — the S58 decision matrix surfaced K/Q × DS_NO_JOINT
as the deepest unaddressed cell (oracle drops max off top 34%/52% but v43
keeps it on top 87%/68% — v44 only partly fixed this via the non-max joint
count). HO11–HO13 re-characterize ORACLE vs v44_dt to identify the dominant
new residual axis for ho_v5 feature design.

This single script consolidates three drills (one sweep over all 1.226M
high_only hands):

  HO11 — per-max_rank residual stratification.
        For each max_rank in {A,K,Q,J,T,9,8}:
          * total v44 vs oracle mismatch contribution ($/1000h whole-grid)
          * top-10 mismatch classes (v44_pick -> oracle_pick)
          * v44 pick distribution (top x bot_suit x mid_suited)
          * oracle pick distribution (same)
          * pct_opt within max_rank class

  HO12 — full structural achievability cross-tabulation per hand:
  ...
  HO13 — what oracle ACTUALLY picks per (max_rank x structural cell):
        ...
        Same for v44's pick. Mismatches surface the per-cell DELTA.

OUTPUTS
-------
1. Console: per-max_rank tables + per-cell oracle-vs-v44 pick distributions
2. data/drill_ho_v44_per_hand_structural.parquet — per-hand structural
   counts + oracle/v44 pick details. Re-used by HO8/HO9 drills.
3. data/drill_ho_v44_summary.json — aggregate stats keyed by
   (max_rank, structural_cell) for downstream analysis.

Run:
  PYTHONUNBUFFERED=1 python3 -u analysis/scripts/drill_high_only_v44_deepdive.py
"""
...
OUT_PARQUET = ROOT / "data" / "drill_ho_v44_per_hand_structural.parquet"
OUT_JSON = ROOT / "data" / "drill_ho_v44_summary.json"
...
def characterize_pick(hand_bytes, feats, idx):
    """Detailed characterization of a chosen setting for HO13 aggregates."""
    pos = SETTING_HAND_INDICES[idx]
    top_pos = int(pos[0])
...
def main() -> int:
    ...
    print("=" * 88)
    print("Session 59 — Drill HO11+HO12+HO13: high_only deep-dive on v44_dt")
    print("=" * 88)
    ...
    arr_v44_top = np.zeros(n, dtype=np.int8)
    arr_v44_bot_suit = np.zeros(n, dtype=np.int8)  # SUIT_PROFILE codes
    ...
    arr_v44_mid_sum = np.zeros(n, dtype=np.int8)
    arr_v44_bot_run = np.zeros(n, dtype=np.int8)
    arr_v44_bot_max = np.zeros(n, dtype=np.int8)
    arr_v44_bot_min = np.zeros(n, dtype=np.int8)
    arr_v44_bot_sum = np.zeros(n, dtype=np.int8)
    ...
    cell_stats = defaultdict(lambda: {
        ...
        # HO13 aggregates: distributions of TOP/BOT/MID for oracle and v44
    ...
    # Per-max-rank-only aggregates (HO11).
    ...
    for k, cid in enumerate(ho_idx):
        ...
        v44_top, v44_suit, v44_midlbl, _, v44_class = classify_pick(h, feats, v44_idx)
        or_top, or_suit, or_midlbl, _, or_class = classify_pick(h, feats, oracle_idx)

        v44_det = characterize_pick(h, feats, v44_idx)
        ...
        arr_regret[k] = regret
        arr_v44_top[k] = v44_det["top_rank"]
        ...
        arr_v44_mid_sum[k] = v44_det["mid_sum"]
        arr_v44_bot_run[k] = v44_det["bot_run"]
        arr_v44_bot_max[k] = v44_det["bot_max"]
        arr_v44_bot_min[k] = v44_det["bot_min"]
        arr_v44_bot_sum[k] = v44_det["bot_sum"]
        ...
        st["v44_bot_sum_total"] += v44_det["bot_sum"]
        st["v44_mid_sum_total"] += v44_det["mid_sum"]
    ...
    # ===========================================================
    # HO11 — per-max-rank residual stratification
    # ===========================================================
    print("=" * 100)
    print("HO11: PER-MAX-RANK RESIDUAL STRATIFICATION (v44_dt vs oracle)")
    print("=" * 100)
    ...
    print("\n  Per-max-rank class distribution (v44 pick % vs oracle pick %):")
    for max_rank in sorted(rank_stats.keys(), reverse=True):
        rs = rank_stats[max_rank]
        if rs["n"] == 0:
            continue
        ...
        print(f"    {'class':<22} {'v44_pct':>9} {'oracle_pct':>11} {'delta':>9}")
    ...
    # ===========================================================
    # HO12 — structural achievability cross-tabulation per max_rank
    # ===========================================================
    print("\n" + "=" * 100)
    print("HO12: STRUCTURAL CELL CROSS-TABULATION (per max_rank x cell)")
    print("=" * 100)
    ...
    # ===========================================================
    # HO13 — what oracle actually picks per cell
    # ===========================================================
    print("\n" + "=" * 100)
    print("HO13: ORACLE'S PICK PROFILE PER (max_rank x cell)")
    print("=" * 100)
    for max_rank in sorted(rank_stats.keys(), reverse=True):
        for cell in cells_order:
            ...
            print(f"    v44    TOP:        {v44_top_summary}")
            ...
            v44_run_summary = ", ".join(
                f"r{r}:{100*c/n:.0f}%"
                for r, c in sorted(st["v44_bot_run_dist"].items())
            )
            print(f"    v44    BOT_run:    {v44_run_summary}  bot_sum_avg={st['v44_bot_sum_total']/n:.1f}")
            v44_ms_pct = 100 * st["v44_mid_suited_n"] / n
            v44_mid_high_summary = ", ".join(
                f"{RANK_CHAR.get(r, r)}:{100*c/n:.0f}%"
                for r, c in sorted(st["v44_mid_high_dist"].items(), key=lambda x: -x[1])[:5]
            )
            print(f"    v44    MID_suited: {v44_ms_pct:.1f}%  mid_sum_avg={st['v44_mid_sum_total']/n:.1f}")
    ...
    if not args.no_parquet:
        ...
        table = pa.table({
            ...
            "v44_top": pa.array(arr_v44_top, type=pa.int8()),
            ...
            "v44_mid_sum": pa.array(arr_v44_mid_sum, type=pa.int8()),
            "v44_bot_run": pa.array(arr_v44_bot_run, type=pa.int8()),
            "v44_bot_max": pa.array(arr_v44_bot_max, type=pa.int8()),
            "v44_bot_min": pa.array(arr_v44_bot_min, type=pa.int8()),
            "v44_bot_sum": pa.array(arr_v44_bot_sum, type=pa.int8()),
