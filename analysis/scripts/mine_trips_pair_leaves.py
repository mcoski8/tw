"""
Session 20 — trips_pair miss-driven feature mining (Steps 1-3).

Modelled on mine_two_pair_leaves.py / mine_high_only_leaves.py / mine_pair_leaves.py.
Filters feature_table.parquet to (mode_count == 3 AND category == 'trips_pair'),
trains a depth=None DecisionTreeClassifier on the current 27-feature baseline,
reports shape-agreement ceiling on the slice, then ranks terminal leaves
by absolute shape-miss count and dumps the cards clustering in the top-N
most impactful leaves.

trips_pair shape: 7 cards = 3-of-rank-A + 2-of-rank-B + 2 distinct singletons
(n_trips == 1 AND n_pairs == 1, no quads). Decomposition has the trip-rank
positions (3 cards), the pair-rank positions (2 cards), and 2 singletons.

The 3 augmented-feature families from Sessions 17/18/19 are all vacuous on
trips_pair:
  - pair-aug: requires n_pairs == 1 AND n_trips == 0  → vacuous (n_trips==1)
  - high_only-aug: requires 7 distinct ranks         → vacuous
  - two_pair-aug: requires n_pairs == 2              → vacuous (n_pairs==1)

Mining uses the 28 baseline features only (the "27 baseline" label is
off-by-one — the actual count is 28; see Session 19 Gotcha note).

Strategic intuition: with one trips and one pair, the routing decision
collapses to two main axes:
  (A) which group lands on bot — trips-on-bot (3 + 1 singleton) vs
      pair-on-bot (2 + 2 singletons)
  (B) which singleton goes top
The bot-suit-coupling discriminator from Sessions 17-19 may apply, but the
3-card trip group is monosuit-impossible on bot (you need 4 cards). With
trips on bot, the 4th bot card is one singleton — bot suit profile is
(suit-of-trips × 3, suit-of-singleton); DS impossible (needs 2,2). With
pair on bot, the 4 bot cards are (pair-rank × 2 + 2 singletons) — DS is
possible (the 2 pair cards are matched-pair ⇒ 2 of one suit ALREADY, so
DS requires the 2 singletons to share a different suit). This is the
likely structural blind spot the 28 baseline features cannot expose.

Output:
  - per-depth report
  - top-50 leaves: leaf id, n_samples, n_misses, miss-share, predicted-shape,
    top-3 true-shape distribution, sample hands tagged correct/miss with
    feature summary
  - miss concentration metric (top-10 / top-50 / top-100 share of misses)
"""
...
mask = (df["mode_count"].values == 3) & (df["category"].values == "trips_pair")
df = df[mask].reset_index(drop=True)
hands = hands_all[mask]
N = len(df)
print(f"slice: mode_count==3 AND category=='trips_pair' → {N:,} hands "
...
# ---------- Feature engineering (identical to dt_phase1.py / mine_two_pair_leaves) ----------
...
print(f"\n=== depth=None DT on trips_pair 3-of-4 slice ===")
...
for rank, (lid, n_s, n_miss, pred_setting, pred_shape, idxs) in enumerate(leaf_stats[:TOP_LEAVES]):
    ...
    for j in sample_miss:
        ...
        ph = int(df["pair_high_rank"].iloc[int(j)])
        tr_rank = int(df["trips_rank"].iloc[int(j)])
        ...
        print(f"    {hand_str(h):<26}  trip={tr_rank} pair={ph} top={tr} 2nd={sr}  "
              f"{suits} conn={conn} bw={nbw}  [{tag_str}]")
