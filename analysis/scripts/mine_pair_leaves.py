"""
Session 17 — single-pair miss-driven feature mining (Steps 1-3).

Filters feature_table.parquet to (mode_count == 3) AND (category == 'pair'),
trains a depth=None DecisionTreeClassifier on the current 27-feature set,
reports shape-agreement ceiling on the slice, then ranks terminal leaves by
absolute shape-miss count and dumps the cards clustering in the top-N most
impactful leaves.

The point is discovery: which hand structures cluster inside a leaf the
features can't distinguish? A leaf's prediction is its modal setting_index;
hands inside that leaf whose TRUE shape disagrees with the modal shape are
the structural blind spots feature engineering must address.
...
Output: prints a per-depth report, then for the top-50 leaves by miss count:
  ...
  - 10 sample hands (cards as readable strings) tagged correct/miss
"""
...
N = len(df)
print(f"slice: mode_count==3 AND category=='pair' → {N:,} hands "
...
print(f"\n=== depth=None DT on single-pair 3-of-4 slice ===")
...
for lid, idxs in leaf_to_indices.items():
    ...
    if n_miss == 0:
        continue  # leaf is structurally correct, skip
    pred_setting = int(preds[idxs[0]])  # all same in a leaf
...
for rank, (lid, n_s, n_miss, pred_setting, pred_shape, idxs) in enumerate(leaf_stats[:TOP_LEAVES]):
    ...
    # Distribution of TRUE shapes in this leaf (top 3).
    ...
    # Sample hands — pick up to N from misses (if any) plus a couple correct.
    ...
    for j in sample_miss:
        ...
        # Hand-feature one-liner for the leaf-level pattern.
        pair_hi = int(df["pair_high_rank"].iloc[int(j)])
        ...
        suits = f"suit2={int(df['suit_2nd'].iloc[int(j)])}"
        tags = []
        ...
        print(f"    {hand_str(h):<26}  pair={pair_hi:>2}  {suits}  [{tag_str}]")
...
# ---------- Side-bar: how many leaves contain ALL the misses? ----------
