"""
Session 52 — Drill N: Q-high no-pair characterization (Phase 1).

Same methodology as Drill K (A-high) + Drill M (K-high) for Q-high.

Population: cat=high_only AND max_rank=12 (Queen), no Ace, no King.

Key question: does the HIMID playbook still apply?
- Q wins top tier ~30% (loses to A or K, ties other Q if opp has Q)
- Oracle Q-on-top fraction likely lower than K-high's 66%
...
from drill_bot_suit_run_priority import compute_connectivity_classes, CONN_LABELS  # noqa: E402
...
def main() -> int:
    ...
    print("=" * 88)
    print("Session 52 Drill N: Q-high no-pair characterization (Phase 1)")
    print("=" * 88)
    ...
    print(f"  Q-high no-pair scope: {len(scope_cids):,}  "
    ...
    v46_top_is_Q = 0
    v46_top_distribution = Counter()
    v46_bot_suit_dist = Counter()
    ...
    by_s2 = defaultdict(lambda: {
        "n": 0, "sum_v46": 0.0, "sum_oracle": 0.0, "n_eq": 0,
        "oracle_top_Q": 0, "oracle_DS": 0, "v46_DS": 0,
    })
    ...
    for i in range(n_total_scope):
        ...
        for suit_code in (SUIT_PROFILE_DS, SUIT_PROFILE_SS, SUIT_PROFILE_RAINBOW,
            ...
            if mask.any():
                ...
                sum_lift_class[suit_code] += (best_ev - v46_ev)
    ...
    mean_v46 = sum_v46_ev / n_processed
    mean_oracle = sum_oracle_ev / n_processed
    mean_regret = mean_oracle - mean_v46
    print(f"\n  v46 mean EV per hand:      ${mean_v46 * EV_TO_DOL:>+10.2f}")
    print(f"  Oracle mean EV per hand:   ${mean_oracle * EV_TO_DOL:>+10.2f}")
    print(f"  Mean regret:               ${mean_regret * EV_TO_DOL * 1000:>+8.1f}/1000h within Q-high no-pair")
    print(f"  Whole-grid contribution:   ${mean_regret * EV_TO_DOL * 1000 * n_processed / n_total:>+8.1f}/1000h")
    print(f"  v46 == oracle:             {n_v46_eq_oracle:>7,}  ({100*n_v46_eq_oracle/n_processed:.1f}%)")

    print(f"\n── ORACLE TOP-CARD DISTRIBUTION ──")
    print(f"  Oracle picks Q on top: {oracle_top_is_Q:,} ({100*oracle_top_is_Q/n_processed:.2f}%)")
    ...
    print(f"\n── v46 TOP-CARD DISTRIBUTION ──")
    print(f"  v46 picks Q on top: {v46_top_is_Q:,} ({100*v46_top_is_Q/n_processed:.2f}%)")
    print(f"  Top-card rank distribution (v46):")
    for r in range(12, 1, -1):
        n = v46_top_distribution.get(r, 0)
        if n > 0:
            print(f"    {RANK_CHAR[r]}: {n:>7,}  ({100*n/n_processed:>5.2f}%)")
    ...
    print(f"\n── v46 BOT SUIT PROFILE DISTRIBUTION ──")
    for code, label in SUIT_LABELS.items():
        n = v46_bot_suit_dist.get(code, 0)
        print(f"  {label:<10}  {n:>7,}  ({100*n/n_processed:>5.2f}%)")

    print(f"\n── BEST-IN-CLASS MINUS v46 (S46 lens) ──")
    print(f"  {'class':<10} {'n_co':>9}  {'lift_vs_v46':>14}  {'whole_grid_full':>18}")
    print("-" * 65)
    for code, label in SUIT_LABELS.items():
        n = n_lift_class[code]
        if n == 0: continue
    ...
    print(f"  {'2nd':<3} {'n':>7} {'mean_v46':>9} {'mean_oracle':>12} "
          f"{'regret($/1000h)':>17} {'eq%':>6} {'oc_top_Q%':>10} "
          f"{'oc_DS%':>8} {'v46_DS%':>9}")
    for s2 in range(11, 1, -1):
        if by_s2[s2]["n"] == 0: continue
        st = by_s2[s2]
        mean_v46_s2 = st["sum_v46"] / st["n"]
        mean_oracle_s2 = st["sum_oracle"] / st["n"]
        regret_s2 = (mean_oracle_s2 - mean_v46_s2) * EV_TO_DOL * 1000
        eq_pct = 100 * st["n_eq"] / st["n"]
        topQ_pct = 100 * st["oracle_top_Q"] / st["n"]
        ds_oracle_pct = 100 * st["oracle_DS"] / st["n"]
        ds_v46_pct = 100 * st["v46_DS"] / st["n"]
        print(f"  {RANK_CHAR[s2]:<3} {st['n']:>7,} ${mean_v46_s2*EV_TO_DOL:>+7.2f} "
              f"${mean_oracle_s2*EV_TO_DOL:>+10.2f} ${regret_s2:>+13.1f} "
              f"{eq_pct:>5.1f}% {topQ_pct:>9.1f}% {ds_oracle_pct:>6.1f}% {ds_v46_pct:>7.1f}%")
