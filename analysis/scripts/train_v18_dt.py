"""
Session 28 — train_v18_dt: same regression-DT methodology as v16 but with
configurable depth/min_samples_leaf, and uses CACHED feature parquets
instead of recomputing from canonical hand bytes (saves the ~20-min
Python feature-compute pass).

Goal: see if a higher-capacity tree (e.g. depth=22, min_samples_leaf=50)
narrows the gap to oracle on the full grid. v16 is depth=18,
min_samples_leaf=100 with 28,790 leaves.

Run:
  PYTHONUNBUFFERED=1 python3 analysis/scripts/train_v18_dt.py \
      --max-depth 22 --min-samples-leaf 50 --output data/v18_dt_model.npz
"""
...
def main() -> int:
    ...
    ap.add_argument("--output", type=Path, default=ROOT / "data" / "v18_dt_model.npz")
    args = ap.parse_args()
    ...
    # Sanity: training-set recovery via sklearn predict.
    ...
    print(f"  v18 picked  mean EV:   {picked_ev:+.4f}", flush=True)
