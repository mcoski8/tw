#!/usr/bin/env python3
"""Grade v45_dt vs v44_dt (added 4 high_only_aug_v5 features)."""
...
def main() -> int:
    ...
    ap.add_argument("--baseline", choices=["v43", "v44"], default="v44")
    args = ap.parse_args()
    ...
    from strategy_v45_dt import strategy_v45_dt
    ...
    res_v45 = grade_strategy(strategy_v45_dt, grid, ch,
                              label="v45_dt (+ high_only_aug_v5)",
                              progress_every=progress_every)
    print(res_v45.summary(), flush=True)

    print("\n" + "=" * 70)
    print(f"v45_dt vs {baseline_label} ({args.grid} grid)")
    print("=" * 70)
    print(compare_grades(res_base, res_v45))
    delta = res_base.mean_regret - res_v45.mean_regret
    print(f"\nv45_dt vs {baseline_label}: {delta:+.4f}  ≈ ${delta * 10 * 1000:+,.0f}/1000h",
          flush=True)
    return 0
