"""
Session 55 — train_v40_dt: v39 (87) + 4 trips_pair_aug_v2 features = 91 total.

S55 Phase 1 diagnostic identified the trips_pair zone's largest mismatch
as v39 picking Pbot_SS while oracle picks Pbot_DS (10,398 hands at
$3,580 mean regret = $6.20/1000h whole-grid). Hand-level inspection
showed v39's existing tp_pair_routing_is_ds_g only captures route R1
(sings fill pair-suits), missing routes R2 (1 trip + 1 sing) and
R3 (2 trip cards). The 4 new rank-valued features describe the QUALITY
of Pbot_DS across all routings, mirroring pair_aug_v5's design.

Run:
  PYTHONUNBUFFERED=1 python3 analysis/scripts/train_v40_dt.py \\
      --max-depth 36 --min-samples-leaf 1 --output data/v40_dt_model.npz
"""
...
from train_v39_dt import build_X as build_X_v39, FEATURE_COLUMNS as V39_COLUMNS  # noqa: E402
from train_v27_dt import ALPHA_MAP  # noqa: E402

GRID_FULL = ROOT / "data" / "oracle_grid_full_realistic_n200.bin"
TP_V2_PATH = ROOT / "data" / "feature_table_trips_pair_aug_v2_gated.parquet"
...
FEATURE_COLUMNS = list(V39_COLUMNS) + TP_V2_COLUMNS  # 87 + 4 = 91


def build_X():
    X39, n = build_X_v39()
    print(f"  v39 base X: {X39.shape}  loading trips_pair_aug_v2 gated ...", flush=True)
    pv2 = pd.read_parquet(TP_V2_PATH)
    extras = np.empty((n, len(TP_V2_COLUMNS)), dtype=np.int16)
    for j, c in enumerate(TP_V2_COLUMNS):
        extras[:, j] = pv2[c].to_numpy().astype(np.int16, copy=False)
    X = np.concatenate([X39, extras], axis=1)
    return X, n
...
def main() -> int:
    ...
    ap.add_argument("--output", type=Path, default=ROOT / "data" / "v40_dt_model.npz")
    args = ap.parse_args()
    ...
    print(f"\nTRIPWIRE — new tp_v2_*_g features placement:")
    for new_col in TP_V2_COLUMNS:
