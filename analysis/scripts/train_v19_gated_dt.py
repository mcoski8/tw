"""
Session 30 — train_v19_gated_dt: extends train_v18_dt with the 6 GATED
suited-broadway aug features (zeros for non-high_only hands). Total
feature count: 37 + 6 = 43, but the 6 new features only fire on the
high_only subpopulation.

Saves data/v19_gated_dt_model.npz with the same npz layout.
Inference path is strategy_v19_gated_dt.py.

Run:
  PYTHONUNBUFFERED=1 python3 analysis/scripts/train_v19_gated_dt.py \
      --max-depth 26 --min-samples-leaf 20 --output data/v19_gated_dt_model.npz
"""
...
# 37 base features + 6 gated suited features = 43.
FEATURE_COLUMNS = [
...
def main() -> int:
    ...
    ap.add_argument("--output", type=Path, default=ROOT / "data" / "v19_gated_dt_model.npz")
    args = ap.parse_args()
