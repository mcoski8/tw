"""
Session 34 — train_v27_dt: extends train_v26 with 4 high_only-gated aug
features (zeros for non-high_only hands). Total feature count:
65 (v26) + 4 (high_only-gated) = 69.

The high_only category is the largest fully-untouched residual on v26:
  20.4% population × $2,894/1000h regret = $590/1000h share. Untouched
  since v20 (Session 30).

The 4 new features are the 6th gating template instance (after
suited/v20, trips_pair/v23, composite/v24, pair/v25, two_pair/v26):

    ho_n_broadway_in_2nd_suit_g    (0..3) — primary signal from
                                            Session 34 distillation
                                            (0.34-0.41 EV within-leaf
                                            separation in top miss leaves)
    ho_n_broadway_in_3rd_suit_g    (0..3)
    ho_connectivity_high_g         (0..5)
    ho_n_broadway_pairs_adj_g      (0..4)

Run:
  PYTHONUNBUFFERED=1 python3 analysis/scripts/train_v27_dt.py \\
      --max-depth 30 --min-samples-leaf 5 --output data/v27_dt_model.npz
"""
...
from train_v26_dt import build_X as build_X_v26, FEATURE_COLUMNS as V26_COLUMNS, ALPHA_MAP  # noqa: E402

GRID_FULL = ROOT / "data" / "oracle_grid_full_realistic_n200.bin"
HIGH_ONLY_GATED_PATH = ROOT / "data" / "feature_table_high_only_aug_gated.parquet"

HIGH_ONLY_GATED_COLUMNS = [
    "ho_n_broadway_in_2nd_suit_g",
    "ho_n_broadway_in_3rd_suit_g",
    "ho_connectivity_high_g",
    "ho_n_broadway_pairs_adj_g",
]
FEATURE_COLUMNS = list(V26_COLUMNS) + HIGH_ONLY_GATED_COLUMNS


def build_X():
    X26, n = build_X_v26()
    print(f"  v26 base X: {X26.shape}  loading high_only gated ...", flush=True)
    hog = pd.read_parquet(HIGH_ONLY_GATED_PATH)
    extras = np.empty((n, len(HIGH_ONLY_GATED_COLUMNS)), dtype=np.int16)
    for j, c in enumerate(HIGH_ONLY_GATED_COLUMNS):
        extras[:, j] = hog[c].to_numpy().astype(np.int16, copy=False)
    X = np.concatenate([X26, extras], axis=1)
    return X, n
...
def main() -> int:
    ...
    ap.add_argument("--output", type=Path, default=ROOT / "data" / "v27_dt_model.npz")
    args = ap.parse_args()
    ...
    print(f"  v27 picked mean EV:    {picked_ev:+.4f}", flush=True)
