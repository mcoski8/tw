"""
Session 84 Phase B — drill v57 on LOW × PMID_DS_MAXTOP.

CONTEXT
-------
S83 shipped v57 = v56 + Rule 20 (LOW pair × PMID_DS_NOMAXTOP × max_sing ≤ Q).
S77 ranked LOW pair cells by STRUCTURE leak; PMID_DS_NOMAXTOP was #1
($31.00 STRUCTURE / 1000h). The next-largest under-rule-covered LOW pair
cell is **PMID_DS_MAXTOP** ($21.68 STRUCTURE / 1000h under v44_dt, 128,304
cell hands).

By construction Rule 20 does NOT fire on this cell — Rule 20's trigger
requires `n_PMID_DS_w_maxtop == 0`, but the cell PMID_DS_MAXTOP is defined
by `n_PMID_DS_w_maxtop > 0`. So v57 = v56 on this cell, and we drill v57
(production) here to characterize leak structure.

S77 told us the dominant v44 mismatch is SPLIT_tmax_SS_mu → PMID_tmax_DS
(3,072 hands, $10.97 / 1000h) — i.e. v44 splits the low pair onto top when
oracle wants pair-in-mid with DS bot AND max kicker on top. But v56 routes
PAIR (non-PBOT-DS) through v52 and is NOT expected to make v44's SPLIT
mistake — so v57 (= v56) may already have closed the v44 SPLIT-leak here.

The question this drill answers: under PRODUCTION v57, what is the
residual leak structure on LOW × PMID_DS_MAXTOP? Is it within-PMID variant
choice (analogous to S83's PMID_DS_NOMAXTOP finding), is it PBOT routing
that v52 misses, or is the leak essentially closed?

WHAT WE DO
----------
  1. Load S77's per-hand parquet.
  2. Filter to LOW (rank 2-7) × PMID_DS_MAXTOP (cell_idx 2).
  3. Run strategy_v57_lo_pair_defensive on each filtered hand.
  4. Classify v57's pick via the S66/S77 classify_pick_pair taxonomy.
  5. Aggregate (v57_class, oracle_class) by count + total leak $.
  6. Verify Rule 20 fires 0 times on the cell (sanity check).

USAGE
-----
  PYTHONUNBUFFERED=1 python3 -u analysis/scripts/drill_v57_low_pmid_ds_maxtop_S84.py

OUTPUT
------
  data/session84/drill_v57_low_pmid_ds_maxtop_summary.json  (machine-readable)
...
OUT_JSON = OUT_DIR / "drill_v57_low_pmid_ds_maxtop_summary.json"

EV_TO_DOL = 10.0
N_TOTAL_GRID = 6_009_159

LOW_PAIR_RANKS = {2, 3, 4, 5, 6, 7}
CELL_PMID_DS_MAXTOP_IDX = CELLS_ORDER.index("PMID_DS_MAXTOP")  # = 2

EPS_REL = 0.005  # matches S77 / S83
...
def main() -> int:
    ...
    # Build the LOW × PMID_DS_MAXTOP mask.
    mask = (df["cell_idx"] == CELL_PMID_DS_MAXTOP_IDX) & np.isin(
        df["pair_rank"], list(LOW_PAIR_RANKS)
    )
    n_target = int(mask.sum())
    print(f"  LOW × PMID_DS_MAXTOP hands: {n_target:,}", flush=True)
    ...
    # --- Sanity: Rule 20 should never fire on PMID_DS_MAXTOP. ---
    print(f"sanity: Rule 20 (gate=A) fires on this cell: {rule20_fires_on_cell:,}")
    print(f"        expected: 0  (cell precondition n_PMID_DS_w_maxtop > 0)")
    ...
    # --- Console ---
    print("=" * 80)
    print(f"S84 Phase B — LOW × PMID_DS_MAXTOP  (n={n_target:,} hands)")
    print("=" * 80)
    ...
    out = {
        ...
        "cell_idx": CELL_PMID_DS_MAXTOP_IDX,
