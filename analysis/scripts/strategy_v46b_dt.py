"""
Session 73 — strategy_v46b_dt: v44 + 2 high_only_aug_v6 rank-valued
features (109 total), trained at v44's saturating regime (depth=36 ml=1).

This is the **regime-isolation retry** of v46_dt. Same 109 features, but
hyperparameters held identical to v44_dt's saturating regime. Disentangles
the S72 NULL into "feature was inert" vs "regime change killed capacity".

See SESSION_72_V46_DT_NULL_REPORT.md and CURRENT_PHASE.md S72.
"""
...
from high_only_aug_v6_features_gated import compute_high_only_v6_features_for_hand  # noqa: E402

_MODEL_CACHE: Optional[dict] = None
MODEL_PATH = ROOT / "data" / "v46b_dt_model.npz"
...
_HO_V6_NAMES = ["ho_v6_topMax_SS_ms_n_configs_g",
...
def load_model(path: Optional[Path] = None) -> dict:
    ...
    _MODEL_CACHE = {
        ...
        "ho_v6_index_in_full":    [feature_columns.index(c) for c in _HO_V6_NAMES],
...
def strategy_v46b_dt(hand: np.ndarray) -> int:
    model = load_model()
    ...
    ho_v6 = compute_high_only_v6_features_for_hand(h)
    ...
    for k, ci in enumerate(model["ho_v6_index_in_full"]):    full[ci] = ho_v6[k]
