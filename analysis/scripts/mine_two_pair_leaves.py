"""
Session 19 — two_pair miss-driven feature mining (Steps 1-3).

Modelled on mine_high_only_leaves.py / mine_pair_leaves.py. Filters
feature_table.parquet to (mode_count == 3 AND category == 'two_pair'),
trains a depth=None DecisionTreeClassifier on the current 27-feature set,
...
Two_pair shape: n_pairs == 2 AND no trips/quads. Hand decomposition has
2 pair-rank positions (4 cards) + 3 distinct-rank singletons.

Pair-aug (Session 17) and high_only-aug (Session 18) features are both
designed for n_pairs==1 / 7-distinct-rank shapes respectively, so both
return 0 on the two_pair slice and contribute nothing. Mining uses the
27 baseline features only; new features (this session's hypothesis) will
encode the structural blind spots specific to two_pair routing.

Output: prints a per-depth report, then for the top-50 leaves by miss
count: leaf id, n_samples, n_misses, miss-share, predicted-shape, top-3
true-shape distribution, sample hands tagged correct/miss with one-line
feature summary.
"""
...
print(f"\n=== depth=None DT on two_pair 3-of-4 slice ===")
...
for rank, (lid, n_s, n_miss, pred_setting, pred_shape, idxs) in enumerate(leaf_stats[:TOP_LEAVES]):
    ...
    for j in sample_miss:
        ...
        pl = int(df["pair_low_rank"].iloc[int(j)])
        ...
        print(f"    {hand_str(h):<26}  pairs={ph}/{pl} top={tr} 2nd={sr}  "
