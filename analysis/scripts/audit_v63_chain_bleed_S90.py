"""S90 chain bleed audit — identify which rule layer (v44→v47→v48→v52) is
responsible for the +$7.23/1000h bleed on HIGH_ONLY × max ∈ {8, 9, T}.

This is the FIRST audit at max ≤ T territory. The crucial open question:
v52-defensive-low (the firing mode for these hands) was specifically
designed for weak-high HIGH_ONLY hands. Does it carry the v47 chain bleed
that drove S87/S88/S89, or is the bleed introduced HERE by a different
layer?

Method: evaluate v44_dt, v47, v48, v52 on the cell hands. Compute
per-version cell leak. The transition that introduces the bleed is the bad
rule layer. v52 == v57 == v63 on these cells (v63's gate fires on
HIGH_ONLY × {J-A}, not {8-T}).

Output:
  - per-cell × rank layer attribution
  - by v52 firing mode within each cell — KEY DIAGNOSTIC for v52-defensive-low
"""
...
def main() -> int:
    print("=" * 100)
    print("S90 chain bleed audit — per-layer attribution on HIGH_ONLY × {8, 9, T}")
    print("=" * 100)
    ...
    print(f"  filtered to {len(sub):,} hands across S90 target cells")
    print()
    ...
    # Diagnostic: what are LOW_MAX_DEFENSIVE / GATED_DEFENSIVE_MAX values?
    print(f"  v52 firing-mode parameters:")
    print(f"    LOW_MAX_DEFENSIVE = {sorted(LOW_MAX_DEFENSIVE)}")
    print(f"    GATED_DEFENSIVE_MAX = {sorted(GATED_DEFENSIVE_MAX)}")
    print(f"    GATED_S2_THRESHOLD = {GATED_S2_THRESHOLD}")
    print(f"    J_HIGH_OFFENSIVE = {J_HIGH_OFFENSIVE}")
    print()
    ...
    print()
    print(f"  Σ Δ across all S90 cells: "
    ...
    # === Net bleed by mode (rolled up across all S90 cells) ===
    print("=" * 120)
    print("NET BLEED BY FIRING MODE (rolled up across all S90 cells)")
    print("=" * 120)
    ...
    print(f"  TOTAL BLEED (sum of v52 − v44 leak across S90 cells): "
