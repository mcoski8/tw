#!/usr/bin/env python3
"""Grade v44_dt vs v43_dt (added 4 high_only_aug_v4 features)."""
...
def main() -> int:
    ...
    ap.add_argument("--baseline", choices=["v42", "v43"], default="v43")
    args = ap.parse_args()
    ...
    if args.baseline == "v43":
        from strategy_v43_dt import strategy_v43_dt as baseline_fn
        baseline_label = "v43_dt"
    else:
    ...
    res_v44 = grade_strategy(strategy_v44_dt, grid, ch,
                              label="v44_dt (+ high_only_aug_v4)",
                              progress_every=progress_every)
    print(res_v44.summary(), flush=True)

    print("\n" + "=" * 70)
    print(f"v44_dt vs {baseline_label} ({args.grid} grid)")
    print("=" * 70)
    print(compare_grades(res_base, res_v44))
    delta = res_base.mean_regret - res_v44.mean_regret
    print(f"\nv44_dt vs {baseline_label}: {delta:+.4f}  ≈ ${delta * 10 * 1000:+,.0f}/1000h",
          flush=True)
    return 0
