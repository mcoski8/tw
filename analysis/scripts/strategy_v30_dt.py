"""
Session 36 — strategy_v30_dt: extends v29 with 6 trips-gated aug
features (the `trips_*_g` family). At runtime computes all 8 gated
families plus base vector, walks the saved tree.
"""
...
MODEL_PATH = ROOT / "data" / "v30_dt_model.npz"
...
def load_model(path: Optional[Path] = None) -> dict:
    ...
    pair_set = set(_PAIR_GATED_NAMES)
    twopair_set = set(_TWO_PAIR_GATED_NAMES)
    ho_set = set(_HIGH_ONLY_GATED_NAMES)
    pair_r4_set = set(_PAIR_R4_GATED_NAMES)
    trips_set = set(_TRIPS_GATED_NAMES)
    base_columns = [
        c for c in feature_columns
        if c not in suited_set and c not in tp_set and c not in comp_set
           and c not in pair_set and c not in twopair_set and c not in ho_set
           and c not in pair_r4_set and c not in trips_set
    ]
...
def strategy_v30_dt(hand: np.ndarray) -> int:
    model = load_model()
