"""
Phase D diagnostic: depth-vs-agreement ceiling curve on the 3-of-4 majority
subset only.

This filters the 6M canonical-hand dataset down to hands where mode_count == 3
(2,432,648 hands). Those have a strict-majority answer (3 of 4 profiles agree)
but the answer is non-trivial — the 4th profile dissents.

Hypothesis: if the 27-feature set is structurally adequate for capturing
clear-majority signals, the depth=None ceiling here should be HIGH (>90%).
If it caps near the 6M-wide ceiling (~62%), the features themselves cannot
see the signal even when the answer key is unambiguous.

Output: depth-vs-agreement table identical in shape to dt_phase1.py.
"""
...
# Filter to mode_count == 3 (3-of-4 strict majority subset).
mask = (df["mode_count"] == 3).values
df = df[mask].reset_index(drop=True)
hands = hands_all[mask]
N = len(df)
print(f"3-of-4 majority subset: {N:,} hands "
...
# Identical feature engineering to dt_phase1.py.
...
# Subsample for CV — use min(1M, N) so we always sweep on a fixed-size set.
sub_n = min(1_000_000, N)
...
print(f"\nReference: depth=None ceiling on full 6M was 61.74% / 57.24% (full / cv).")
print(f"This subset is mode_count==3 only — strict-majority answer key.")
