#!/usr/bin/env python3
"""Grade v38_dt vs v36_dt (added 2 pair_aug_v4 features at same capacity)."""
...
def main() -> int:
    ...
    from strategy_v38_dt import strategy_v38_dt
    ...
    res_v38 = grade_strategy(strategy_v38_dt, grid, ch,
                              label="v38_dt (v36 + pair_aug_v4)",
                              progress_every=progress_every)
    print(res_v38.summary(), flush=True)

    print("\n" + "=" * 70)
    print(f"v38_dt vs v36_dt ({args.grid} grid)")
    print("=" * 70)
    print(compare_grades(res_v36, res_v38))
    delta = res_v36.mean_regret - res_v38.mean_regret
    print(f"\nv38_dt vs v36_dt: {delta:+.4f}  ≈ ${delta * 10 * 1000:+,.0f}/1000h",
          flush=True)
    return 0
