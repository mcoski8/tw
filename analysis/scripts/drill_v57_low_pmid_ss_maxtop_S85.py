"""
Session 85 Phase B — drill v57 on LOW × PMID_SS_MAXTOP.

CONTEXT
-------
S83 shipped v57 = v56 + Rule 20 (LOW pair × PMID_DS_NOMAXTOP × max_sing ≤ Q).
S84 MIXED on the next cell, LOW × PMID_DS_MAXTOP (Rule 21 candidate cleared
the prefix SHIP gate but failed the full-grid SHIP gate; precedent requires
both grids to clear $5).

S77 ranking of LOW pair under-rule-covered cells by STRUCTURE leak (under
v44_dt baseline):
  1. PMID_DS_NOMAXTOP — $31.00 (S83 SHIPPED Rule 20)
  2. PMID_DS_MAXTOP   — $21.68 (S84 MIXED — did NOT ship)
  3. PMID_SS_MAXTOP   — $9.71  (S85 default target — THIS DRILL)
  4. PMID_OTHER       — $11.81 (catchall — S85 alternative)

PMID_SS_MAXTOP cell precondition:
  - n_PBOT_DS == 0  AND
  - n_PMID_DS == 0  AND
  - n_PMID_SS_w_maxtop > 0

By construction Rule 20 does NOT fire here either: Rule 20's trigger requires
n_PMID_DS > 0 (specifically n_PMID_DS_w_maxtop == 0 AND n_PMID_DS > 0); but the
PMID_SS_MAXTOP cell has n_PMID_DS == 0 entirely. So v57 = v56 = v52 on this
cell, and we drill v57 (production) to characterise residual leak structure.

S77 told us the dominant v44 mismatches were:
  STRUCTURE bucket:
    - SPLIT_tmax_SS_mu → PMID_tmax_SS  (732 hands, $1.99/1000h)
    - PBOT_tmax_RB_mu → PMID_tmax_SS   (482 hands, $0.94)
    - SPLIT_tmax_RB_mu → PMID_tmax_SS  (192 hands, $0.73)
    - SPLIT_tmax_31_mu → PMID_tmax_SS  (119 hands, $0.31)
    - SPLIT_tmax_SS_mu → PMID_tnomax_31 (103 hands, $0.30)
    - SPLIT_tmax_SS_mu → PBOT_tmax_SS_mu (102 hands, $0.26)
  MID bucket:
    - PBOT_tmax_SS_mu → PMID_tmax_SS (1090 hands, $1.19)
    - PMID_tmax_SS → PBOT_tmax_SS_mu (1128 hands, $0.75)
    - PMID_tmax_SS → PBOT_tmax_SS_ms (860 hands, $0.67)
    - PBOT_tmax_RB_mu → PMID_tmax_SS (479 hands, $0.62)

Note: v44's leak is dominated by SPLIT / PBOT-RB → PMID transitions which v52
already routes correctly (pair-to-mid by default). We expect v57 to close most
of v44's STRUCTURE leak. The residual will likely be within-PMID variant
choice (PMID_tmax_SS vs PMID_tnomax_SS / PMID_tmax_31 / etc) and the PBOT_SS
side-channel (oracle sometimes wants pair-in-bot when mid is same-suit and
top is max).

WHAT WE DO
----------
  1. Load S77's per-hand parquet.
  2. Filter to LOW (rank 2-7) × PMID_SS_MAXTOP (cell_idx 4).
...
EARLY-OUT CHECK
---------------
Per S85 resume prompt: if v57 residual leak < $5/1000h whole-grid, no
single rule can ship (ceiling argument). In that case, log the result
and recommend pivoting to LOW × PMID_OTHER or MID pair extension.

USAGE
-----
  PYTHONUNBUFFERED=1 python3 -u analysis/scripts/drill_v57_low_pmid_ss_maxtop_S85.py

OUTPUT
------
  data/session85/drill_v57_low_pmid_ss_maxtop_summary.json
...
OUT_JSON = OUT_DIR / "drill_v57_low_pmid_ss_maxtop_summary.json"

EV_TO_DOL = 10.0
N_TOTAL_GRID = 6_009_159

LOW_PAIR_RANKS = {2, 3, 4, 5, 6, 7}
CELL_PMID_SS_MAXTOP_IDX = CELLS_ORDER.index("PMID_SS_MAXTOP")  # = 4

EPS_REL = 0.005  # matches S77 / S83 / S84

# Early-out: by S85 resume prompt — if total v57 leak < $5/1000h whole-grid,
# no single rule can ship by ceiling argument. Pivot recommended.
...
def main() -> int:
    ...
    # Build the LOW × PMID_SS_MAXTOP mask.
    mask = (df["cell_idx"] == CELL_PMID_SS_MAXTOP_IDX) & np.isin(
        df["pair_rank"], list(LOW_PAIR_RANKS)
    )
    n_target = int(mask.sum())
    print(f"  LOW × PMID_SS_MAXTOP hands: {n_target:,}", flush=True)
    ...
    rule20_fires_on_cell = 0  # sanity counter — should be 0

    for i in range(n_target):
        cid = int(target_canonical_ids[i])
        hand_bytes = np.asarray(hands_arr[cid], dtype=np.uint8)

        # Sanity check: Rule 20 (max gate 14 = always-on) must NOT fire on this cell.
    ...
    # --- Sanity: Rule 20 should never fire on PMID_SS_MAXTOP. ---
    print(f"sanity: Rule 20 (gate=A) fires on this cell: {rule20_fires_on_cell:,}")
    print(f"        expected: 0  (cell precondition n_PMID_DS == 0)")
    if rule20_fires_on_cell != 0:
        print(f"  WARNING: Rule 20 fired {rule20_fires_on_cell} times on the cell — "
              f"v57 ≠ v56 here, taxonomy bug?", flush=True)
    ...
    # --- Console ---
    print("=" * 80)
    print(f"S85 Phase B — LOW × PMID_SS_MAXTOP  (n={n_target:,} hands)")
    print("=" * 80)
    ...
    # ---- Early-out check (S85 resume prompt) ----
    print("\n" + "=" * 80)
    print(f"EARLY-OUT CEILING CHECK (S85)")
    print("=" * 80)
    ...
    if total_v57_leak < EARLY_OUT_THRESHOLD:
        ...
        print(f"         Recommend pivot to LOW × PMID_OTHER or MID pair extension.")
        ...
        print(f"         total cell leak is ${total_v57_leak:.2f} but STRUCTURE alone is")
        print(f"         ${bucket_summary['STRUCTURE']['wg_dollars']:.2f}; remaining is MID/NOISE which a single")
        print(f"         placement rule unlikely to help. Recommend pivot.")
    else:
        ceiling_verdict = "PROCEED-PHASE-B+"
        print(f"VERDICT: {ceiling_verdict} — cell residual is large enough; proceed to discriminator drill.")

    out = {
        "session": 85,
        "cell": "LOW × PMID_SS_MAXTOP",
        "cell_idx": CELL_PMID_SS_MAXTOP_IDX,
