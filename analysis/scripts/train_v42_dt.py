"""
Session 56 — train_v42_dt: v41 (95) + 4 high_only_aug_v2 features = 99 total.

S56 Phase 1 diagnostic identified the high_only zone's largest residual
as a systematic DS-bot under-routing: v41 picks SS bot 48% vs oracle
33% and picks DS bot 35% vs oracle 52% (−17% absolute under-routing).
Top-mismatch class: tA_SS_mu → tA_*_DS. Existing high_only features
describe broadway distribution but do not expose "what bot configurations
yield DS while preserving an A/K/Q top." The 4 new ho_v2 features mirror
pair_aug_v5 / trips_pair_v2 / two_pair_v2: rank-valued conditional
features describing achievability + quality of DS-bot routings.

Run:
  PYTHONUNBUFFERED=1 python3 analysis/scripts/train_v42_dt.py \\
      --max-depth 36 --min-samples-leaf 1 --output data/v42_dt_model.npz
"""
...
from train_v41_dt import build_X as build_X_v41, FEATURE_COLUMNS as V41_COLUMNS  # noqa: E402
from train_v27_dt import ALPHA_MAP  # noqa: E402

GRID_FULL = ROOT / "data" / "oracle_grid_full_realistic_n200.bin"
HO_V2_PATH = ROOT / "data" / "feature_table_high_only_aug_v2_gated.parquet"
...
FEATURE_COLUMNS = list(V41_COLUMNS) + HO_V2_COLUMNS  # 95 + 4 = 99


def build_X():
    X41, n = build_X_v41()
    print(f"  v41 base X: {X41.shape}  loading high_only_aug_v2 gated ...", flush=True)
    pv2 = pd.read_parquet(HO_V2_PATH)
    extras = np.empty((n, len(HO_V2_COLUMNS)), dtype=np.int16)
    for j, c in enumerate(HO_V2_COLUMNS):
        extras[:, j] = pv2[c].to_numpy().astype(np.int16, copy=False)
    X = np.concatenate([X41, extras], axis=1)
    return X, n
...
def main() -> int:
    ...
    ap.add_argument("--output", type=Path, default=ROOT / "data" / "v42_dt_model.npz")
    args = ap.parse_args()
    ...
    print(f"\nTRIPWIRE — new ho_v2_*_g features placement:")
    for new_col in HO_V2_COLUMNS:
