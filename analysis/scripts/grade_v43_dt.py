#!/usr/bin/env python3
"""Grade v43_dt vs v42_dt (added 4 high_only_aug_v3 features)."""
...
def main() -> int:
    ...
    ap.add_argument("--baseline", choices=["v41", "v42"], default="v42")
    args = ap.parse_args()
    ...
    if args.baseline == "v42":
        from strategy_v42_dt import strategy_v42_dt as baseline_fn
        baseline_label = "v42_dt"
    else:
        from strategy_v41_dt import strategy_v41_dt as baseline_fn
        baseline_label = "v41_dt"
    from strategy_v43_dt import strategy_v43_dt
    ...
    res_v43 = grade_strategy(strategy_v43_dt, grid, ch,
                              label="v43_dt (+ high_only_aug_v3)",
                              progress_every=progress_every)
    print(res_v43.summary(), flush=True)

    print("\n" + "=" * 70)
    print(f"v43_dt vs {baseline_label} ({args.grid} grid)")
    print("=" * 70)
    print(compare_grades(res_base, res_v43))
    delta = res_base.mean_regret - res_v43.mean_regret
    print(f"\nv43_dt vs {baseline_label}: {delta:+.4f}  ≈ ${delta * 10 * 1000:+,.0f}/1000h",
          flush=True)
    return 0
