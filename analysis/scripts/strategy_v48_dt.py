"""
Session 78 — strategy_v48_dt: v44 + H6/H7/H8 pair features = 110 total.

Mirrors strategy_v44_dt.py with three additional pair-gated feature blocks:
  H6: pair_pmid_ds_n_configs_g       (single-pair)
  H7: pair_kicker_max_in_pair_suit_g (single-pair)
  H8: pair_low_pmid_safety_g         (LOW pair only)
"""
...
from two_pair_aug_v2_features_gated import compute_two_pair_v2_features_for_hand  # noqa: E402
from high_only_aug_v2_features_gated import compute_high_only_v2_features_for_hand  # noqa: E402
from high_only_aug_v3_features_gated import compute_high_only_v3_features_for_hand  # noqa: E402
from high_only_aug_v4_features_gated import compute_high_only_v4_features_for_hand  # noqa: E402
from pair_pmid_ds_features_gated import compute_pair_pmid_ds_n_configs_for_hand  # noqa: E402
from pair_kicker_align_features_gated import compute_pair_kicker_max_in_pair_suit_for_hand  # noqa: E402
from pair_low_pmid_safety_features_gated import compute_pair_low_pmid_safety_for_hand  # noqa: E402

_MODEL_CACHE: Optional[dict] = None
MODEL_PATH = ROOT / "data" / "v48_dt_model.npz"
...
_T2P_V2_NAMES = ["t2p_v2_layout_C_DS_n_configs_g", "t2p_v2_layout_C_max_top_rank_g",
                  "t2p_v2_layout_B_max_top_rank_g", "t2p_v2_layout_C_DS_max_top_rank_g"]
_HO_V2_NAMES = ["ho_v2_bot_DS_n_configs_g", "ho_v2_bot_DS_max_top_rank_g",
                 "ho_v2_bot_DS_min_top_rank_g", "ho_v2_bot_DS_max_mid_sum_g"]
_HO_V3_NAMES = ["ho_v3_topMax_DS_ms_n_configs_g", "ho_v3_topMax_DS_ms_max_mid_high_g",
                 "ho_v3_topMax_DS_ms_min_mid_high_g", "ho_v3_topMax_DS_ms_max_mid_sum_g"]
_HO_V4_NAMES = ["ho_v4_topMax_DS_max_bot_pair_high_g", "ho_v4_topMax_4f_ms_max_mid_high_g",
                 "ho_v4_topNonMax_DS_ms_n_configs_g", "ho_v4_topNonMax_DS_ms_max_top_rank_g"]
_H6_NAMES = ["pair_pmid_ds_n_configs_g"]
_H7_NAMES = ["pair_kicker_max_in_pair_suit_g"]
_H8_NAMES = ["pair_low_pmid_safety_g"]
...
def load_model(path: Optional[Path] = None) -> dict:
    ...
    all_gated = [
        ...
        set(_T2P_V2_NAMES), set(_HO_V2_NAMES), set(_HO_V3_NAMES), set(_HO_V4_NAMES),
        set(_H6_NAMES), set(_H7_NAMES), set(_H8_NAMES),
    ]
    ...
    _MODEL_CACHE = {
        ...
        "t2p_v2_index_in_full":   [feature_columns.index(c) for c in _T2P_V2_NAMES],
        "ho_v2_index_in_full":    [feature_columns.index(c) for c in _HO_V2_NAMES],
        "ho_v3_index_in_full":    [feature_columns.index(c) for c in _HO_V3_NAMES],
        "ho_v4_index_in_full":    [feature_columns.index(c) for c in _HO_V4_NAMES],
        "h6_index_in_full":       [feature_columns.index(c) for c in _H6_NAMES],
        "h7_index_in_full":       [feature_columns.index(c) for c in _H7_NAMES],
        "h8_index_in_full":       [feature_columns.index(c) for c in _H8_NAMES],
...
def strategy_v48_dt(hand: np.ndarray) -> int:
    model = load_model()
    ...
    t2p_v2 = compute_two_pair_v2_features_for_hand(h)
    ho_v2 = compute_high_only_v2_features_for_hand(h)
    ho_v3 = compute_high_only_v3_features_for_hand(h)
    ho_v4 = compute_high_only_v4_features_for_hand(h)
    h6 = (compute_pair_pmid_ds_n_configs_for_hand(h),)
    h7 = (compute_pair_kicker_max_in_pair_suit_for_hand(h),)
    h8 = (compute_pair_low_pmid_safety_for_hand(h),)
    ...
    for k, ci in enumerate(model["t2p_v2_index_in_full"]):   full[ci] = t2p_v2[k]
    for k, ci in enumerate(model["ho_v2_index_in_full"]):    full[ci] = ho_v2[k]
    for k, ci in enumerate(model["ho_v3_index_in_full"]):    full[ci] = ho_v3[k]
    for k, ci in enumerate(model["ho_v4_index_in_full"]):    full[ci] = ho_v4[k]
    for k, ci in enumerate(model["h6_index_in_full"]):       full[ci] = h6[k]
    for k, ci in enumerate(model["h7_index_in_full"]):       full[ci] = h7[k]
    for k, ci in enumerate(model["h8_index_in_full"]):       full[ci] = h8[k]
