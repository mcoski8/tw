"""
Session 85 Phase C — grade strategy_v59_lo_pair_ss_tnomax at multiple gates
(full grid, cell hands only).

Since v59 only changes picks on LOW × PMID_SS_MAXTOP hands (85,536 of
6,009,159 = 1.4% of grid), and all other hands route to v57 unchanged, the
whole-grid lift can be computed by iterating ONLY the 85k cell hands and
summing per-hand delta = ev[v59_pick] - ev[v57_pick].

Grades each max_sing gate ∈ {9, 10, 11, 12, 13, 14} and reports:
  - n hands fired
  - n hands "swap-right" (v59 closer to oracle than v57)
  - sum delta in EV units
  - lift in $/1000h whole-grid
  - cell-leak under v57 vs v59
...
USAGE
-----
  PYTHONUNBUFFERED=1 python3 -u analysis/scripts/grade_v59_lo_pair_ss_tnomax_S85.py
"""
...
OUT_JSON = OUT_DIR / "grade_v59_summary.json"

EV_TO_DOL = 10.0
N_TOTAL_GRID = 6_009_159

LOW_PAIR_RANKS = {2, 3, 4, 5, 6, 7}
CELL_IDX_PMID_SS_MAXTOP = 4

# Gates to grade (max_sing thresholds — rule fires when max_sing ≤ gate)
...
# prefix-grid SHIP (graded separately by grade_v59_prefix_multigate_S85.py).
...
def main() -> int:
    ...
    for gate in GATES:
        ...
        print(f"  n_changed (v59 != v57): {n_changed:>10,}")
        print(f"  n_v59_closer:           {n_v59_closer:>10,}  (v59 reduced regret)")
        print(f"  n_v59_worse:            {n_v59_worse:>10,}  (v59 increased regret)")
        print(f"  n_v59_match_oracle:     {n_v59_match_oracle:>10,}  (v59 == oracle pick)")
        ...
        print(f"  cell leak v59:          ${cell_leak_v59:>9.2f}/1000h whole-grid")
        print(f"  LIFT v59 over v57:      ${lift_dol:>+9.2f}/1000h whole-grid")
    ...
    print(f"\nOverall S85 candidate verdict (full grid only): {best['verdict']}")
