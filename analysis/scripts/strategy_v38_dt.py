"""
Session 54 — strategy_v38_dt: v36 (83 features) + 2 pair_aug_v4 features (85 total).
Runtime feature pipeline extends strategy_v31b_dt with pair_aug_v4.
"""
...
from pair_aug_v4_features_gated import compute_pair_v4_features_for_hand  # noqa: E402

_MODEL_CACHE: Optional[dict] = None
MODEL_PATH = ROOT / "data" / "v38_dt_model.npz"
...
_PAIR_V4_NAMES = ["pair_aug_v4_bot_DS_achievable_g", "pair_aug_v4_n_sings_in_pair_suits_g"]
...
def load_model(path: Optional[Path] = None) -> dict:
    ...
    all_gated_groups = [
        set(_SUITED_NAMES), set(_TP_NAMES), set(_COMP_NAMES),
        set(_PAIR_NAMES), set(_TWOP_NAMES), set(_HO_NAMES),
        set(_PAIR_R4_NAMES), set(_TRIPS_NAMES), set(_TRIPS_V2_NAMES),
        set(_PAIR_V4_NAMES),
    ]
    base_columns = [c for c in feature_columns
                     if not any(c in gs for gs in all_gated_groups)]
    _MODEL_CACHE = {
        ...
        "pair_v4_index_in_full":  [feature_columns.index(c) for c in _PAIR_V4_NAMES],
        "cat_map": cat_map,
        "depth": int(arr["depth"]),
...
def strategy_v38_dt(hand: np.ndarray) -> int:
    model = load_model()
    ...
    pair_v4 = compute_pair_v4_features_for_hand(h)
    ...
    for k, ci in enumerate(model["pair_v4_index_in_full"]):  full[ci] = pair_v4[k]
