"""
Session 32 — strategy_v25_dt: extends v24 with the 6 pair-gated aug
features. At runtime computes all 4 gated families plus base vector,
...
MODEL_PATH = ROOT / "data" / "v25_dt_model.npz"
...
def load_model(path: Optional[Path] = None) -> dict:
    ...
    base_columns = [
        ...
        if c not in suited_set and c not in tp_set and c not in comp_set and c not in pair_set
    ]
...
def strategy_v25_dt(hand: np.ndarray) -> int:
    model = load_model()
