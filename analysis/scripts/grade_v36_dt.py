#!/usr/bin/env python3
"""Grade v36_dt vs v34_dt (capacity-only retrain at depth=36 ml=1)."""
...
def main() -> int:
    ...
    from strategy_v34_dt import strategy_v34_dt
    from strategy_v36_dt import strategy_v36_dt

    res_v34 = grade_strategy(strategy_v34_dt, grid, ch,
                              label="v34_dt (current ML champion)",
                              progress_every=progress_every)
    print(res_v34.summary(), flush=True)

    res_v36 = grade_strategy(strategy_v36_dt, grid, ch,
                              label="v36_dt (depth=36 ml=1 retrain)",
                              progress_every=progress_every)
    print(res_v36.summary(), flush=True)

    print("\n" + "=" * 70)
    print(f"v36_dt vs v34_dt ({args.grid} grid)")
    print("=" * 70)
    print(compare_grades(res_v34, res_v36))
    delta = res_v34.mean_regret - res_v36.mean_regret
    print(f"\nv36_dt vs v34_dt: {delta:+.4f}  ≈ ${delta * 10 * 1000:+,.0f}/1000h",
          flush=True)
    return 0
