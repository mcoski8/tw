"""
Phase 1: decision-tree complexity-vs-agreement ceiling curve.

Predicts setting_index (0..104, 105 classes), scored by SHAPE-equivalence
between predicted and true (multiway_robust). This avoids the 254K-class
explosion of shape-tuple as a label while still using shape-agreement
(the project's stated metric) as the evaluation.

Output: per-depth (cv_acc, cv_shape, full_train_acc, full_train_shape,
n_leaves, fit_time). Use this to pick the depth where the curve hits the
knee.
"""
...
def shape_agreement(predicted, indices):
    """Score predicted setting_index against target SHAPE."""
    n = len(predicted)
    correct = 0
    for k in range(n):
        i = indices[k]
...
print(f"\nv3 baseline (current rule chain) shape-agreement on full 6M: 56.16%")
print(f"Inter-profile floor implies opponent-agnostic ceiling ≈ 83.2%.")
