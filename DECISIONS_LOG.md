...
## Decision 124 — Session 89 v63 SHIPS Rule 23 (chain gate-out closure on JOINT_MED + JOINT_LOW × {J-A}): the S87/S88 chain-audit pattern reapplied to the remaining two prefix-silent HIGH_ONLY × {J-A} cells (JOINT_MED × {J-A} = 44,562 hands, JOINT_LOW × {J-A} = 3,570 hands; total 48,132) uncovered another +$10.09/1000h v47-chain bleed; v63 extends v62's gate as a strict superset (cell ∈ {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH, JOINT_MED, JOINT_LOW} for HIGH_ONLY × max ∈ {J-A}); full-grid grader auto-fires SHIP at +$10.09/1000h whole-grid (v62 $1,610.04 → v63 $1,620.13); NEITHER × {J-A} proven structurally empty so HIGH_ONLY × {J-A} audit is now CLOSED; combined S87+S88+S89 chain-audit recovery = $207.60/1000h

### Session

Session 89 — planned execution of the S88-defined PRIMARY path: chain-audit expansion to the remaining prefix-silent HIGH_ONLY × {J-A} cells (JOINT_MED, JOINT_LOW, NEITHER). Phase A surfaced two surprises: NEITHER is structurally empty (combinatorial proof, no test needed); JOINT_MED + JOINT_LOW combined is ~14× smaller than S87 and ~7× smaller than S88. Pre-committed thresholds calibrated to the smaller cell size (SHIP $5 vs S88's $30). Three consecutive sessions with v47 as the dominant culprit; HIGH_ONLY × {J-A} audit now closed at $207.60/1000h total recovery.

### Context

S88 (Decision 123) shipped v62 via the S87 chain-audit pattern applied to DS_NO_MAXTOP + MS_ONLY + JOINT_HIGH × {J-A}, recovering another $98.84/1000h. S88's path-forward section identified four S89 candidates; PRIMARY was "continue chain-audit expansion to JOINT_MED + JOINT_LOW + NEITHER × {J-A}". S89 inherited this plan verbatim from CURRENT_PHASE.md; no mid-session strategic redirect from user.

### What S89 ran (phases)

**Phase A — S71 data review (no new compute).** Queried `drill_v44_high_only_S71_summary.json` and the per-hand parquet for the three target cells. Findings:

* HIGH_ONLY × JOINT_MED × {J-A}: 44,562 hands, cid_min 593,079 (prefix-silent ✓), S71 v44 baseline leak $6.23/1000h
* HIGH_ONLY × JOINT_LOW × {J-A}:  3,570 hands, cid_min 590,709 (prefix-silent ✓), S71 v44 baseline leak $0.13/1000h
* HIGH_ONLY × NEITHER × {J-A}: **0 hands across all ranks** — structurally empty

**Combinatorial proof that NEITHER × {J-A} is empty:** NEITHER requires both n_DS=0 AND n_ms_mid_with_max_top=0. The latter requires "no two non-max cards share a suit." A HIGH_ONLY hand has 7 distinct-rank cards, so 6 non-max cards. By pigeonhole on 4 suits, ≥2 of the 6 must share a suit → n_ms_mid_with_max_top ≥ 1 → cell ≠ NEITHER. So NEITHER × HIGH_ONLY is impossible for any max-rank.

NEITHER dropped from S89 target set with no loss. Combined target: 48,132 hands.

**Phase B — addressability pre-drill (6s compute, 48K hands).** Wrote `drill_v62_high_only_addressability_S89.py`. Re-evaluated v62 (production) on the 48K hands. Result:
...
**v62 leaks +$10.09/1000h MORE than v44_dt on these cells.** Per-cell × rank, all 4 JOINT_MED rank cells (J/Q/K/A) cleared the gate-out criterion (Δ ≥ $1, v44 leak ≥ $0.05 — threshold relaxed from S88's $1 since these cells are tiny). JOINT_LOW × {J-A} cells cleared the chain-bleed gate (a) but missed the v44-floor gate (b); the cell is too small individually to support a "v44 ≥ $0.05" criterion. Combined $1.85 contribution still contributes to the aggregate ship.

Override activity: 46-56% on JOINT_MED, 72-93% on JOINT_LOW.

Addressability gate verdict: **GATE-OUT** on all 4 JOINT_MED cells (and all 4 JOINT_LOW cells in aggregate, even with the WEAK per-cell flag).

**Phase B+ — v63 chain audit (9s compute, 48K hands × 3 strategies).** Wrote `audit_v62_chain_bleed_S89.py`. Layer attribution:
...
**96% of the bleed comes from the v44→v47 transition.** Third consecutive session with v47 as the dominant culprit. v47→v48 contributes a small improvement (−$0.44, mostly on MS_ONLY-J behavior leaking into the audit); v48→v52 contributes a small bleed (+$0.85, mostly v52-defensive-gated and v52-J-HIMID).
...
**Phase C — v63 design + full-grid grade (49s compute, 48K hands × 2 strategies + 50K out-of-gate sanity).** Wrote `strategy_v63_high_only_chain_fix_full.py`. Triggers on HIGH_ONLY × max ∈ {J,Q,K,A} × cell ∈ {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH, JOINT_MED, JOINT_LOW} — all 6 non-empty cells in the HIGH_ONLY × {J-A} zone. Strict superset of v62's gate. Cell detector match against S71 ground truth: 100.00%.

Pre-committed thresholds locked in code BEFORE grader ran: SHIP ≥ $5/1000h whole-grid (lowered from S87/S88's $30 to reflect smaller cell population), NULL ≤ $1/1000h, MIXED in between.

**Grader auto-fired SHIP at +$10.09/1000h whole-grid lift.** Per-cell breakdown matched audit prediction exactly. Per-hand effect: 48.8% same, 43.6% better, 7.6% worse. **Swap-right rate on 24,637 changed hands: 85.2%** — HIGHEST across S87/S88/S89 (S87: 62.3%, S88: 76.8%, S89: 85.2%). Out-of-gate sanity: **0 v63≠v62 disagreements** on 50,000-hand random sample.

Implied production: $1,610.04 → **$1,620.13/1000h** (+$10.09, +0.6%).

### Verdict + production state

**VERDICT: SHIP.** Pre-committed SHIP threshold cleared by 2×.

**Production ADVANCES to v63_high_only_chain_fix_full.**
* Full grid: $1,610.04 → **$1,620.13/1000h** (+$10.09)
* Prefix grid: $776.88 → **$776.88/1000h** UNCHANGED (rule fires entirely outside prefix coverage)
* v44_dt ML champion: UNCHANGED ($1,081 full / $686 prefix)
* Production vs v44_dt: **$539.13/1000h** (was $529)
* Remaining gap to oracle ceiling (the "two-track divergence"): **$125.07/1000h** (was $135.16)
* Cumulative closure since pre-S68: **$1,283.93 of $1,409 = 91.1%** (was 90.4%)
* Total project rule count: 22 → **23**

### Why N=1000 validation was not blocking (replay of S87/S88 rationale at smaller scale)

EFFECT-SIZE-DOMINANCE rule, calibrated to the smaller cell:
1. Effect size is $10.09/1000h, 2× the SHIP threshold ($5).
2. Effect size is ~33× the LLN aggregate noise floor (~$0.30 estimated for a 48K-hand population). Above the 20× criterion that the rule requires.
3. Population is 48,132 hands. Aggregate measurement is statistically tight.
4. Mechanism is REMOVE-AN-OVERRIDE, not ADD-A-NEW-SETTING.
5. Per-hand split (49% same / 44% better / 8% worse) demonstrates decisive swap-right majority (85.2%) — the highest of any chain-audit ship.
6. Pre-committed grader, mechanical verdict.

The criterion "effect ≫ noise floor by 20×+" generalizes across scales; the SHIP threshold itself is a per-cell calibration. Threshold $5 scaled down with cell size, the rule survived.

Option C N=1000 oracle generator infrastructure remains DEFERRED. v60 from S86 still needs N=1000 validation. v60 STAYS UNSHIPPED pending S90+ Option C build.

### What this answers about the cascade — the HIGH_ONLY × {J-A} audit is CLOSED

1. **v47 is net-negative across the ENTIRE HIGH_ONLY × {J-A} zone, at every magnitude.** Three consecutive sessions, three ships from the same diagnosis. Per-session magnitudes: $98.67, $98.84, $10.09. The smaller cells produced smaller ships (bounded by cell size), not different mechanisms. v47's bleed is structural to the rule, not specific to any cell.

2. **Combined S87+S88+S89 chain-audit recovery: $207.60/1000h.** Bigger than every rule-extraction ship from S71-S86 combined (Rule 20 at $16.81 prefix was the only SHIP across that 16-session window). The chain-audit lever has now shipped 12× the cumulative value of incremental rule extraction across three sessions.

3. **HIGH_ONLY × {J-A} zone audit: COMPLETE.** NEITHER × {J-A} is structurally empty (combinatorial proof). All 6 non-empty cells (DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH, JOINT_MED, JOINT_LOW) are now gated. v63 effectively returns strategy_v44_dt on the entire HIGH_ONLY × {J-A} population.

4. **The chain-audit pattern transferred 1:1 for a third consecutive session.** S87 → S88 → S89: identical script structure, rebound parameters. Infrastructure cost is empirically near-zero per cell. The pattern is fully transferable.

5. **Per-hand swap-right rate improved as cell size decreased (62.3% → 76.8% → 85.2%).** Smaller, more uniform cells contain hands where v47's bias is more consistently wrong. The big DS_NO_JOINT cell from S87 had heterogeneous behavior; JOINT_MED and JOINT_LOW are smaller and more uniform — v47 misroutes them more consistently.

### Methodology lessons (Session 89)

1. **The CHAIN AUDIT pattern is transferable infrastructure (S87 → S88 → S89 proved three times).** Three S89 scripts directly templated from S88's three scripts. Rebind target cells + baseline strategy (v62 → v63 instead of v61 → v62) + thresholds. Total infrastructure cost ~10 minutes of editing. The pattern is the project's most-used reusable diagnostic.

2. **The EFFECT-SIZE-DOMINANCE rule generalizes to smaller scales when phrased as "effect ≫ noise floor by 20×+".** S87 ($98.67), S88 ($98.84), S89 ($10.09): three orders of magnitude apart, all three pass the noise-floor criterion. The per-session SHIP threshold ($30, $30, $5) is a per-cell calibration; the noise-floor multiple is what generalizes.

3. **Pre-Phase A planning surfaced a free win: NEITHER × {J-A} is structurally empty.** Five-minute combinatorial check (pigeonhole on 4 suits + 6 non-max cards) closed out a target cell with no compute needed. Worth applying the "is this cell achievable at all?" check before any future chain-audit pivot to a new cell type.

4. **Pre-committed verdict thresholds scale down cleanly with cell size.** S89's SHIP threshold $5 was set in the S88 resume prompt based on "these cells are smaller individually; the aggregate might still be significant." The grader auto-fired SHIP at $10.09, 2× the threshold. Pre-commitment + threshold calibration kept the verdict mechanical. No narrative arbitrage.

5. **Per-hand swap-right rate is a useful secondary verdict signal.** S89's 85.2% (highest of the three) is a non-obvious win — smaller cells, more uniform misrouting, cleaner gate-out behavior. Future audits should track this metric.

6. **The HIGH_ONLY × {J-A} audit closure is itself a methodology milestone.** Three sessions of chain-audit work converged on a defined and bounded zone. The dominant project lever is no longer "find new value-extraction rules"; it is "find and remove chain regressions." But within HIGH_ONLY × {J-A}, that lever is now exhausted — S90+ must find new audit zones (HIGH_ONLY × max ≤ T, or prefix-COVERED cells).

### Files (Session 89)

**New code (committed):**
* `analysis/scripts/drill_v62_high_only_addressability_S89.py` — Phase B pre-drill on JOINT_MED + JOINT_LOW × {J-A} under v62
* `analysis/scripts/audit_v62_chain_bleed_S89.py` — Phase B+ layer-by-layer audit of v44 vs v47 vs v48 vs v52
* `analysis/scripts/strategy_v63_high_only_chain_fix_full.py` — Rule 23 SHIPPED
* `analysis/scripts/grade_v63_full_grid_S89.py` — Phase C grader with pre-committed thresholds

**New artifacts (under `data/session89/`):**
* `drill_v62_high_only_addressability.log`
* `audit_v62_chain_bleed.log`
* `grade_v63_full_grid.log`

**Documentation:**
* `SESSION_89_REPORT.md` — this session's plain-language TL;DR + HIGH_ONLY × {J-A} audit-closure framing
* `DECISIONS_LOG.md` — this section (Decision 124)
* `CURRENT_PHASE.md` — rewritten for S90
* `STRATEGY_GUIDE.md` — Part 1 Session 89 entry added; Part 5 Rule 23 entry added; Part 6 current standard updated; front-matter rewritten

### Path forward (S90 candidates)

1. **PRIMARY: HIGH_ONLY × max ∈ {7-T} chain audit.** Different firing mode (v52-defensive-low rather than v52-fallthrough). UNKNOWN whether the v47 chain bleed extends here. v52-defensive-low was specifically designed for these hands and may NOT carry the same regression. A pre-drill (~5 min) answers definitively. If audit positive, potential $5-50+/1000h ship; if null, clean confirmation that v52-defensive-low is well-designed.

2. **SECONDARY: Audit prefix-COVERED cells with the chain-audit pattern.** LOW pair, two_pair, trips. Existing per-hand parquets cover most. Low compute cost (no new oracle eval), potentially high information value if buried regressions exist outside HIGH_ONLY. Different oracle eval baseline (v63 vs v44_dt on prefix-covered cells, with prefix grader as a real two-grid check).

3. **TERTIARY: Build Option C N=1000 oracle generator infrastructure.** Required for v60 (S86 MID-pair candidate, MIXED-by-methodology) and for any future smaller-effect rule on prefix-silent cells. Engineering scope: ~30-60 min Rust modification + test + launch. Once built, validate v60 retroactively.
...
## Decision 129 — Session 94 STRUCTURAL NULL on within-v44_dt rule-extraction (Option D-revised) applied to two_pair LAYOUT_A_SS + adjacent residual cells; production v65 UNCHANGED; Phase A audit DEFINES the rule-extraction lever's structural saturation boundary (analogous to S91/S92's chain-audit boundary closure); the only candidate showing Rule-20-style intra-layout signal is trips B_DS_AVAIL_LKR intra-Layout-A bot_suit (S95 borderline ship candidate, $5.89/1000h at ≥70% trigger).

### Session

Session 94 — executed the S93-defined PRIMARY path verbatim: rule-extraction on the largest unaddressed within-v44_dt residual cell, two_pair LAYOUT_A_SS at $35.22/1000h on 437,580 hands. The plan was to identify a Rule-20-style structural-feature trigger (analogous to S83's Rule 20 / S86+S93's Rule 25), pre-drill on N=200, run Option C N=1000 sparse grid for the two-grid grader, and ship strategy_v66 if SHIP.

Phase A characterization found v44_dt is structurally saturated on this cell at the (hi_pair, lo_pair) bucket level: 75 of 78 buckets have matching v44/oracle modal layouts. The largest single off-diagonal pivot (v44=Layout B → oracle=Layout C) is $8.98/1000h; the second (v44=C → oracle=B) is $10.47/1000h. But searching 691 (hi_pair, lo_pair, max_sing) sub-buckets within v44_layout=B, **ZERO have P(oracle = Layout C | v44 = Layout B) ≥ 0.70** — Rule 20's anchor threshold. Top sub-bucket reaches only 50.0% on 4 hands; no large sub-bucket exceeds 35%. The signal at this cell is genuinely HAND-SPECIFIC (suit details, exact singleton ranks within bucket), not STRUCTURAL.

### Context

S93 (Decision 128) SHIPPED v65 via Option C infrastructure on the parked v60 candidate, closing the MIXED-by-methodology backlog. S93's resume prompt promoted "rule-extraction on two_pair LAYOUT_A_SS" from S86 SECONDARY to S94 PRIMARY (largest unaddressed within-v44_dt residual cell at $35.22/1000h on 437K hands; S69 confirmed v44_dt dominates the aggregate but individual sub-cells were not exhaustively probed).

S94 probed all 10 within-v44_dt residual cells (6 two_pair + 4 trips) via a cross-cell saturation audit. Result: STRUCTURAL NULL on bucket-level layout-flip rule extraction across all cells. The maximum recoverable lift from a perfect bucket-level layout-flip rule across ALL cells is $5.08/1000h — under the $5 SHIP bar even at 100% trigger accuracy, AND maximum sub-bucket predictivity (the gating signal that determines extractability) is 57.5%, below Rule 25's 62% anchor.

### What S94 ran

**Phase A audit (cross-cell saturation map):** `audit_rule_extraction_structural_S94.py`.

* Read both `drill_two_pair_v44_per_hand_structural.parquet` (S69 output, 1,338,480 hands across 6 cells) and `drill_trips_v44_per_hand_structural.parquet` (S70 output, 328,185 hands across 4 cells).
* Per cell, computed:
  * Whole-grid wg contribution (sum_regret × EV_TO_DOL × 1000 / 6,009,159).
  * Top-level (v44_layout × or_layout) pivot.
  * (hi_pair, lo_pair) bucket-level modal-direction analysis (or {trip, max_kicker} for trips).
  * Sub-bucket trigger search: P(oracle's preferred layout | v44's wrong layout) across (hi, lo, max_sing) sub-buckets.
* Cross-cell summary:
...
**Phase A.2 — Intra-Layout-A bot_suit follow-up (trips B_DS_AVAIL_LKR):** Bucket-level layout flips don't capture Rule-20-style intra-layout structural improvements. The follow-up drill restricted to Layout-A-agree (v44 and oracle both pick Layout A, 105,031 hands) and pivoted on bot_suit:
...
| 4 (4f) | 0 (DS) | 835 | +0.73 |
| 3 (RB) | 0 (DS) | 186 | +0.30 |
| **Aggregate (v44 non-DS → oracle DS)** | | **25,905** | **+$19.49** |

Sub-bucket trigger search across (kickers_max_suit_count, n_kickers_in_trip_suits, n_b_ds_routings):

| kickers_max_suit_count | n_kickers_in_trip_suits | n_b_ds_routings | n | P(oracle DS) | wg ($/1000h) |
|---:|---:|---:|---:|---:|---:|
| 2 | 4 | 1 | 210 | **100.0%** | $0.17 |
| 3 | 4 | 1 | 370 | **98.4%** | $0.36 |
| 2 | 4 | 3 | 4,911 | 48.4% | $2.42 |
| 2 | 3 | 1 | 17,838 | 28.9% | $4.70 |
| 2 | 2 | 1 | 5,776 | 10.4% | $0.53 |
| 1 | 3 | 3 | 6,615 | 0.0% | $0.65 |

Two narrow sub-buckets reach ≥70% predictivity (Rule 20 anchor) totaling 8,160 hands at $5.89/1000h aggregate — **borderline at $5 SHIP bar even at 100% trigger accuracy**. Realistic expected lift is $2-4/1000h after accounting for false-positive cost on the 0-2% wrong predictions, plus deterministic-picker-vs-oracle gap. **Borderline S95 ship candidate; most likely lands MIXED on the two-grid grader.**

### Verdict + production state

**VERDICT: STRUCTURAL NULL.** Production v65_mid_pair_chain_extend UNCHANGED.

| metric | pre-S94 (v65) | post-S94 (v65) | Δ |
|---|---:|---:|---:|
...
* v44_dt ML champion: UNCHANGED ($1,081 full / $686 prefix) — **22 consecutive sessions** running, since v44 in S58.
* Combined S87-S93 production-chain recovery: $221.26/1000h (S94 contributes $0).

### Methodology lessons (Session 94)

1. **OPTION D-REVISED HAS A STRUCTURAL SATURATION BOUNDARY (NEW S94).** Just as the chain-audit lever has a saturation boundary (S91 POPULATION-DIVERGENCE / S92 STRUCTURAL NULLs), the rule-extraction lever applied to within-v44_dt residuals has a STRUCTURAL TRIGGER-PREDICTIVITY BOUNDARY. The boundary is **trigger predictivity ≥ 62-70%** at the (cell × structural feature) level. At the (hi_pair, lo_pair, max_sing) feature level on two_pair LAYOUT_A_SS and at the equivalent feature level on the 9 other within-v44_dt residual cells, no sub-bucket reaches Rule 25's 62% threshold. The chain-audit + rule-extraction levers are BOTH characterized as saturated on the current ML architecture.
...
3. **INTRA-LAYOUT IMPROVEMENTS ARE A DISTINCT LEVER SUB-CLASS WITHIN OPTION D-REVISED (NEW S94).** Bucket-level layout-flip rules (force Layout B instead of Layout C) only capture $5.08/1000h across all 10 within-v44_dt residual cells — under the SHIP bar. The Rule-20-style mechanism is INTRA-LAYOUT (within the modal layout choice, flip a sub-structural feature like bot_suit). The trips B_DS_AVAIL_LKR intra-Layout-A bot_suit candidate ($19.49 potential aggregate, $5.89 at trigger ≥70%) is the first identified candidate of this sub-class on a within-v44_dt residual cell. Whether it ships is an S95 empirical question.

4. **HAND-SPECIFIC RESIDUAL DOMINATES STRUCTURAL RESIDUAL ON THESE CELLS (NEW S94).** Two_pair LAYOUT_A_SS's $35.22/1000h leak distributes across 702 (hi_pair, lo_pair, max_sing) sub-buckets with no concentration: top-30 sub-buckets sum to $3.82; top-100 sum to $11.18. The residual leak is not waiting for a structural rule to capture it — it's hand-specific variance that requires either oracle-quality picks (i.e., a richer ML champion) or a different lever entirely. **This explains why v44_dt is well-calibrated at the structural feature level despite the cell's large aggregate residual.**

### Files (Session 94)

**New audit script (committed):**
* `analysis/scripts/audit_rule_extraction_structural_S94.py` — cross-cell saturation audit + intra-Layout-A bot_suit follow-up.

**New data artifacts:**
* `data/session94/audit_summary.log` — full audit script output.
* `data/session94/audit_rule_extraction_structural_summary.json` — per-cell saturation evidence + sub-bucket trigger search results + intra-Layout-A bot_suit pivot table.

**Documentation:**
* `SESSION_94_REPORT.md` — plain-language session report + complete saturation evidence.
* `DECISIONS_LOG.md` — this section (Decision 129).
...
### Path forward (S95 candidates)

1. **PRIMARY (NEW S94 finding): trips B_DS_AVAIL_LKR intra-Layout-A bot_suit candidate.** Borderline at $5.89/1000h on 8,160 hands at ≥70% trigger predictivity ($19.49/1000h aggregate at 100% accuracy). Engineering scope: design "force-best-DS-bot-within-Layout-A" picking logic (analog of Rule 20's "best DS configuration"), pre-drill on N=200 multi-gate (narrow vs wide trigger), Option C sparse N=1000 if N=200 ≥ $5, two-grid grader at pre-committed thresholds. **Estimated ship probability: 20-40% — most likely MIXED.**

2. **SECONDARY: headline-goal recalibration.** Carried from S92/S93/S94. With both chain-audit and rule-extraction levers characterized as saturated, this is the moment to make explicit that 95% match% is unreachable on current ML architecture and reset the success criterion to maximize $/1000h. Affects future MIXED-session interpretation and resource allocation.

3. **TERTIARY: ML retrain (A3 full 6M-hand N=1000 grid).** Formally closed at v44 in S78 (Decision 113). Option C infrastructure now provides the foundation; ~70 hours wall on current hardware. Reopening requires either a new feature family or explicit operator authorization. The structural saturation findings from S91-S94 raise the question of whether a richer ML champion would shift the saturation boundary — but the path requires substantial compute investment.

4. **DEFERRED: v52-defensive-low partial-effectiveness exploit (S90).** Speculative.
...
## Decision 130 — Session 95 MIXED on trips B_DS_AVAIL_LKR intra-Layout-A force-best-DS-bot candidate (Rule 26); production v65 UNCHANGED; picker-criterion sweep surfaces NEW S95 methodology lesson — picker alignment with oracle's specific pick is the SECOND necessary condition for rule extraction (alongside S94's trigger-predictivity boundary). Best gate: NARROW at N=200 +$4.59 / N=1000 +$4.75, |Δ|=$0.16 — borderline below $5 SHIP bar by ~$0.25.

### Session

Session 95 — executed the S94-defined PRIMARY path: design strategy_v66 with a Rule-20-analog "force-best-DS-bot-within-Layout-A" picker, multi-gate pre-drill on N=200, Option C N=1000 sparse-grid grading. Phase A re-confirmed the S94 sub-bucket numbers from per-hand parquet (resume prompt + SESSION_94_REPORT.md had transcription typos: NARROW was n=8,160 at 90.4% meanP / $5.89 wg, not n=580 at near-100% / $0.53 wg). Phase B built strategy_v66 with the naive `bot_pair_high` carry-over picker — all three gates strongly NEGATIVE at N=200 ($−3.22 / $−6.25 / $−8.84). A picker diagnostic showed the rule's exact-pick match with oracle was only 7.5-13.8% despite the high direction-predictivity. An 8-criterion picker sweep identified **TOP_HIGH_then_pair_tops** as the correct criterion, lifting NARROW to $+4.59 N=200 / 85.2% swap-right rate. Phase C two-grid grader: MIXED on all 3 gates (NARROW closest at N=200 +$4.59 / N=1000 +$4.75).

### Context

S94 (Decision 129) closed the bucket-level rule-extraction lever as STRUCTURALLY SATURATED (max sub-bucket trigger predictivity 57.5% across all 10 within-v44_dt residual cells). The Phase A.2 follow-up identified trips B_DS_AVAIL_LKR intra-Layout-A bot_suit as the only OPEN candidate showing Rule-20-style intra-layout signal: $19.49/1000h aggregate when v44 picks Layout A with non-DS bot but oracle picks DS, with sub-buckets at 88-100% trigger predictivity on 8,160 hands. S94 estimated 20-40% ship probability.

S95 confirms the candidate cannot ship cleanly at the current architecture, but for a NEW reason: even high trigger predictivity is necessary-but-not-sufficient for clean ship. The rule's deterministic picker must also align with oracle's specific pick within the predicted direction.

### What S95 ran

**Phase A — re-confirm sub-bucket numbers + lock trigger gates.** `phaseA_trips_b_ds_avail_lkr_intra_layout_a_S95.py`.

Cell B_DS_AVAIL_LKR (cell_idx=1): 163,170 hands, $42.61/1000h. v44 picks Layout A non-DS: 65,811 hands. Sub-bucket pivot on (kickers_max_suit_count, n_kickers_in_trip_suits, n_b_ds_routings) for v44 non-DS Layout-A population:
...
(Note: resume prompt's n=210/n=370/n=4,911/n=17,838 lines were transcription typos from an earlier draft. The S94 JSON `audit_rule_extraction_structural_summary.json` shows the correct numbers used above.)

**Phase B — strategy_v66 design + N=200 pre-drill.** `strategy_v66_trips_layout_a_force_ds_bot.py` + `prepare_v66_id_list_S95.py`.

Rule 26 candidate: fires when (trips × cell B_DS_AVAIL_LKR × sub-bucket ∈ gate × v44_dt picks Layout A non-DS bot). Forced pick: enumerate (mid_pair_of_trips × top_kicker) settings yielding DS bot, pick the best by deterministic criterion. Initial criterion: **bot_pair_high desc, bot_pair_2nd desc, top_rank desc** (analog of Rule 20's "best DS configuration").

**N=200 pre-drill — initial picker — ALL THREE GATES STRONGLY NEGATIVE:**

| Gate | n_changed | swap-right | N=200 lift |
|---|---:|---:|---:|
| NARROW | 8,751 | 36.6% | **$−3.22** |
| MEDIUM | 18,734 | 39.3% | **$−6.25** |
| WIDE | 51,531 | 47.6% | **$−8.84** |

**Diagnostic** (`diagnose_v66_picker_S95.py`): among NARROW-gate fired+oracle-picks-DS hands (n=7,765 = 88.7% of fired), the rule's specific setting matches oracle exactly only **7.5%** of the time. In MEDIUM: 13.8%. In WIDE: 51.6%. Per-sub-bucket: (2,3,1) at 89.1% (single mostly-forced config), but (2,4,1) at 3.5% and (3,4,1) at 8.7%. Mean EV(v66 − oracle) on NARROW fired+oracle-DS: **−656 ev units** — the rule's "best DS" pick is 656 ev-units worse on average than oracle's DS pick. The picker criterion was wrong.

**Picker-criterion sweep** (`picker_sweep_v66_S95.py`): 8 deterministic Layout-A DS-bot picker criteria across 3 gates. Winner: **TOP_HIGH_then_pair_tops** (highest kicker on top, then bot pair-tops desc).

| Criterion | NARROW | MEDIUM | WIDE |
|---|---:|---:|---:|
| BOT_PAIR_HI then 2 then TOP_DESC (initial) | $−3.22 (36.6%) | $−6.25 (39.3%) | $−8.84 (47.6%) |
| BOT_PAIR_HI then 2 then TOP_ASC | $−4.56 (31.9%) | $−6.71 (39.4%) | $−9.30 (47.6%) |
...
Per-sub-bucket and cumulative-slice analysis under TOP_HIGH (`picker_sweep_subbucket_v66_S95.py`) confirms **no slice clears the $5 SHIP bar at N=200**. Best individual slice: (3,4,1) at +$3.50 / 82.0%. Best cumulative slice: NARROW at +$4.59 / 85.2% (NARROW = (2,4,1)+(3,4,1)).

**Phase C — Option C N=1000 sparse-grid grading.** `prepare_v66_id_list_S95.py` (re-run with corrected picker) emitted id list of 51,531 ids (WIDE superset). Engine ran in 1,901 s wall at 27.1 hands/s. Grader (`grade_v66_id_list_n1000_S95.py`) applied pre-committed two-grid thresholds:
...
The two-grid noise is small ($0.16 on NARROW, $0.42 on MEDIUM, $1.73 on WIDE — the N=1000 estimate is higher on every gate but still below the $5 bar). NARROW at N=1000 $+4.75 is $0.25 short of the SHIP bar; an "honest miss" rather than noise-driven MIXED.
...
* v44_dt ML champion: UNCHANGED ($1,081 full / $686 prefix) — **23 consecutive sessions** running, since v44 in S58.
* Combined S87-S93 production-chain recovery: $221.26/1000h (S94 + S95 contribute $0).

### Methodology lessons (Session 95)

1. **PICKER ALIGNMENT IS THE SECOND NECESSARY CONDITION FOR RULE EXTRACTION (NEW S95).** S94 established trigger predictivity ≥ 62-70% as the operational definition of "rule extractable" — necessary for clean ship. S95 adds the second necessary condition: the rule's deterministic picker must align with oracle's specific pick within the predicted direction. **High direction predictivity (90%) is useless if the rule picks a different config 90% of the time.** Rules 20 and 25 had ≥89% picker alignment by happy accident — their `bot_pair_high` criterion = oracle's. The trips analog under that same criterion got 3-8% alignment in the highest-P sub-buckets. Both conditions (trigger predictivity ≥ 62%, picker alignment ≥ ~70%) must be verified empirically for each new cell.

2. **PICKER CRITERIA ARE CELL-SPECIFIC, NOT TRANSFERABLE (NEW S95).** Rule 20 / 25 use `bot_pair_high` because the bot's pair value dominates top's single-card value for hands with a pair. Trips invert this — bot's strength is mostly the trip card itself, top wants the highest kicker. **Never assume a successful rule's picker criterion will carry over to a new cell;** validate empirically. The 8-criterion picker sweep takes ~3 min on this cell with the `picker_sweep_v66_S95.py` template; a 0-min cost diagnostic that swung the lift by $7.81/1000h on NARROW ($−3.22 → $+4.59).

3. **RESUME-PROMPT TRANSCRIPTION TYPO (operational lesson).** The S94 resume prompt and SESSION_94_REPORT.md listed the NARROW sub-buckets as n=210 + n=370 totaling 580 hands at near-100% predictivity / $0.53 wg (a transcription typo from an earlier draft); the S94 audit JSON shows the true numbers n=1,618 + n=6,542 totaling 8,160 at 90.4% mean predictivity / $5.89 wg. Always cross-check resume-prompt numbers against the JSON of record at session start; if numbers look anomalously small or large, run the JSON.

4. **OPTION D-REVISED RULE-EXTRACTION LEVER IS NOW EXHAUSTED ACROSS BOTH SUB-CLASSES (carried + extended).** S94 closed bucket-level layout-flip rules ($5.08/1000h ceiling across all 10 within-v44_dt cells). S95 closes intra-layout structural-feature rules on the largest remaining candidate (trips B_DS_AVAIL_LKR — the only cell with Rule-20-style intra-layout signal). With both sub-classes exhausted on the current ML architecture, the headline-goal recalibration option promotes from S94/S95 SECONDARY → S96 PRIMARY: this is the moment to make explicit that 95% match% is unreachable on current architecture and reset the success criterion to maximize $/1000h subject to the current cascade.

5. **MIXED VERDICTS ARE INFORMATIVE EVEN WHEN THEY DON'T SHIP (carried).** This session produced no production change but characterized a genuinely-borderline candidate at $4.59/$4.75 — within $0.25 of the SHIP bar on both grids. The candidate is honestly close to the bar but cleanly fails it; no methodological adjustment recovers it without lowering the two-grid SHIP standard.

### Files (Session 95)

**New strategy file:**
* `analysis/scripts/strategy_v66_trips_layout_a_force_ds_bot.py` — Rule 26 candidate with the corrected TOP_HIGH-then-pair_tops picker.

**New analysis scripts:**
* `analysis/scripts/phaseA_trips_b_ds_avail_lkr_intra_layout_a_S95.py`
* `analysis/scripts/sanity_v66_on_parquet_S95.py`
* `analysis/scripts/prepare_v66_id_list_S95.py`
* `analysis/scripts/diagnose_v66_picker_S95.py`
* `analysis/scripts/picker_sweep_v66_S95.py`
* `analysis/scripts/picker_sweep_subbucket_v66_S95.py`
* `analysis/scripts/grade_v66_id_list_n1000_S95.py`

**New data artifacts (`data/session95/`):**
* `phaseA_summary.{json,log}`, `phaseA_cids_gate_{NARROW,MEDIUM,WIDE}.txt`
* `phaseB_prepare.{log,summary.json}`, `diagnose_v66_picker.log`
* `picker_sweep.log`, `picker_sweep_subbucket.log`
* `v66_per_hand_picks.npz`, `v66_id_list_wide.txt`
* `v66_n1000_sparse.bin` (21.8 MB Option C grid), `engine_n1000_sparse.log`
* `grade_v66_n1000_summary.json`, `grade_v66_n1000.log`

**Documentation:**
* `SESSION_95_REPORT.md` — this session's plain-language report.
* `DECISIONS_LOG.md` — this section (Decision 130).
...
### Path forward (S96 candidates)

1. **PRIMARY (PROMOTED from S95 SECONDARY) — headline-goal recalibration.** With chain-audit + both rule-extraction sub-classes characterized as saturated on the current ML architecture, this is the moment to make explicit that the 95% match% goal is unreachable from the current architecture. Reset the success criterion to maximize $/1000h. Doc-only session; sharpens S97+ planning.
