...
>
> **Last updated:** 2026-05-16 (Session 98, V4). Reflects production strategy `v65`. Supersedes V1/V2/V3.
...
| Shape | Quick play |
|---|---|
| **Quads + pair** (4+2+1) | Top=singleton, Mid=2 non-pair-suit quads, Bot=other 2 quads + pair (always double-suited). ⚠️ Low quads (≤5)? See Ch. 5 BUYOUT |
| **Plain quads** (4+1+1+1) | Top=high singleton, Mid=2 non-singleton-suit quads, Bot=other 2 quads + low singletons. ⚠️ Low quads (≤5)? See Ch. 5 BUYOUT |
| **Two trips / Trips+two-pair** | Rare composite shapes — see Rule 9 below |
| **Trips + pair** (3+2+1+1) | Split trips, keep pair: Mid=2 trip cards, Bot=pair + 1 trip + 1 kicker. ⚠️ Low trips (≤4) + low pair (≤3)? See Ch. 5 BUYOUT |
...
| **One pair (low/mid, no Ace, max ≤ Q, 2+2 DS bot reachable)** | 2e **PMID-swap**: max non-pair card goes to BOT (not top); next-highest to top; pair stays in mid |
| **One pair (other), normal** | Top=Ace (or highest), Mid=pair, Bot=rest. Pair-to-bot only if narrow gates met |
| **No pair, A-high (any body)** | Rule 1a: A on top, build best DS bot, mid keeps 2 highest leftovers |
| **No pair, K/Q/J-high, healthy body (2nd-high ≥ 9)** | Rule 1b/c/d: max on top, build best DS bot, HIMID |
| **No pair, T-or-lower max, OR K/Q/J-high with 2nd-high ≤ 8 (vulnerable broadway)** | DEFENSIVE (Ch. 4) |
...
> ⚠️ **The defensive trigger.** Before applying Rule 1a-1d below, check this gate:
>
> If your **max card is T or lower**, OR if your **max is K/Q/J but your 2nd-highest is 8 or lower** (vulnerable broadway), **go to Chapter 4 (Defensive Play)**.
>
> Only continue to 1a-1d below if your hand passes this gate — i.e., A-high (any body), or K/Q/J-high with 2nd-highest ≥ 9.

### 1a — A-high (you hold an Ace)

A-high always plays offensively — an Ace on top is too strong to give up regardless of body.
...
### 1b — K-high, healthy body (2nd-highest ≥ 9)

Same structure as 1a, with K on top.

(If 2nd-highest is 8 or lower, the defensive gate fires — go to Chapter 4 instead.)

### 1c — Q-high, healthy body (2nd-highest ≥ 9)
...
### 1d — J-high, healthy body (2nd-highest ≥ 9)

Same structure: J on top, build DS bot, HIMID. The lifts continue to shrink at J, but with a 9+ second card, you still have a real shot at the top.

(If 2nd-highest is 8 or lower, the defensive gate fires — go to Chapter 4 instead.)

### 1e — T-high or lower max, OR vulnerable broadway → DEFENSIVE

This is the catch-all for any no-pair hand whose top isn't strong enough to play offensively. Triggers when:
- Max card is T, 9, 8, 7, or lower — OR —
- Max card is J, Q, or K but the 2nd-highest is 8 or lower.

**→ GO TO CHAPTER 4 (DEFENSIVE).**
...
> ⚠️ **Is this a low-trips + low-pair hand?** (trips of rank ≤ 4 AND pair of rank ≤ 3, e.g. `3♣ 3♦ 3♥ 2♣ 2♦ K♠ 8♥`) Check **Chapter 5 on the Buyout option** before setting — this is a structurally trapped shape and the math says buy out.
...
> ⚠️ **Are these low quads (rank ≤ 5)?** (e.g. `2♣ 2♦ 2♥ 2♠ K♣ K♦ 8♠` or `3♣ 3♦ 3♥ 3♠ ...`) Check **Chapter 5 on the Buyout option** before setting — low quads are structurally trapped under the Omaha 2+3 rule and the math says buy out even though the hand "looks" strong.
...
> ⚠️ **Are these low quads (rank ≤ 5)?** (e.g. `3♣ 3♦ 3♥ 3♠ K♠ Q♣ J♦`) Check **Chapter 5 on the Buyout option** before setting — same Omaha-rigidity trap as Rule 7.
...
The project never directly graded "a human applying this guide cleanly" against the same opponent model. Doing so would require simulating perfect-rule-application across the full 6M-hand grid, which is feasible but hasn't been done. **A V5 of this guide could include that grade if you want the exact number.**
...
3. Quad + pair?     → Rule 7.     ⚠️ Low quads? Ch.5 Buyout
4. Plain quads?     → Rule 8.     ⚠️ Low quads? Ch.5 Buyout
5. Two trips OR     → Rule 9.
   Trips + 2 pair?
6. Trip + pair?     → Rule 6.     ⚠️ Low trips+low pair? Ch.5 Buyout
7. Pure trips?      → Rule 5.     ⚠️ Pure trips ≤5? Ch.5 Buyout (maybe)
8. Three pair?      → Rule 4.
...
10. One pair?
    ...
    - Other pair, low/mid with max ≤ Q, 2+2 DS bot reachable? → 2e PMID-swap
    - Other pair, normal? → 2c (gates) or 2f (default)
11. No pair?
    - Max ≤ T, OR K/Q/J with 2nd-high ≤ 8? → CHAPTER 4 DEFENSIVE
    - A-high (any body)?  → Rule 1a
    - K/Q/J-high with 2nd-high ≥ 9? → Rule 1b / 1c / 1d
...
*This is V4, replacing V1/V2/V3. Final fixes from two independent Gemini reviews of V3 incorporated: (1) cheat sheet now includes PMID-swap row, (2) Rule 1 sub-rules restructured to match Chapter 4's actual defensive trigger exactly (T-or-lower OR vulnerable broadway), (3) buyout off-ramp callouts added at end of Rules 6, 7, 8 to catch study-readers. The canonical engineering reference remains `STRATEGY_GUIDE.md`. Update when production strategy changes.*
