...
- **Rule 15 — K-high no-pair, K-on-top + DS/SS HIMID (Session 51, CURRENT PRODUCTION)** → `analysis/scripts/strategy_v46_rule15_Khigh_DS.py` — same structure as Rule 14 but for K-high. Trigger: cat=high_only AND max=K AND DS-bot OR SS-bot achievable with K on top. Setting builder: TOP=K always, try DS-bot first then SS-bot fallback, HIMID tie-break. Fires on 95.8% of K-high no-pair (= 5.3% of grid). **+$51/1000h whole-grid (full) + $0 prefix (UNCHANGED — high_only zero prefix coverage).** pct_opt full: +0.19%; high_only sub-category +0.9%. high_only $3,439 → $3,187 (−$252, −7.3%). p90 regret IMPROVED 0.745→0.730. **3rd-largest single-rule lift in project history.** Cumulative v45 → v46 = −$51 full / $0 prefix; v39 → v46 = −$311 full / −$185 prefix. Known coverage gap: oracle picks K-on-top only 66% of K-high (vs A-on-top 93%); 34% prefer Q/J/lower on top — Rule 15 v2 candidate.
...
- **Combined chain (15 rules, CURRENT PRODUCTION)** → `analysis/scripts/strategy_v46_rule15_Khigh_DS.py` (wraps v45 with Rule 15 — K-high no-pair + DS/SS HIMID; 3rd-largest single-rule lift in project history)
