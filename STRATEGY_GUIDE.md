...
| **v43_dt** | **S57** | **36** | **1** | **103 (99 + 4 high_only_aug_v3 rank-valued JOINT)** | **2,177,798** | **$1,123** | **$686** | **CURRENT ML CHAMPION (end of S57).** +3.2% leaves over v42 (modest, surgical to high_only). All 4 new features at LOW individual importance: #63 min_mid_high (0.07%), #64 max_mid_sum (0.07%), #100 max_mid_high (0.01%), #102 n_configs (0.00%) — lowest-importance-per-ship in project. Yet ships +$69 full grid via surgical gating: only 14.7% of high_only hands have any joint config, but those concentrate in the dominant SS→DS mismatch class. pct_opt full 63.08% → 63.99% (+0.91%); prefix unchanged at 67.13%. **high_only zone within-category $2,411 → $2,075 (−$336, −13.9%); high_only pct_opt 33.4% → 37.9% (+4.5%)** via JOINT (DS bot + ms mid) achievability features conditional on top=max-rank-of-hand. C(6,4)=15 candidate bots from non-top cards, filtered to (a) bot 2+2, (b) leftover 2 cards same-suit. All other 7 categories byte-identical to v42 on both grids. Lift: −$69 full / $0 prefix (prefix neutrality is by design). Cumulative v32 → v43 = −$592 full / −$218 prefix (8 ML ships). p90 0.400; p99 0.980. **Methodology validation: the 4-phase playbook is transferable to the SAME zone for a SECOND pass.** Two-session high_only collapse (S55 → S57): $2,796 → $2,411 → $2,075 = −$721 within-category (−25.8%). Joint (DS bot AND ms mid) achievability is a distinct structural axis from single-axis (DS bot only) achievability — joint features are NOT redundant with components. +4.7% leaves over v41 (modest, surgical to high_only). 3 of 4 new features in top-32 importance (#26 max_mid_sum 0.28%, #31 max_top 0.22%, #32 min_top 0.21%, #80 n_configs 0.03%) — deeper integration than S55's t2p_v2 family. pct_opt full 62.18% → 63.08% (+0.91%); prefix unchanged at 67.13%. **high_only zone within-category $2,796 → $2,411 (−$385, −13.8%); high_only pct_opt 29.0% → 33.4% (+4.4%)** via DS-bot achievability features mirroring pair_aug_v5 / trips_pair_v2 / two_pair_v2 (enumerates C(7,4)=35 candidate bot subsets, filters to 2+2 DS, characterizes max_top, min_top, max_mid_sum). All other 7 categories byte-identical to v41 on both grids. Lift: −$79 full / $0 prefix (prefix neutrality is by design — prefix slice has zero high_only hands so gating mathematically guarantees identical metrics). Cumulative v32 → v42 = −$524 full / −$218 prefix (7 ML ships). p90 0.425; p99 1.035. Methodology validation: the 4-phase playbook is fully transferable to the largest population zone (40.4%) without modification. Phase 1b 100% confirmation (100% of dominant-class mismatches have max_top=A AND 100% of oracle picks use it) — the cleanest Phase 1b in the project. User-prediction "different feature types needed" was partially wrong: the dominant high_only blind spot was the SAME DS-routing pattern as prior zones; only the bot suit profile differs in 100% of mismatches. |
