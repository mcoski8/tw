...
>
> **Production rule chain (end of Session 90): v64_high_only_chain_fix_zone** —
> stacks atop v57 via Rule 24, a strict superset of Rule 23's gate-out
> that EXTENDS the chain bypass to the entire structurally-non-empty
> HIGH_ONLY × max ≥ 8 zone:
> for HIGH_ONLY (no-pair) hands with max ∈ {8,9,T,J,Q,K,A} in any of
> the 6 non-empty cells (DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH,
> JOINT_MED, JOINT_LOW) per the S71 taxonomy, bypass the v47→v48→v52
> chain and return strategy_v44_dt directly. Else route to
> v57_lo_pair_defensive (which adds Rule 20 atop v56 etc.). HIGH_ONLY ×
> max=7 is structurally empty (only 6 ranks ≤ 7 exist). Other
> structurally-empty combinations at max ≤ T (JOINT_HIGH ∀, NEITHER ∀,
> JOINT_MED × 8) are filtered by the cell classifier. Score:
> **$1,627.36/1000h full grid / $776.88 prefix** (prefix unchanged —
> the rule fires entirely outside prefix coverage) — beats v14 by
> **+$2,466/1000h** on the full grid. Lives at
> `analysis/scripts/strategy_v64_high_only_chain_fix_zone.py`. **Total
> project rule count: 24** (Rule 24 added in S90 — chain gate-out
> extension to HIGH_ONLY × max ∈ {8, 9, T}, recovering another
> $7.23/1000h after S87's $98.67, S88's $98.84, and S89's $10.09).
> S90 is the fifth strategy-of-record change in 8 sessions (S83 added
> Rule 20, S87 added Rule 21, S88 added Rule 22, S89 added Rule 23,
> S90 added Rule 24).
>
...
>
> The production rule chain at v64 now outperforms v44_dt on the full
> grid by **+$546/1000h** (v64 $1,627.36 vs v44_dt $1,081). The remaining
> gap to the oracle ceiling (the "two-track divergence" in the project's
> historical framing) is **$117.84/1000h**, down from $1,409 pre-S68 —
> **cumulative closure: 91.6%.** Closures by session:
...
> extension to max ≤ T). The S68-S70 arc is architectural routing
> (route categories through v44); S83 is rule extraction;
> S87-S88-S89-S90 are CHAIN AUDIT recoveries — finding and removing
> buried v47/v52 regressions on prefix-silent HIGH_ONLY cells. The
> chain-audit lever has now shipped **$214.83/1000h across four
> sessions**, dominating value-extraction ($16.81 from Rule 20 across
> the prior 16-session window). **Structurally-non-empty HIGH_ONLY ×
> max ≥ 8 chain-audit zone is now COMPLETE** — all 17 non-empty
> (cell × rank) combinations (6 in {J-A}, 11 in {8, 9, T}) are gated;
> HIGH_ONLY × max = 7 is structurally empty by combinatorial
> necessity. S90 also produced the **first project-level audit of
> v52-defensive-low**: it is PARTIALLY effective — it recovers
> $9.52 of v47's $19.28 bleed on the S90 target population but does
> not fully restore v44_dt's performance. v52-DL actively wins on
> ~23% of those hands; future v65 refinement opportunity (per-hand
> picker) is deferred.
