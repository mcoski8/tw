...
>
> Last updated: 2026-05-16 (Session 98 — **PROJECT CLOSURE. STRATEGY OF RECORD UNCHANGED: v65 remains production. v44_dt UNCHANGED for 26 consecutive sessions.** S98 produced the human-facing shipping artifact suite: V4 + V5 Lean play guides (V5 collapses 25 sub-rules to 10 named rules); true hand-by-hand Monte Carlo simulator (5 scripts using project's lookup table); 8 opponent archetype implementations (operator's described Pair-First Standardist + Top-Greedy Defender, Rust-engine ports, 3 NEW archetypes, composite Grid Oracle); ~400K hands of MC across 8 scenarios; bankroll + hourly economics doc; 3-way Socratic closure discussion with unanimous SHIP verdict from Gemini 3.0 + Gemini 2.5 across two rounds. **Key empirical finding for closure:** v65 vs the composite Grid Oracle = +$0.41/hand (4/10 sims negative) = statistically tied at the heuristic ceiling. Any further EV gain lives in 2.25M-leaf ML decision-tree territory inaccessible to human-codifiable rules. Decision 133 records the formal closure. No engine runs. No grader runs. No production change. Next direction (operator-stated): camera-app project for 7-card recognition + optimal-setting output; kickoff doc at CAMERA_APP_PROJECT_KICKOFF.md. Previous front-matter (Session 97 — **STRATEGY OF RECORD UNCHANGED: v65 remains production. MAINTENANCE LEVER STRUCTURAL NULL (Decision 132).** v60-gate11 — the S96 MAINTENANCE candidate carried at "MIXED +$4.85/+$4.77 from S93 SECONDARY" — is empirically absorbed by v65 on 14,160 / 14,160 firing hands. Rule 25 (= v60-gate12) already covers gate-11's firing zone (`max_sing ≤ J` ⊂ `max_sing ≤ Q`) with byte-identical picker logic; the +$4.85 reading was measured against the v57 baseline (pre-Rule-25) and is stale post-v65. The composite hypothesis (v60-gate11 + v66-NARROW for ~$9 joint ship) collapses — v60-gate11 contributes $0 over v65; the composite reduces to v66-NARROW alone, which is locked MIXED at $+4.59 N=200 / $+4.75 N=1000 per S95 (locked by the two-grid bar, $0.25 short on each grid). Phase B identified the structural concern *before any code ran* (gate-11 ⊂ gate-12 with same picker); Phase C confirmed empirically via `spot_check_v60g11_subset_v65_S97.py` — 14,160 / 14,160 = 100.0000% v65_pick == v60_gate11_pick on the firing zone, 0 fall-through to v57, 3.4s wall on a single core. Phase E records Decision 132 closing the maintenance lever as STRUCTURAL NULL. **No code change to any strategy script or engine. No grader run. v65 + v44_dt UNCHANGED (v44_dt 25th consecutive session since S58).** New artifacts: `analysis/scripts/spot_check_v60g11_subset_v65_S97.py`, `data/session97/spot_check_v60g11_subset_v65.json`. Production v65 UNCHANGED at $1,633.79 full / $776.88 prefix. v66-NARROW characterization UNCHANGED at $+4.59/$+4.75. Rule count UNCHANGED at 25. Cumulative closure UNCHANGED at 92.09%. NEW S97 methodology lessons: (1) **PARKED-CANDIDATE BOOK-KEEPING MUST RECORD BASELINE + RE-GRADE BEFORE COMPOSITE HYPOTHESES.** A "+$X lift" is baseline-relative; once a related candidate from the same cell ships, the +$X becomes double-counted relative to current production. v60-gate11's S93 +$4.85 was true vs v57 at the time but became stale the moment Rule 25 (= v60-gate12) shipped, since gate=11's firing zone is a strict subset of gate=12's. Future parked-candidate annotations must record the baseline strategy used and re-grade against current production before any composite hypothesis. (2) **STRUCTURAL SUBSET CHECKS ARE CHEAP AND DECISIVE.** Cells like `max_sing ≤ J` ⊂ `max_sing ≤ Q` are immediately testable on the per-hand picks NPZ that already exists; no engine run is needed to decide whether a candidate is structurally absorbed by current production. ~5 min of code + 3.4s of compute resolved the question; if the structural relationship had been checked at S93 SECONDARY framing time, the candidate wouldn't have been carried as "open" for 4 sessions. Decision 131's lesson was about zombie *headline* framing; this is about zombie *candidate* framing. With chain-audit (Decision 127), rule-extraction bucket-level (Decision 129), rule-extraction intra-layout (Decision 130), headline-goal recalibration (Decision 131), and now maintenance composite (Decision 132) all closed, the remaining open levers are A3 ML retrain (operator-authorization-gated; ~70h wall; honest NULL prior per Decisions 113+117; only remaining $50+/1000h lever), v52-DL exploit (speculative), and v44_RULE13 replacement (modest at best). S98 PRIMARY: A3 ML retrain. S98 SECONDARY: v52-DL exploit. S98 TERTIARY: v44_RULE13 fallthrough replacement. Previous front-matter (Session 96 — **STRATEGY OF RECORD UNCHANGED: v65 remains production. DOCUMENTATION-ONLY SESSION: headline-goal recalibration; 95% match% formally retired second-stage; $/1000h on the production grid is the project's headline metric (Decision 131).** S96 closed a multi-session loop dating to Decision 033 (S16): Decision 033 retired the *shape-agreement* form of the 95% target in favor of "directional EV-loss reduction + non-negative absolute EV per profile" but its Consequence #1's "CLAUDE.md headline section may need updating, deferred to S17" was never executed; the *match%* form of the 95% target re-emerged after the S27+ ML pivot and surfaced in S79/S81/S82/S85/S95 framing despite four major lever cascades (Decisions 113 ML, 117 A-path, 122-127 chain-audit complete, 129+130 rule-extraction both sub-classes) being characterized as saturated at the v44 architecture level. Phase A audit found three load-bearing zombie-95% references: CLAUDE.md:12 ("Primary Goal: ... matches the solver 95%+ of the time"), CLAUDE.md:20 ("Critical Output #4: ... matches the solver for 95-99% of hands"), checklist.md:159 ("Push toward 95%+ agreement"). Phase B drafted the replacement headline: PRIMARY = production strategy $/1000h on full grid (currently $1,633.79); SECONDARY (diagnostic) = remaining gap to oracle ceiling ($111.41), two-track divergence ($552.79), rule count (25); operational ship standard unchanged at the pre-committed two-grid $5/1000h bar. Phase C applied edits: CLAUDE.md headline section rewritten; checklist.md 95%/70% agreement tasks struck through with decision pointers; Decision 131 appended; CURRENT_PHASE.md rewritten for S97; STRATEGY_GUIDE.md Part 1 appended (this session); MASTER_HANDOFF_01 + SPRINT_INDEX updated. **No code touched. No engine runs.** Match% retained as diagnostic only — still informative for debugging, comparable across sessions, but not the headline. Production v65 UNCHANGED at $1,633.79 full / $776.88 prefix; v44_dt UNCHANGED for 24th consecutive session. Rule count UNCHANGED at 25. Cumulative closure UNCHANGED at 92.09%. NEW S96 methodology lessons: (1) doc drift compounds across sessions — periodic audits of load-bearing headlines against actual project state catch zombie targets cheaply (~1 session of grep + editing); (2) headline-goal recalibration is a one-off — three saturation closures across major levers is the legitimate evidentiary trigger, not a single NULL session; (3) $/1000h survives label noise where match% does not (32% oracle self-disagreement at N=200↔N=1000 per S79 means >68% match% is partially memorizing label noise; $/1000h averaged across 6M × 105 × N is robust by CLT). S97 PRIMARY: A3 ML retrain (~70h wall, credibility-low expected outcome, operator authorization required). S97 SECONDARY: v52-defensive-low partial-effectiveness exploit. S97 TERTIARY: v44_RULE13 fallthrough replacement. Previous front-matter (Session 95 — **STRATEGY OF RECORD UNCHANGED: v65 remains production. MIXED on trips B_DS_AVAIL_LKR intra-Layout-A force-best-DS-bot candidate (Rule 26).** S95 executed the S94-defined PRIMARY path: design strategy_v66 with a Rule-20-analog "force-best-DS-bot-within-Layout-A" picker for trips, multi-gate pre-drill on N=200, Option C N=1000 sparse-grid grading. Phase A re-confirmed S94 sub-bucket numbers from per-hand parquet (resume prompt's n=210/n=370 lines were transcription typos — JSON shows NARROW=8,160 hands at 90.4% meanP / $5.89 wg; MEDIUM=15,072@77.7%/$10.07; WIDE=44,537@56.8%/$22.64). Phase B's initial picker carried over Rule 20 / 25's `bot_pair_high desc, top_rank desc` criterion: ALL three gates strongly NEGATIVE at N=200 ($−3.22/$−6.25/$−8.84) because exact-pick match with oracle was only 7.5% on NARROW despite 88.7% direction-predictivity. An 8-criterion picker sweep identified **TOP_HIGH then pair_tops** as the correct criterion: NARROW $+4.59 (85.2% sr) — a $7.81/1000h swing from picker change alone. Phase C two-grid grader (51,531-id WIDE engine run, 1,901s wall at 27.1 hands/s, N=1000 sparse grid bit-identical to existing prefix grid by construction): **NARROW $+4.59 N=200 / $+4.75 N=1000 (|Δ|=$0.16, 93.9% sign-agree) — MIXED. MEDIUM $+2.95/$+3.37 — MIXED. WIDE $+0.36/$+2.09 — MIXED.** No gate clears the $5 SHIP bar on either grid; closest is NARROW at N=1000 $+4.75 ($0.25 short). Pre-committed two-grid standard locks all 3 gates as MIXED. Production v65 UNCHANGED at $1,633.79 full / $776.88 prefix. v44_dt UNCHANGED for 23rd consecutive session. Rule count UNCHANGED at 25. Decision 130 records the MIXED + 5 methodology refinements: (1) **PICKER ALIGNMENT IS THE SECOND NECESSARY CONDITION FOR RULE EXTRACTION (NEW S95)** alongside S94's trigger-predictivity boundary — high direction predictivity is necessary but not sufficient; the rule's deterministic picker must also align with oracle's specific pick (Rules 20/25 had ≥89% picker alignment by happy accident; the same `bot_pair_high` criterion got 3-8% alignment for trips); (2) PICKER CRITERIA ARE CELL-SPECIFIC, NOT TRANSFERABLE — Rule 20/25's `bot_pair_high` priorities bot strength which doesn't carry over to trips where bot strength = trip card and top wants the highest kicker; (3) operational lesson — resume-prompt transcription typo on S94 sub-bucket sizes; always cross-check anomalous numbers against the JSON of record; (4) Option D-revised rule-extraction lever now exhausted across both bucket-level AND intra-layout sub-classes on the current ML architecture; (5) MIXED verdicts are informative — this candidate is borderline ($4.59-$4.75) but cleanly fails the bar, no methodology adjustment recovers it. S96 PRIMARY (PROMOTED from S94/S95 SECONDARY): headline-goal recalibration. S96 SECONDARY: A3 ML retrain (~70h wall). S96 TERTIARY: v52-defensive-low partial-effectiveness exploit. Previous front-matter (Session 94 — **STRATEGY OF RECORD UNCHANGED: v65 remains production. STRUCTURAL NULL on within-v44_dt rule-extraction (Option D-revised) applied to two_pair LAYOUT_A_SS + 9 adjacent residual cells.** S94 executed the S93-defined PRIMARY path verbatim: rule-extraction on the largest unaddressed within-v44_dt residual cell, two_pair LAYOUT_A_SS at $35.22/1000h on 437,580 hands. Phase A characterization: v44_dt structurally saturated at the (hi_pair, lo_pair) bucket level — 75 of 78 buckets have matching v44/oracle modal layouts. Sub-bucket trigger search across 691 (hi, lo, max_sing) sub-buckets within v44_layout=B found ZERO buckets with P(oracle=Layout C | v44=Layout B) ≥ 70% (Rule 20 anchor). Cross-cell audit across all 10 within-v44_dt residual cells (6 two_pair + 4 trips): bucket-level layout-flip ceiling **$5.08/1000h** under the $5 SHIP bar even at 100% trigger accuracy; max sub-bucket predictivity 57.5% (below 62% Rule 25 anchor). Intra-Layout-A bot_suit follow-up on trips B_DS_AVAIL_LKR identified $19.49/1000h aggregate signal; two narrow sub-buckets reach 98-100% predictivity but together only $0.53/1000h; ≥70% sub-buckets total 8,160 hands at $5.89/1000h — borderline S95 ship candidate. Verdict: STRUCTURAL NULL on PRIMARY. Production v65 UNCHANGED at $1,633.79 full / $776.88 prefix. v44_dt UNCHANGED for 22nd consecutive session. Decision 129 records the NULL + 4 methodology refinements: (1) Option D-revised rule extraction has a STRUCTURAL SATURATION BOUNDARY analogous to S91/S92's chain-audit boundary; (2) Rule 20 / Rule 25 trigger anchor (≥62-70% predictivity) is the OPERATIONAL DEFINITION of "rule extractable"; (3) intra-layout improvements are a distinct lever sub-class within Option D-revised; (4) hand-specific residual dominates structural residual on these cells. Both chain-audit + bucket-level rule-extraction levers are now characterized as saturated on the current ML architecture. S95 PRIMARY: trips B_DS_AVAIL_LKR intra-Layout-A bot_suit candidate (borderline 20-40% ship probability); S95 SECONDARY: headline-goal recalibration; S95 TERTIARY: A3 ML retrain. Previous front-matter (Session 93 — **STRATEGY OF RECORD CHANGED: v64 → v65_mid_pair_chain_extend. FIRST SHIP via two-grid bar on a prefix-uncovered cell; unblocks the v60 candidate parked since S86.** S93 executed the S92-defined PRIMARY path verbatim: build Option C N=1000 oracle generator infrastructure via engine `--id-list-file` mode and retroactively validate v60. Phase A: added `solve_grid_ids` to `engine/src/oracle_grid.rs` and the `--id-list-file` CLI option to the `OracleGrid` subcommand in `engine/src/main.rs`; output is a SPARSE TWOG file (records carry own canonical_id) with header `canonical_total` = id-list length so resume + header-mismatch guards still work. Phase B correctness test: 100 ids spread across prefix range [0, 500K) produce **bit-identical EVs to the existing prefix N=1000 grid** (per-hand seed = `base_seed + canonical_id × φ` is deterministic). Throughput ~25 hands/s parallel at N=1000. Phase C-1 (`prepare_v60_id_list_S93.py`): iterated MID × PMID_DS_NOMAXTOP cell (114,048 hands), identified 32,304 changed-hand canonical_ids at gate=12 (max_sing ≤ Q), recomputed S86 N=200 baselines and got an EXACT match (gate 10 +$1.63 / gate 11 +$4.85 / gate 12 **+$6.43**). Phase C-2: invoked engine on the 32,304-id list, **1,277 s wall (21:17) at 25.3 hands/s** producing `data/session93/v60_n1000_sparse.bin` (13.7 MB). Phase C-3 (`grade_v60_id_list_n1000_S93.py`, pre-committed thresholds locked BEFORE reading the sparse grid): N=1000 lift on the same 32,304 hands = **+$6.34/1000h**; |N=200 − N=1000| = $0.09; two-grid SHIP standard cleared (both grids ≥ $5). Built `strategy_v65_mid_pair_chain_extend.py` composing v64's HIGH_ONLY chain-audit gate with v60-gate12's MID pair rule (firing zones DISJOINT — HIGH_ONLY has no pair, v60 requires exactly one MID pair). Final whole-grid grader (`grade_v65_full_grid_S93.py`): v65 whole-grid N=200 lift over v64 = **+$6.43/1000h** (matches Phase C-1 baseline exactly); v64==v57 on 32,304/32,304 changed hands (composition assumption holds); v65==v64 on 50,000/50,000 out-of-cell random sample (composition is safe outside the cell). **Production: $1,627.36 → $1,633.79/1000h on full grid; prefix UNCHANGED at $776.88 (rule fires entirely outside prefix coverage); production vs v44_dt $546.36 → $552.79; remaining gap to oracle $117.84 → $111.41/1000h; cumulative closure since pre-S68 91.6% → 92.09%.** Rule count: 24 → **25** (Rule 25 = MID pair × PMID_DS_NOMAXTOP × max_sing ≤ Q × v57-pick-tmax-style → force PMID_tnomax_DS). Decision 128 records the SHIP + five methodology refinements: (1) Option C N=1000 sparse infrastructure works at production quality (bit-identical to existing prefix grid); (2) MIXED-by-methodology candidates are recoverable if the only blocker was prefix coverage (v60 parked 7 sessions); (3) pre-committed two-grid thresholds are robust at low effect sizes (77.8% per-hand sign-agreement but $0.09 aggregate agreement); (4) disjoint firing zones make production composition trivially safe (no separate whole-grid regrade needed when zones are demonstrably disjoint); (5) chain-audit-arc-complete (S92 closure) does NOT mean rule-arc-complete — S93's ship is rule-extraction (Option D-revised) on a within-v44_dt residual, unblocked by infrastructure (Option C). v44_dt UNCHANGED for 21st consecutive session. S94 default plan: rule-extraction on within-v44_dt residual (two_pair LAYOUT_A_SS at $35.22/1000h on 437K hands; OR LOW pair PMID_DS_MAXTOP refinement), OR validate other parked MIXED candidates via Option C (gate 11 of v60 currently MIXED at +$4.85/+$4.77), OR headline-goal recalibration. Chain-audit lever remains exhausted on current architecture. Previous front-matter (Session 92 — **STRATEGY OF RECORD UNCHANGED: v64 remains production. Second consecutive non-ship session in chain-audit run.** S92 was the planned execution of the S91-defined PRIMARY + SECONDARY paths: pivot to two_pair chain audit and trips chain audit. Pre-flight code trace of `strategy_v55_two_pair_hybrid` and `strategy_v56_trips_hybrid` revealed both blanket-route 100% of target hands → v44_dt, so production v64 ≡ v44_dt on every two_pair and trips hand by construction. Empirical confirmation: 0 mismatches across all 1,338,480 two_pair hands; Δ(v64 − v44_dt) = $0.00 on every cell. Chain audit produced the architectural snapshot: **v44_RULE13 chain bleeds $+515.80/1000h prefix vs v44_dt on two_pair (3× the LOW pair bleed S91 found — largest single-layer bleed in project) and $+33.21 on trips**. v55 absorbs $515.80 (blanket two_pair → v44_dt); v56 absorbs $33.21 (blanket trips → v44_dt). v54/v55/v56/Rule 29 cumulative absorption across pair-family: **$731/1000h** — most load-bearing infrastructure in the project. Phase C grader: 0 candidates exist (STRUCTURAL NULL distinct from S91's POPULATION-DIVERGENCE NULL). Verdict: STRUCTURAL NULL on both targets. Decision 127 records the NULL + seven methodology refinements: (1) CHAIN-AUDIT METHODOLOGY ARC COMPLETE across all 4 major hand-categories (HIGH_ONLY/single-pair/two_pair/trips) — 5 sessions, 4 SHIPS ($214.83), 2 NULLs at well-characterized boundaries; (2) PRE-FLIGHT CODE TRACE AS PIVOT-GATE (NEW) — read strategy code before drilling; (3) STRUCTURAL NULL is distinct from POPULATION-DIVERGENCE NULL (NEW) — different mechanisms, different path forward; (4) v44_RULE13 chain bleed quantified across pair-family ($731 absorbed by infrastructure); (5) TWO-CONSECUTIVE-NULL is decision-relevant signal that lever is saturated (NEW) — default next-session plan should pivot, not default to more of the same lever; (6) chain-audit applicability test now 3-pronged with NEW condition (c) "production picks must differ from v44_dt"; (7) NULL audits are still complete cycles. Rule count UNCHANGED at 24. Cumulative closure since pre-S68 UNCHANGED at 91.6%. Combined S87-S92 chain-audit recovery still $214.83/1000h (S91+S92 contribute $0). v60 candidate from S86 STILL UNSHIPPED. v44_dt UNCHANGED for 20th consecutive session. S93 default plan: pivot from chain-audit to Option C N=1000 oracle generator infrastructure (PROMOTED from TERTIARY to PRIMARY) — modify engine/src/main.rs to add --id-list-file option, ~30-60 min Rust mod, unlocks retroactive v60 validation and broader two-grid checking; OR rule-extraction on two_pair LAYOUT_A_SS residual ($35.22/1000h on 437K hands; SECONDARY); OR headline-goal recalibration (TERTIARY). Previous front-matter (Session 91 — **STRATEGY OF RECORD UNCHANGED: v64 remains production. First non-ship session in chain-audit run (S87/88/89/90 streak of four broken cleanly).** S91 was the planned execution of the S90-defined PRIMARY path: pivot to LOW pair prefix-COVERED cells using the chain-audit pattern — methodologically stronger than the prefix-silent S87-S90 ships because the prefix grader provides a real two-grid SHIP standard. Phase A confirmed all 6 LOW pair cells are prefix-covered (cid_min 61,085-62,041; well below 500K prefix boundary). Phase B pre-drill on all 1,292,544 LOW pair hands: v64 LIFTS $92.41/1000h over v44_dt on full grid and $13.36 on prefix — production works as designed. Phase B+ chain audit (layer attribution v44_dt → v44_RULE13 → v54 → v57 → v64 on 215,162 prefix hands) identified the architectural source: **v44_RULE13 (the rule-based chain fallthrough) BLEEDS $182.28/1000h vs v44_dt on LOW pair** — biggest single-layer bleed in the project. v54's PBOT_DS→v44_dt hybrid routing absorbs $178.82; v57's Rule 29 absorbs another $16.81. **v54 + Rule 29 are LOAD-BEARING: they recover $195.63/1000h, more than fully compensating for the chain bleed.** Five residual sub-cell bleeds totaling $14.24/1000h on prefix were identified but ALL DISAGREE with the full grid on direction (population-divergence noise — prefix is a non-random lower-cid subset). Three v65 candidates evaluated against pre-committed two-grid thresholds (SHIP requires prefix ≥ $5 AND full ≥ $5): all FAIL — Candidate A (PMID_DS_NOMAXTOP × max_sing=K) NULL (grid negative), Candidate B (PMID × max_sing=J non-DS_NOMAXTOP) MIXED, Candidate C (combined) MIXED. **Aggregate verdict: NULL. Production v64 UNCHANGED.** Decision 126 records the NULL + seven methodology refinements: (1) TWO-GRID SHIP STANDARD applied to prefix-COVERED audits for first time, (2) POPULATION-DIVERGENCE NOISE definition (NEW) — prefix and full evaluate different canonical_id populations within the same nominal sub-cell so per-sub-cell Δ can legitimately diverge in direction when effects are small, (3) CHAIN-AUDIT APPLICABILITY TEST (NEW) — pattern needs EITHER prefix-silent target OR ≥$5/1000h per-cell bleed on both grids, (4) v44_RULE13 is project's largest hidden net-negative layer at $182.28/1000h, (5) v54 + Rule 29 are load-bearing, (6) pre-committed-threshold pattern critical for honest verdicts, (7) NULL audit is a complete cycle. Rule count UNCHANGED at 24. Cumulative closure since pre-S68 UNCHANGED at 91.6%. Combined S87-S91 chain-audit recovery still $214.83/1000h (S91 contributes $0). v60 candidate from S86 STILL UNSHIPPED. S92 default plan: pivot to two_pair chain audit (PRIMARY), OR trips chain audit (SECONDARY), OR build Option C N=1000 oracle infra (TERTIARY). Previous front-matter (Session 90 — **STRATEGY OF RECORD CHANGED: v63 → v64_high_only_chain_fix_zone. FOURTH CONSECUTIVE chain-audit ship; structurally-non-empty HIGH_ONLY × max ≥ 8 audit COMPLETE; first project-level audit of v52-defensive-low confirms it PARTIALLY effective.** Executed the S89-defined PRIMARY path verbatim: pivot to HIGH_ONLY × max ∈ {7-T}. Phase A structural feasibility eliminated 4 (cell × rank) combinations for free (max=7 × any cell; max ∈ {8,9,T} × {JOINT_HIGH, NEITHER}; max=8 × JOINT_MED). Pre-drill on 25,740 prefix-silent hands: **v63 leaks +$7.23/1000h MORE than v44_dt on these cells** — smaller magnitude than S87/S88/S89 (cell pop ~half S89's 48K) but mechanically identical. Chain audit attributed v44→v47 = +$19.28 (4th consecutive session with v47 as culprit), v47→v48 = -$2.53, v48→v52 = -$9.52 (v52-defensive-low partially recovers but doesn't fully restore v44). Rule 24 (v64): strict superset of Rule 23's gate. For HIGH_ONLY × max ∈ {8,9,T,J,Q,K,A} × cell ∈ all 6 non-empty cells (DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH, JOINT_MED, JOINT_LOW), bypass v47→v48→v52 chain and return strategy_v44_dt directly. Full-grid grader auto-fired SHIP: **+$7.23/1000h whole-grid (v63 $1,620.13 → v64 $1,627.36)**. Per cell (rolled up): DS_NO_JOINT +$3.75 / JOINT_MED +$1.35 / DS_NO_MAXTOP +$0.92 / MS_ONLY +$0.90 / JOINT_LOW +$0.31. Per-hand: 22% same / 55% better / 23% worse. Swap-right rate on changed hands: **70.4%** — LOWER than S88/S89 (76.8%/85.2%) but higher than S87 (62.3%). The elevated 23.2% "worse" rate is the signature of v52-defensive-low's partial effectiveness. Out-of-gate sanity: **0 v64≠v63 disagreements** on 50K random sample. Pre-drill ↔ grader match to $0.01 across all cells (high aggregate stability). Rule count: 23 → **24**. Remaining gap to oracle ceiling: $117.84/1000h (was $125.07; cumulative closure since pre-S68 now **91.6% of original $1,409**). Combined S87+S88+S89+S90 chain-audit recovery: **$214.83/1000h**. **Structurally-non-empty HIGH_ONLY × max ≥ 8 chain-audit zone is now COMPLETE.** FIRST PROJECT-LEVEL AUDIT OF v52-defensive-low (LOW_MAX_DEFENSIVE = {7,8,9,10}): 100% of S90 target hands fire this mode. Verdict: PARTIALLY effective (recovers $9.52 of v47's $19.28 bleed; aggregate still favors v44_dt by $7.23). S53 design intent was correct but conservative. Decision 125 records the ship + seven methodology refinements: (1) chain-audit pattern transferred 1:1 for fourth consecutive session, (2) EFFECT-SIZE-DOMINANCE rule generalizes across four ships and three orders of magnitude, (3) pre-drill ↔ grader match-to-2-decimal-places is the load-bearing stability signal, (4) per-hand swap-right < 65% is a "chain-layer-doing-real-work" flag, (5) v52-defensive-low is partially effective (first project audit), (6) Phase A structural feasibility check is fully transferable, (7) future v65 refinement opportunity for v52-DL partial-effectiveness exploit (per-hand picker on the ~23% of S90 hands where v52-DL wins). v60 candidate from S86 STILL UNSHIPPED, MIXED-by-methodology, pending Option C N=1000 oracle infrastructure. S91 default plan: pivot to prefix-COVERED cells (LOW pair / two_pair / trips) using the chain-audit pattern — key methodological advantage is the prefix grader provides a real two-grid SHIP standard; or build Option C N=1000 oracle infra; or refine v52-defensive-low. Previous front-matter (Session 89 — **STRATEGY OF RECORD CHANGED: v62 → v63_high_only_chain_fix_full. THIRD CONSECUTIVE chain-audit ship; HIGH_ONLY × {J-A} audit ZONE CLOSED.** Executed the S88-defined PRIMARY path verbatim: chain-audit expansion to the remaining prefix-silent HIGH_ONLY × {J-A} cells (JOINT_MED × {J-A} = 44,562 hands, JOINT_LOW × {J-A} = 3,570 hands, total 48,132). Phase A surprise: **NEITHER × {J-A} is structurally empty** (combinatorial proof: HIGH_ONLY hands have 6 non-max cards across 4 suits, so by pigeonhole ≥2 share a suit → cell ≠ NEITHER). Pre-drill: **v62 leaks +$10.09/1000h MORE than v44_dt on these cells** — smaller magnitude than S87/S88 (cells ~14× and ~7× smaller respectively) but mechanically identical. Chain audit attributed 96% of bleed to the v44→v47 transition — third consecutive session with same culprit. Rule 23 (v63): strict superset of Rule 22's gate. For HIGH_ONLY × max ∈ {J,Q,K,A} × cell ∈ all 6 non-empty cells (DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH, JOINT_MED, JOINT_LOW), bypass v47→v48→v52 chain and return strategy_v44_dt directly. Full-grid grader auto-fired SHIP: **+$10.09/1000h whole-grid (v62 $1,610.04 → v63 $1,620.13)**. Per cell: JOINT_MED +$8.24 / JOINT_LOW +$1.85. Per-hand: 49% same / 44% better / 8% worse. Swap-right rate on changed hands: **85.2%** — HIGHEST across S87/S88/S89. Out-of-gate sanity: **0 v63≠v62 disagreements** on 50K random sample. Rule count: 22 → **23**. Remaining gap to oracle ceiling: $125/1000h (was $135; cumulative closure since pre-S68 now **91.1% of original $1,409**). Combined S87+S88+S89 chain-audit recovery: **$207.60/1000h** — bigger than every value-extraction ship from S71-S86 combined. **HIGH_ONLY × {J-A} chain-audit zone is now CLOSED**; all 6 non-empty cells gated. Decision 124 records the ship + five methodology refinements: (1) chain-audit pattern transferred 1:1 for third consecutive session, (2) EFFECT-SIZE-DOMINANCE rule generalizes across scales via noise-floor multiple, (3) pre-Phase A structural feasibility check can close target cells for free, (4) per-hand swap-right rate is a useful secondary verdict signal, (5) HIGH_ONLY × {J-A} audit closure is a methodology milestone. v60 candidate from S86 STILL UNSHIPPED, MIXED-by-methodology, pending Option C N=1000 oracle infrastructure (now deferred to S90+). S90 default plan: pivot to HIGH_ONLY × max ∈ {7-T} (DIFFERENT firing mode — v52-defensive-low rather than v52-fallthrough; UNKNOWN whether v47 chain bleed extends here); or audit prefix-COVERED cells (LOW pair / two_pair / trips); or build Option C N=1000 oracle infra. Previous front-matter (Session 88 — **STRATEGY OF RECORD CHANGED: v61 → v62_high_only_chain_fix. BACK-TO-BACK $98+ production ships; chain-audit lever is now the project's dominant ship vector.** Planned expansion of the S87 chain-audit pattern to three more prefix-silent HIGH_ONLY × {J-A} cells: DS_NO_MAXTOP × {K,A} (133K hands), MS_ONLY × {J-A} (108K hands), JOINT_HIGH × {K,A} (117K hands) = 357,504 hands total carrying $95.62/1000h v44_dt baseline leak. Pre-drill replay: **v61 leaks +$98.84/1000h MORE than v44_dt on these cells** — virtually identical magnitude to S87's $98.67 finding on DS_NO_JOINT. Chain audit attributed 99.7% of bleed to the v44→v47 transition (same culprit as S87 — Rules 13-16, Q-high DS chain). Rule 22 (v62): strict superset of v61's gate. For HIGH_ONLY × max ∈ {J,Q,K,A} × cell ∈ {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH}, bypass v47→v48→v52 chain and return strategy_v44_dt directly. Full-grid grader auto-fired SHIP: **+$98.84/1000h whole-grid (v61 $1,511.20 → v62 $1,610.04)**. Per cell: DS_NO_MAXTOP +$52.88 / MS_ONLY +$31.51 / JOINT_HIGH +$14.45. Swap-right 76.8% on 245K changed hands (better than S87's 62.3%). Out-of-gate sanity: **0 v62≠v61 disagreements** on 50K random sample. Rule count: 21 → **22**. Two-track divergence (remaining gap to oracle): $135/1000h (was $234; cumulative closure since pre-S68 now **90.4% of original $1,409**). Combined S87+S88 chain-audit recovery: **$197.51/1000h** — bigger than every value-extraction ship from S71-S86 combined. Decision 123 records the ship + the "v47 is net-negative across the entire HIGH_ONLY × {J-A} weak-hand zone" architectural finding. v60 candidate from S86 STILL UNSHIPPED — waits on Option C N=1000 oracle infrastructure (now deferred to S89+). S89 default plan: continue chain-audit expansion to JOINT_MED/LOW/NEITHER × {J-A} (smaller cells but transferable pattern); or pivot to HIGH_ONLY × max ∈ {7-T} (different firing mode, unknown bleed); or audit prefix-COVERED cells (LOW pair/two_pair/trips). Previous front-matter (S87 — **STRATEGY OF RECORD CHANGED: v57 → v61_high_only_ds_no_joint_fix. LARGEST single-rule production ship since S70.** User reframed strategic priority at session start: "easy hands are easy to play — bleeding lives on weak hands where damage control is hard." Empirical look at S71 HIGH_ONLY data confirmed: HIGH_ONLY × DS_NO_JOINT × {J-A} cells leak $76-125/1000h per cell vs v44. User picked a gated-pivot plan (pre-drill addressability first). Pre-drill bombshell: v57 leaks **+$98.67/1000h MORE than v44_dt** on this 756,000-hand cell sub-population. v52's HIGH_ONLY handler had been actively bleeding EV for 33+ sessions, undetected because prefix grader is structurally silent for these cells. Layer-by-layer audit identified v47 (Rules 13-16, Q-high DS chain) as the dominant source (+$82/1000h on fallthrough subset), with v52-J-HIMID adding $12.76 and v52-defensive-gated $3.84. Rule 21 (v61): surgical gate-out — for HIGH_ONLY × DS_NO_JOINT × max ∈ {J,Q,K,A}, bypass v47→v48→v52 chain and return strategy_v44_dt directly. Full-grid grader auto-fired SHIP: +$98.67/1000h whole-grid (v57 $1,412.53 → v61 $1,511.20). Per-rank: J +$13.59 / Q +$19.24 / K +$28.95 / A +$36.89. Swap-right 62.3% on 561K changed hands. Out-of-cell sanity: 0 disagreements on 50K random sample. Rule count: 20 → **21**. Two-track divergence v61-vs-v44: $234/1000h (was $332; cumulative closure since pre-S68 now 83% of original $1,409). Decision 122 records the ship + the prefix-silent-can-hide-regressions methodology lesson. v60 candidate from S86 remains DEFERRED (Option C N=1000 oracle infra build deferred to S88). S88 default plan: expand the audit pattern to DS_NO_MAXTOP and MS_ONLY cells (next-largest prefix-silent zones likely carrying buried regressions). Previous front-matter (S83 — **STRATEGY OF RECORD CHANGED: v56 → v57_lo_pair_defensive. First production change in 12 sessions.** v57 adds Rule 20 (LOW pair × PMID_DS_NOMAXTOP × max_sing ≤ Q → force PMID with DS-bot and non-max top) over v56. Pre-committed grader auto-fired SHIP on both grids: +$16.47/1000h whole-grid (v56 $1,429 → v57 $1,412.53), +$16.81/1000h prefix (v56 $794 → v57 $776.88), prefix match% 66.07% → 66.49% (+0.42pp). Mechanism: v56 (= v52 for this cell) over-routes to max-kicker-on-top + single-suited bot when oracle wants drop-max + double-suited bot; the LOW pair has no offensive top potential so the DS-bot trade is the real value. Discriminator `max_sing` is razor-sharp: swap-right 89-93% at ≤J, 77% at Q, 54% at K, WRONG 86% at A. Multi-gate grade chose Q as the lift maximum (K still ships at +$12.94 but smaller; A is catastrophic NULL at −$89). User's strategic redirect at end of S82 — "stop min-maxing well-covered areas, focus on under-rule-covered weak-hand zones" — was the load-bearing decision. Rule count: 19 → **20**. Two-track divergence v57-vs-v44: $332/1000h (was $348; cumulative closure since pre-S68 is 76% of original $1,409). Decision 118 records the ship + methodology. The Option D rule-layer track is now FORMALLY VALIDATED as alive while the ML-cascade track remains exhausted at v44 saturating regime per Decisions 113 + 117. S84 default plan: extend playbook to LOW pair × PMID_DS_MAXTOP ($21.68 STRUCTURE leak). Previous front-matter (S79 — **MEASUREMENT-ONLY / NO STRATEGY OF RECORD CHANGED: label-noise measurement on existing 500K N=1000 prefix grid vs the same 500K hands labeled at N=200 in the full grid.** 500K hands swept in 68.8s (~7,263 hands/s). v44 match rate vs N=200 = 72.98%, vs N=1000 = 67.05% — **match-rate shift = −5.93pp**, with **oracle self-agreement of 68.00%** (N=200 vs N=1000 argmax disagree on 32% of hands). Bucket-level decomposition shows the MATCH bucket (rank 1, 73% of mass) loses 21.89pp at N=1000 (v44 memorizing N=200 noise) while the NOISE bucket (rank 2-3, 19.5% of mass) gains +43.58pp (most of v44's apparent "leak" at N=200 was just label noise). Per-category, pair (+2.10pp) and three_pair (+1.59pp) are least-overfit; two_pair (−13.69pp) and trips_pair (−19.43pp) are most-overfit. The pre-committed shift-based criterion read mechanically as **C-PATH** (shift < +2pp → "labels stable, real model error") but its underlying assumption is refuted by the 32% oracle self-disagreement, so the **honest verdict is MIXED — surface options to user**. Decision 114 records the MIXED verdict + criterion blind spot + S80 plan: **M2 = parallel A1 + C2 one-session experiments** (A1: retrain v44 DT on N=1000 prefix labels; C2: regularize v44 DT with max_leaf_nodes=500K and min_samples_leaf=5). S81 chooses among A2/C1/M1/headline-recalibration based on which experiment lifts N=1000 match rate above v44's 67.05%. **No ship attempted; production UNCHANGED for the eighth consecutive session.** Previous front-matter (S78 — v48_dt H6/H7/H8 NULL): v48_dt H6+H7+H8 pair feature pack CLEAN NULL at prefix grade Δ +$2/1000h** (v44_dt $686 → v48_dt $684; within-pair lift +$5/1000h on 215,162 hands, $595→$590; all other categories byte-identical, confirming gating works correctly). Per S78 directive `prefix Δ < +$5 → NULL ship`, Phase 6 (full grade) was SKIPPED. Phase 1 implemented and sanity-tested H6 (`pair_pmid_ds_n_configs_g`, int8 0..5 — actual reachable values {0,1,3}), H7 (`pair_kicker_max_in_pair_suit_g`, bool 0/1), H8 (`pair_low_pmid_safety_g`, int8 0..5 LOW-pair-only). H8's inline cell logic cross-validated against S66's `compute_pair_structural` on 23,377 random pair hands (0 disagreements). Phase 2 persisted three gated parquets (~18.6 MB each) with zero-on-non-gated verification. Phase 3 smoke train (100K rows, 2.5s wall) placed H7 #51 / H8 #69 / H6 #73 — under abort-rank #80 but well below top-30 ideal. Phase 4 full train (4.8M rows, 578s wall, 2.29M leaves, 1,285 MB) improved placements to H7 #43 / H8 #57 / H6 #68. **Why the predicted lift didn't land — structural-redundancy NULL reconfirmed.** S77 budgeted 50% redundancy across H6+H7+H8 and predicted $30-45/1000h within-pair joint lift; observed effective redundancy is ~85-90% at v44's 2.25M-leaf saturating regime. Three contributors: (a) H6 only emits {0,1,3} due to deck structure (suit profile 2+2+1 → 1 config, 3+2 → 3 configs, else → 0) — partitioning surface ~3 bits, not the planned 6; (b) H7's 1-bit signal is derivable in 2 splits from `pair_kickers_in_pair_suit_max_g ≥ 1 AND pair_default_top_rank_g == max_sing_rank`; (c) H8's 5-split cell synthesis is reachable in 3-4 splits at v44's saturating capacity via `pair_aug_v5_bot_DS_n_configs_g` + `pair_r4_bot_suit_profile_g` + `pair_kickers_in_pair_suit_*_g`. **The S74 redundancy doctrine — features the DT can derive in <few splits at saturation capture zero or near-zero lift — is empirically reconfirmed.** S77's project-cleanest diagnostic (sharp gap_2nd 0.385, single dominant mismatch class with $10.97/1000h on 3,072 hands) does NOT break the redundancy ceiling. Diagnostic precision and feature lift are orthogonal at saturation. **Decision 113: single-model ML feature-engineering track formally CLOSED at v44 regime.** Six consecutive sessions (S72 NULL, S73 PARTIAL/NULL, S74 NULL, S75 boosting NULL, S77 diagnostic, S78 NULL) failed to ship. Path forward priority order: (1) Option D rule-chain extension on S77 LOW pair kicker_max-in-pair-suit discriminator — operates at rule layer outside DT saturation, where redundancy ceiling does NOT apply; (2) fresh cross-product diagnostic on two_pair / trips STRUCTURE under S71-bucket × cell lens (never probed); (3) Option A oracle-label N=1000 re-eval (gated on cluster access). Production state UNCHANGED for the **seventh** consecutive session.)
...
## Session 61: K-high cell-by-cell catalog audit — NULL RESULT (all 7 candidates fail; K-high formally labeled ML-only; lesson generalizes from 6% to 34% drop-max rate)

Session 60's null result on A-high left the per-max-rank rule catalog methodology open: would K-high — where oracle drops max off top **5.6× more often** (34% in DSnj vs A's 6%) — yield a Threshold-2 shippable rule? Session 61 reused the S60 harness verbatim, audited Rule 15 (K-high HIMID, S51) cell-by-cell across K-high's 6 structural cells, and tested **7 K-specific candidate refinement rules** against the same T1/T2/T3 thresholds.

**Harness — second validation.** Sanity check: Rule 15 (`strategy_v46_rule15_Khigh_DS`) vs its pre-Rule-15 predecessor (`strategy_v45_rule14_Ahigh_DS`) reproduced **+$51.38/1000h whole-grid lift on K-high** — matches CURRENT_PHASE's documented S51 ship of +$51 to within 0.7%. **The harness is now validated on TWO independent shipped rules** (Rule 14 +$131 to 0.2%, Rule 15 +$51 to 0.7%). Production-grade audit infrastructure.

**Phase 2 — Rule 15 cell-by-cell audit (K-high).** Per-cell remaining gap to oracle after Rule 15, in $/1000h whole-grid: JOINT_HIGH=$12.45, JOINT_MED=$2.86, JOINT_LOW=$0.08, **DS_NO_JOINT=$105.94** (60% of K-high leak), DS_NO_MAXTOP=$36.76, MS_ONLY=$18.26. **Total K-high residual after Rule 15: $176.35/1000h whole-grid** — vs v44_dt's $110.94/1000h. v44 captures $65.41 more than Rule 15 on K-high. **Within-cell DSnj gap is 31% deeper at K than A** ($3,062/1000h vs $2,337/1000h) — the structural consequence of oracle dropping K off top 34% in this cell while Rule 15 keeps K on top 100%.

**Phase 3+4 — 7 candidate refinement rules tested vs v52 baseline.** Five DSnj candidates (drop-K and SS_ms switch variants), one MSonly drop-K candidate, one DSnj HIBOT negative control:
- **C_K1 (drop K to top ≤ 7 + DS bot + ms_mid_high ≥ J):** fires **34.9% of DSnj** — almost EXACTLY oracle's drop-K rate of 34% — yet net **−$20.08/1000h WG**. The set MEMBERSHIP is wrong even when the COUNT matches.
- **C_K2 (drop K when K is in DS bot pair):** fires 44.6%, **−$34.46/1000h WG** (worst). The S58 Observation 1 axis ("max in DS pair → oracle prefers DS_NONJOINT") is a tendency, not a deterministic rule.
- **C_K3 (top=Q + DS bot + ms_mid ≥ J):** fires 7.9%, **+$3.53/1000h WG**. Surgical Q-on-top capture is mathematically positive but capture% is 3.33% — far below T1's 40% bar.
- **C_K4 (switch K-top DS_mu → K-top SS_ms when SS_ms mid_high ≥ J):** fires 50.9%, **−$13.10/1000h WG**. Mirror of A-high C2 — same negative pattern even at K's higher 51% oracle ms_mid rate.
- **C_K7 (top=2 + DS bot + ms_mid ≥ T):** fires 7.9%, **+$1.05/1000h WG**. Tiniest positive lift; capture% 0.99%.
- **C_K5 (drop K in MSonly to top ≤ 7 + ms_mid ≥ J + SS or 31 bot):** fires **82.7%**, **−$21.48/1000h WG** (catastrophic). Way over oracle's 22% drop-K rate in MSonly.
- **C_K6 (HIBOT tiebreaker control):** fires 100%, **−$21.73/1000h WG**. **Confirms Rule 15's S51 HIMID design empirically — same retrospective validation pattern as A-high C10 produced for Rule 14.**

**Result: every candidate falls below Threshold 1.** K-high cells are formally labeled **ML-only territory** at the catalog granularity tested. Catalog produced: `SESSION_61_K_HIGH_CATALOG.md` (second page of `HIGH_ONLY_RULE_CATALOG.md`).

**Score: v52 UNCHANGED at $2,498/1000h on full grid. v44_dt UNCHANGED at $1,081 full / $686 prefix.** Improvement: unchanged at −$963 vs v16 ($2,044) full, −$1,167 vs v14 ($2,248) full.

**Methodology lesson — drop-max rate is NOT a recoverable quantity for simple gates, EVEN AT 34%.** S60's lesson said decision-matrix percentages overstate refinement headroom because oracle knows WHICH q% to switch on. S61 tested whether the lesson generalizes when q% is 5.6× larger. **It does.** C_K1 fires at 34.9% — almost exactly matching oracle's drop-K rate of 34% — and is net **−$20.08/1000h WG**. The set MEMBERSHIP is what matters, not the COUNT. **The lesson generalizes from 6% to 34%; expect it to generalize further at Q (52% drop), J/T/9/8 (similarly high).**

**Methodology lesson — the catalog approach systematically validates HIMID design choices.** Rule 14's HIMID was confirmed by C10 (S60). Rule 15's HIMID was confirmed by C_K6 (S61). The pattern: HIBOT replacement consistently shipping −$20 to −$40/1000h WG. Both shipped rules' tiebreaker design choices from S50/S51 are sound.

**Methodology lesson — K-high cells are STRUCTURALLY DEEPER than A-high but exhibit the same rule-saturation.** Rule 15's residual cells are 31–88% deeper per hand than Rule 14's (DSnj $3,062 vs $2,337; DS_NO_MAXTOP $4,980 vs $3,734; MS_ONLY $3,710 vs $3,598). The MORE oracle deviates from "max-on-top + DS HIMID" (K's 34% drop-K vs A's 6%), the bigger the within-cell gap to oracle, **but the rule space tested STILL can't close it.** This strengthens the case that high_only's residual is fundamentally non-rule-shaped at the catalog's "one sentence statable" granularity.

**End of S61:** Rule chain UNCHANGED at v52. ML champion UNCHANGED at v44_dt. **Two-thirds of high_only's WG residual ($457 of $755) now formally in ML-only zone after two audits.** Harness PROVEN AND READY for S62 (Q-high), where oracle drops max off top **52% in Q × DSnj** (vs K's 34%, A's 6%) — the most aggressive drop-max profile of zones tested so far. **Session 62's pivot: reuse harness verbatim (third use), audit Rule 16 (S52, +$19/1000h shipped) cell-by-cell, design Q-specific candidates. If pattern continues (likely), Q lands all-T3 and the high_only ML-only zone grows to nearly all of $755/1000h WG.**

## Sessions 62–66: high_only catalog closure, ML capacity null, pair Phase 1+2 audit (no strategy of record change)

Sessions 62–64 completed the per-max-rank high_only catalog audits (Q/J/T/9/8) — **all-T3 NULL across 26 candidates**. Session 65 synthesized the five per-max-rank catalogs into the canonical `HIGH_ONLY_RULE_CATALOG.md` formalizing the ML-only boundary claim: **across 1,226,940 canonical high_only hands, the production rule chain v52 cannot be refined further at the "one-sentence-statable" granularity. v44_dt captures $233/1000h of the $615 catalog-measured v52→oracle gap; the residual $381/1000h is ML-only territory at current rule-chain depth.** Five independent shipped-rule reproductions (Rule 14 +$131 to 0.2%, Rule 15 +$51 to 0.7%, Rule 16 +$19 to 1.7%, v52-vs-v47 ensemble +$17 to 1.4%, Rules 25/26/27 +$12 to 0.08%) validate the harness across a 23× magnitude range. Best single candidate is J-high's C_J1 at +$6.44/1000h WG / 21.54% capture (clears T2 dollars but fails T1's 40% capture bar).

Session 66 produced `PAIR_DECISION_MATRIX.md` — the pair-category Phase 1 (v44_dt vs oracle descriptive matrix) AND Phase 2 (v52 rule-chain audit) on 2,800,512 canonical pair hands. Phase 1 found the v44 vs oracle residual decomposes by a 6-cell taxonomy (PBOT_DS_JOINT, PBOT_DS_PARTIAL, PMID_DS_MAXTOP, PMID_DS_NOMAXTOP, PMID_SS_MAXTOP, PMID_OTHER). Phase 2 found v52 underperforms v44 by $341/1000h on pair, concentrated in PBOT cells ($391 WG gap; PMID cells favor v52 by $50). Six pair_ranks (A, K, 6, 7, 8, 9) have ZERO PBOT routing in v52; v9_2 covers only {2-5, T-J-Q} with an Ace predicate; Rule 11 fires uniquely on J-pair-J. Matrix framed five Phase 3 candidates (C_PAIR_1 through C_PAIR_5) with quantified WG ceilings ($28-89/1000h each) for S67 candidate sweep.

**Score (S62-S66): rule chain UNCHANGED at v52 ($2,498 full / $1,522 prefix); v44_dt UNCHANGED at $1,081 full / $686 prefix.** Improvement: unchanged at −$963 vs v16, −$1,167 vs v14 (full grid).

**Methodology lesson —** the catalog audit approach is a transferable methodology: 78 candidates tested across S60-S66, with 77 falsified and 1 (J-high C_J1) clearing T2 dollars but failing T1 capture. The pattern "candidate fires at oracle's deviation rate → still net-negative" generalized cleanly from 6% (A-high) through 52% (Q-high). The pair Phase 2 audit demonstrated that the matrix doc + v52 sweep produce richer direction-setting data than Phase 1 alone — Phase 1 suggested "fix v44's SPLIT bug at low pairs"; Phase 2 inverted to "extend v52 rule chain on PBOT cells", which then became S67's testable hypothesis.

## Session 67: Rule 19 (Q-pair × PBOT_DS_JOINT) ships as v53 — first rule chain ship since S53 (+$9/1000h)

Per S66's Phase 3 candidate sizing, S67 tested the largest-ceiling family C_PAIR_3 (6/7/8/9-pair PBOT-take, $88.71 catalog ceiling) plus the fallback C_PAIR_5 (Q-pair refinement, $41.20 ceiling). Built `analysis/scripts/test_rule_catalog_pair.py` — the pair-specific catalog harness, mirroring S60's high_only harness with the (pair_rank, cell) key + pre-computed v52 baseline from `drill_pair_v52_per_hand.parquet`. **Harness sanity check: Rule 11 reproduction +$6.21/1000h vs Decision 079's grader-confirmed +$6 baseline — 3.5% error, well within <5% tolerance** (CURRENT_PHASE end-S66 misquoted the target as +$11/Decision 080; corrected to +$6/Decision 079).

**Twelve candidates tested across C_PAIR_3 and C_PAIR_5 families; eleven are T3 net-negative.**

| Candidate | Fire region | Capture | WG lift | Verdict |
|---|---|---:|---:|---|
| C_PAIR_3a (simple 6-9) | 4 ranks × {JOINT, PARTIAL} | -202% | -$315.39 | T3 |
| C_PAIR_3b (mid≥J) | + mid≥J gate | -106% | -$165.20 | T3 |
| C_PAIR_3b (mid≥Q) | + mid≥Q gate | -73% | -$114.04 | T3 |
| C_PAIR_3b (mid≥K) | + mid≥K gate | -31% | -$48.71 | T3 |
| C_PAIR_3b (mid≥A) | + mid≥A gate (0 fires) | n/a | $0 | T3 |
| C_PAIR_3c (joint-only 6-9) | 4 ranks × JOINT | -12% | -$17.92 | T3 |
| C_PAIR_5 simple (Q-all) | Q × {JOINT, PARTIAL} | -25% | -$17.44 | T3 |
| **C_PAIR_5_joint** | **Q × JOINT only** | **+53% within-cell** | **+$8.50 harness** | **T2 → SHIPS** |

**Root cause of falsification:** at pair_ranks 6/7/8/9, oracle's average PBOT preference is 20-30%. Forcing PBOT on 100% of PBOT_DS-feasible hands at these ranks is wrong on 70-80% of cell hands. In 8-pair × PBOT_DS_JOINT specifically, rule fires 100% → rule pct_opt=26.57% vs v52 baseline pct_opt=44.69% — oracle prefers PMID on 73% of these JOINT cells. The matrix's "$391 WG PBOT-cell catalog-shippable headroom" was v44's *selective* gating (PMID/PBOT picked per-hand using 107 features), NOT something a blanket rule could realize. **The pair PBOT-routing problem is mostly ML-only at catalog granularity, structurally analogous to high_only S60-S64.**

**Why Q-pair × PBOT_DS_JOINT specifically ships:** Q-pair has the highest oracle PBOT preference of any pair_rank (53%, vs 18-37% at K/A/J/T/9/8/7/6), and the JOINT cell is structurally the strongest PBOT play (max sing on top + ms mid + DS bot all simultaneously achievable — the dominant oracle PBOT class). Blanket-PBOT rule pct_opt = 49.86% vs v52 baseline 31.95% (+18 pp) — the only cell where blanket-PBOT is right more often than wrong.

**Rule 19 design (codified in `analysis/scripts/strategy_v53_qpair_joint_pbot.py`):** if hand has exactly one pair AND pair_rank = Q (12) AND a JOINT PBOT-DS setting is achievable, play that JOINT setting (max non-pair singleton on top + 2 same-suit singletons in mid + 2 pair-suit-matching kickers in bot to complete DS). Tie-break (multiple JOINT configs): highest mid_high.

**Non-targeted regression sweep:** full pair-category sweep (2,800,512 hands × 78 (rank, cell) combos) confirmed rule fires ONLY on Q-pair × PBOT_DS_JOINT (28,512 hands). All other 77 combos: ZERO fires, $0 lift. Non-pair categories structurally excluded by pair gate. Prefix grader: v53 == v52 ($1,522/1000h, 53.06% pct_opt) — Q-pair JOINT canonical IDs concentrated outside prefix-500K.

**v53 grader-confirmed: $2,498 → $2,490 full grid (+$9/1000h), 43.34% → 43.43% pct_opt (+0.09pp), p90 0.720 → 0.715, p99 1.645 → 1.640.** Within pair: 51.6% → 51.8% pct_opt, $1,829 → $1,811 within-cat. All non-pair categories byte-identical. The harness-projected +$8.50 matched grader within 6%.

**Score: $2,490/1000h on full grid. Improvement: −$971 vs v16, −$1,175 vs v14.** Rule count 17 → 18. **First rule chain ship since v52 in S53 (six months wall-clock).** Two-track divergence v53 vs v44_dt: $1,417 → $1,409.

**Methodology lesson — catalog ceilings from per-category matrix docs OVERSTATE realizable rule lift.** The PAIR_DECISION_MATRIX.md "$391 WG catalog-shippable PBOT-cell headroom" was the v52→v44 gap, but v44 realizes it via selective ML gating. Only $9/1000h of that $391 is actually catalog-shippable at "one-sentence-statable" granularity (= Q-pair × PBOT_DS_JOINT only). **Future per-category matrix docs should treat ceiling estimates as capacity upper bounds, NOT realizable rule lift estimates; the realizable portion is typically <5% of capacity** (high_only's was <0%; pair's is 2.3% = $9/$391).

**Methodology lesson — selectivity is the catalog-shippable boundary.** When oracle's preference for a placement is <50%, a blanket-route rule will regress (the 30-77% wrong-fire population dominates). The Q-pair JOINT cell at 49.86% pct_opt is right at the threshold — the lone catalog-shippable PBOT-pair cell. Cells below this threshold are ML-only at catalog granularity. **The 50% rule_pct_opt boundary is now a usable filter for future per-cell candidate work.**

## Session 68: v54 hybrid chain SHIPS — LARGEST single production ship in project history (+$382/1000h full grid)

Per S67's falsification result + PAIR_DECISION_MATRIX.md Recommendation Path B, S68 built and shipped a cell-routed hybrid chain that captures the entire $382 WG pair-PBOT catalog gap by delegating to v44_dt on pair PBOT cells.

**Strategy v54 design.** Single binary routing gate: if hand is single-pair (exactly 1 pair, no trips/quads) AND pair-suits-distinct AND ≥1 non-pair singleton of each pair-suit, route to `strategy_v44_dt`. Else route to `strategy_v53_qpair_joint_pbot`. The gate is equivalent to "in the S66 6-cell pair taxonomy, cell ∈ {PBOT_DS_JOINT, PBOT_DS_PARTIAL}". PMID cells share the same fall-through target so the routing collapses to a binary check — no full taxonomy needed at inference time.

**v54 subsumes Rule 19 (S67) entirely.** Rule 19 inside v53 fires only on Q-pair × PBOT_DS_JOINT hands; v54 routes those hands to v44_dt before Rule 19 gets a chance. v44 catches ~+$13/cell in Q-pair JOINT vs Rule 19's +$8.50. Rule 19 remains in v53 as a fallback layer; the actual production chain is v54.

**Harness pre-validation (`validate_v54_routing_S68.py`)** swept all 2,800,512 pair canonical hands across 78 (rank, cell) combos with v54 as `rule_fn` and v53 as baseline. Predicted aggregate: **+$382.34/1000h** pair-cat lift (PBOT cells +$382.34; PMID cells $0.00 — zero spillover). Per-cell highlights: Q-pair × PBOT_DS_PARTIAL = +$41.20 (matches matrix-predicted ceiling exactly); K-pair total +$35.60 (matches C_PAIR_1 ceiling); Fire rate 27.7% of pair (= 12.9% of canonical grid; 775,915 hands).

**Grader confirmation (both grids).** Prefix: v53 $1,522 → v54 $1,343 = **+$179/1000h**; pct_opt 53.06% → 56.64% (+3.58pp); within-pair on prefix 60.6% → 68.9% (+8.3pp); all non-pair categories byte-identical. **Full grid: v53 $2,490 → v54 $2,108 = +$382/1000h**; pct_opt 43.43% → 49.74% (+6.31pp); within-pair full 51.8% → 65.3% (+13.5pp); within-pair $1,811 → $991 (−$820 within-cat); p90 0.715→0.640; p99 1.640→1.625. **Harness-to-grader fidelity: 0.1% error** (cleanest match in S60+ catalog/harness history).

**Score: $2,108/1000h on full grid. Improvement: −$1,353 vs v16, −$1,557 vs v14.** Two-track divergence v54 vs v44_dt: $1,409 → **$1,027** (closed 27% in a single session — largest single-session divergence reduction in project history).

**Records set in S68.**
- **Largest single production ship in project history**: +$382 (3× Rule 14's prior +$131 record at S50).
- **Largest pct_opt jump in production**: +6.31pp (exceeds v39_dt's +4.5pp at S54).
- **First Path-B-style hybrid ship** (rule chain + ML champion routed per-cell).
- **Largest single-session divergence reduction**: −$382/1000h between rule chain and ML champion.

**Per-category residual map (v54 framing).** high_only $755 WG (CLOSED S65), **pair $462 WG (CLOSED S68 — within $50 of v44-alone)**, two_pair $920 WG (S69 PRIMARY target), trips $110 WG, trips_pair $155 WG (collapsed S55a), three_pair $32 WG. With high_only and pair both addressed at their respective limits, the two largest categories (4M of 6M canonical hands = 67% of grid) are now both closed. Two_pair becomes the next absolute target.

**Methodology lesson — the catalog → matrix → falsification → hybrid arc is a replicable end-to-end methodology.** S66 (matrix doc) + S67 (candidate sweep, 12 candidates / 11 T3) + S68 (hybrid ship +$382) took 3 sessions to produce the project's largest ever ship. The arc is directly transferable to any per-category audit: produce a Phase 1+2 matrix → test blanket-rule candidates against thresholds → if blanket rules fail but matrix gap is large, ship a cell-routed hybrid that delegates to the ML champion.

**Methodology lesson — matrix predictions are accurate when the realization mechanism matches.** S66 matrix predicted $390 WG capture for Path B; S68 grader returned $382 (2% error). The same matrix's $391 "catalog-shippable" framing was wrong because the assumed mechanism (catalog blanket rules) couldn't realize it (S67 falsified 11 of 12 at the per-cell pct_opt<50% boundary). **Future matrix docs should distinguish "capacity" (the v52→v44 gap) from "realizable via mechanism X" — they can differ by orders of magnitude.**

**Methodology lesson — harness-to-grader fidelity at the routing layer is best-of-project.** Rule 19 (S67): 6% error. v54 (S68): 0.1% error. Routing-based predictions are simpler than rule-prediction (which has setting-builder noise). **Future hybrid-extension proposals can be pre-validated via the catalog harness to <1% error BEFORE committing to a multi-hour grader run** — saves grading time on dead-end candidates.

**Methodology lesson — records are set by clever architecture, not clever individual rules.** The +$382 ship is a routing wrapper, not a new rule. v53 + v44 are unchanged; the intelligence is in the binary gate. **This suggests the catalog approach's "$5-$50 per rule" ship size is a ceiling on rule-chain expressivity — bigger ships require architectural moves like Path B hybrid, ML retrain, or category-level rerouting.**

**End of S68:** Rule chain ADVANCES to v54 ($2,108 full / $1,343 prefix). ML champion UNCHANGED at v44_dt. **Pair catalog CLOSED. High_only catalog CLOSED.** S69 pivots to two_pair audit ($920 WG, largest remaining absolute residual) using the validated S66-S67-S68 methodology arc. Expected S69 outcome: TWO_PAIR_DECISION_MATRIX.md + candidate verdicts + potentially v55 = v54 + two_pair hybrid extension (if cells are similarly v44-favorable).

## Session 69: v55 two_pair hybrid SHIPS — NEW LARGEST single production ship in project history (+$634/1000h full grid; 1.66× S68 record)

Per S68's CURRENT_PHASE Recommendation, S69 audited two_pair using the validated S66-S67-S68 methodology arc. The matrix immediately revealed two_pair as a structurally PURER ML-only category than pair (no PMID-cell counter-headroom), so the hybrid extension simplifies to a category-blanket. The matrix → candidate sweep → hybrid ship arc completed in ONE session by reusing all S66-S68 templates.

**Strategy v55 design.** Single binary routing gate, even simpler than v54's pair gate: if hand is two_pair (n_pairs == 2 AND no_trips AND no_quads), route to `strategy_v44_dt`. Else route to `strategy_v54_pair_hybrid` (which preserves all v54 pair-routing behavior and falls through to v53/v52 for everything else). The two gates are structurally disjoint; v55 stacks atop v54 cleanly with zero conflict risk.

**Phase 1 — `drill_two_pair_v44_S69.py` (matrix Phase 1).** Built two_pair-specific 7-cell structural taxonomy informed by v44's existing `t2p_v2_*` Layout-B/C feature family from S55. Cells: LAYOUT_A_DS (HH+LL pair-suits FULLY match → bot is "two pair DS"), LAYOUT_C_DS (Hbot+Lmid+DS), LAYOUT_B_DS (Lbot+Hmid+DS), LAYOUT_A_SS (HH and LL share 1 suit), LAYOUT_C_SS_ONLY, LAYOUT_B_SS_ONLY, LAYOUT_OTHER (empirically EMPTY). Sweep on all 1,338,480 canonical two_pair hands: v44 pct_opt = 82.84% (highest of any non-degenerate category); v44 WG residual = $80.82 (canonical-equal framing).

**Phase 2 — `sweep_v54_on_two_pair_S69.py` + TWO_PAIR_DECISION_MATRIX.md.** v54 (= v52 on two_pair, since both v54's pair-hybrid gate and v53's Rule 19 are single-pair-only) WG residual = $715.29 — leaks **$634/1000h MORE than v44** in canonical-equal framing. Per-cell breakdown: every cell favors v44 by $7-261 WG; **NO PMID-style counter-headroom anywhere**. This is qualitatively cleaner than pair (where PMID cells favored v52 by $50 WG) — for two_pair the entire category is uniformly v44-better. Per-rank concentration: top 4 hi_pair_ranks (A, K, Q, J) account for 69% of the gap. Dominant mismatch class is the same `B_SS ↔ C_SS` Hbot/Hmid layout confusion that S55 surfaced.

**Phase 3a — Catalog candidate sweep (`test_two_pair_catalog_candidates_S69.py`).** Tested 5 catalog candidates per S69 acceptance criteria. **3 of 5 cleared T2 vs v54 baseline** (C_T2P_1/K +$30 WG, C_T2P_1/A +$60 WG, C_T2P_2/Q +$13 WG) — but **EVERY candidate lost to v44 within-cell** (capture vs_v44 negative everywhere). The Path B hybrid captures STRICTLY MORE than any catalog rule. Candidate ships would be subsumed by v55. **Key methodology lesson: a T2 verdict against the rule baseline is no longer sufficient evidence to ship a rule when the hybrid option exists.** Always also test "lift vs v44" — if negative, the candidate is dominated.

**Phase 3b — `validate_v55_routing_S69.py` + `grade_v55_two_pair_hybrid.py`.** Pre-grader validation reused existing v44 + v54 sweep parquet (no new computation): predicted +$634.47/1000h FULL GRID. Grader confirmed: **v54 $2,108 → v55 $1,473 = +$634/1000h** (full grid, n=200) and **+$516/1000h** (prefix grid, n=1000). Within-two_pair full pct_opt 44.1% → 83.2% (+39.1pp); within-two_pair $3,211 → $363 (-$2,848). All non-two_pair categories byte-identical to v54 on both grids. p90 0.640 → 0.505; p99 1.625 → 1.160. **Harness-to-grader fidelity: 0.07% error** — best in project history (vs S68 v54: 0.1%; S67 Rule 19: 6%).

**Score: $1,473/1000h on full grid. Improvement: −$1,988 vs v16, −$2,191 vs v14.** Two-track divergence v55 vs v44_dt: $1,027 → **$393** (closed 62% in a single session — largest single-session divergence reduction in project history). Cumulative S68+S69 closed $1,016/1000h = 72% of original $1,409 divergence.

**Records set in S69.**
- **NEW LARGEST single production ship in project history**: +$634 (1.66× S68's prior +$382; 4.8× Rule 14's S50 +$131).
- **NEW LARGEST pct_opt jump in production**: +8.70pp (exceeds S68's +6.31pp).
- **Cleanest harness-to-grader fidelity ever**: 0.07% error.
...
**Per-category residual map (v55 framing).** high_only $755 WG (CLOSED S65, ML-only), pair $462 WG (CLOSED S68 — within $50 of v44-alone via PMID preservation), **two_pair $80.82 WG (CLOSED S69 — EQUALS v44 since gate is total)**, trips $110 WG (S70 PRIMARY target), trips_pair $155 WG (collapsed S55a), three_pair $32 WG (small target). With the THREE largest categories now all addressed (5.36M of 6.01M canonical hands = 89% of grid), the remaining residual is concentrated in trips ($110 WG) + three_pair ($32 WG) + minor cells.

**Methodology lesson — the Path-B hybrid arc compresses dramatically with reuse.** Pair took 3 sessions (S66 matrix + S67 candidates + S68 ship); two_pair took 1 session (S69 matrix + candidates + ship) by reusing the S66 cell-taxonomy template, S68 hybrid-routing template, and S68 grader template. Each Phase 1+2 sweep took <10 min; Phase 3a candidate sweep <2 min; Phase 3b grader confirmation ~30 min total. **Future per-category audits should compress further still — trips is structurally simpler than two_pair (1 trip rank + 4 singletons, no pair-vs-no-pair confusion).**

**Methodology lesson — catalog T2 verdicts vs the rule baseline are NOT enough to ship when the hybrid option exists.** S69 Phase 3a found 3 T2 candidates — but ALL lose to v44 ceiling. Always test "lift vs v44" alongside "lift vs baseline" as part of the threshold evaluation. **A new "T2-hybrid-dominated" verdict category (T2 vs baseline + negative vs v44) is now the implicit failure mode for the catalog approach in any category where the hybrid is viable.**

**Methodology lesson — cleaner gates ship cleaner.** v54's pair gate required suit-pattern checks (single-pair AND distinct-suits AND singleton-suit-coverage). v55's two_pair gate needs only structural-category check (n_pairs == 2 AND no_trips AND no_quads). The simpler gate ships at higher harness-grader fidelity (0.07% vs 0.1%). **Suggests future hybrid extensions should prefer category-blanket routing over multi-condition gates when the per-cell data supports it.**

**Methodology lesson — architectural moves dwarf rule-tuning ships.** S68 (+$382) and S69 (+$634) were both Path-B routing wrappers, not new rules. Compare to the S43-S67 era's max single rule ship of +$131 (Rule 14, S50). Average rule ship across S43-S67: $5-$50. **The remaining production headroom is dominated by architecture (routing, delegation, hybrid extensions), not by individual rules.**

**Methodology lesson — two_pair was structurally PURER ML-only than pair.** Pair had PMID-cell counter-headroom requiring v54 to *preserve* PMID routing to v53 ($50 WG anti-headroom). Two_pair has NO counter-headroom — every cell favors v44. The hybrid simplifies to "blanket: route the whole category to v44". This may also generalize to future categories with simpler structural axes (trips).

**Methodology lesson — records can compound across sessions when architecture matches.** S68 set the +$382 record; S69 broke it 1 session later at +$634. Both are Path-B hybrids of the same architectural family. **This suggests trips, if also a Path-B candidate, could ship at +$50-110 WG full-grid in S70 — establishing a 3rd consecutive Path-B record-extending ship.**

**End of S69:** Rule chain ADVANCES to v55 ($1,473 full / $827 prefix). ML champion UNCHANGED at v44_dt. **Two_pair catalog CLOSED. Pair catalog CLOSED. High_only catalog CLOSED.** S70 pivots to trips audit ($110 WG, largest remaining absolute residual) using the validated S66-S69 methodology arc — expected to compress further than two_pair did. Expected S70 outcome: TRIPS_DECISION_MATRIX.md + candidate verdicts + potentially v56 = v55 + trips hybrid extension (if cells are similarly v44-favorable). Alternative S70 path: ML retrain (v45_dt+) to reduce v44's residuals further — given v55 routes ~35% of grid through v44 (pair PBOT 12.9% + two_pair 22.3%), any v44 improvement compounds via v55.

## Session 70: v56 trips hybrid SHIPS — third consecutive Path-B hybrid ship; closes trips category (+$45/1000h full grid)

**The story:** S69 closed two_pair with v55 (+$634). CURRENT_PHASE end-S69 mandated trips audit using the validated S66-S69 methodology arc. S70 executed Phase 1 (drill_trips_v44_S70.py — structural cell sweep), Phase 2 (sweep_v55_on_trips_S70.py + TRIPS_DECISION_MATRIX.md), Phase 3a (catalog candidate sweep), Phase 3b (v56 build + grade) all in a single session by reusing the S69 architecture templates.
...
### Per-cell matrix prediction vs grader

`sweep_v55_on_trips_S70.py` swept all 328,185 trips canonical hands. Harness predicted v56's grader lift from existing v44 + v55 sweep parquets without any new computation:
...
| B_DS_AVAIL_HKR (n=62,055) | $0.84 (near-tie) |
| **TOTAL (after $0.76 counter-headroom forfeit)** | **$44.61** ← matches grader $45 to 0.87% |

Per-rank: trips=Q peak ($5.66 lift), declining roughly proportional to v55-v44 gap. Counter-headroom of $0.76 WG (B_DS_AVAIL_HKR at ranks {K,J,T,4,3}) forfeited in the blanket; net $44.61 already accounts for it.
...
| Candidate | Cell | lift_wg vs v55 | lift_wg vs v44 | rule pct_opt | v44 pct_opt | verdict |
|---|---|---:|---:|---:|---:|---|
| C_TR_1 / B_DS_AVAIL_HKR (Force A_DS_tkmax) | HKR | −$1.32 | −$2.16 | 47.0% | 49.6% | T3 |
| C_TR_1 / B_DS_AVAIL_LKR | LKR | −$9.59 | −$35.09 | 35.95% | 50.75% | T3 |
| C_TR_2 / NO_BDS_AKDOM (Force A_DS_tkmax) | AKDOM | −$0.11 | −$15.69 | 38.77% | 76.35% | T3 |
| C_TR_3 / NO_BDS_CTOP (Force C_top_trip) | CTOP | $0.00 | −$2.70 | 37.32% | 64.66% | T3 |

**0 of 4 candidates clear T1 vs v55 baseline — 3 of 4 actually LOSE to the baseline.** Even MORE decisive than S69's pattern. The structural rule space is GENUINELY empty at the catalog level for trips. **The within-Layout-A bot-suit selectivity that v44 performs is purely the ML-residual that crude deterministic rules cannot replicate.**
...
In 3 consecutive sessions, the project closed 75% of the original two-track divergence. **The architectural-routing headroom is largely exhausted.**

### Methodology lessons

1. **The Path-B hybrid arc generalizes to small categories with consistent fidelity.** S68 (pair $382) → S69 (two_pair $634) → S70 (trips $45). Cumulative $1,061. Each ship's harness-to-grader fidelity within reliable range (0.1% / 0.07% / 0.87%).
2. **Catalog candidates can be MORE dominated as v44 gets selective on smaller categories.** S69: 3/5 cleared T2 vs baseline. S70: 0/4 cleared T1 vs baseline (3/4 LOSE to baseline). The catalog-vs-hybrid dominance gap WIDENS for categories where v44's selectivity matters more.
3. **Cleaner gates ship cleaner; three-gate hybrid chain is the project's cleanest architecture.** Pair-PBOT (v54) + two_pair (v55) + trips (v56) structurally disjoint at rank-count signature. Zero conflict.
...
## Session 87: v61 SHIPS Rule 21 (v47/v52 chain gate-out on HIGH_ONLY × DS_NO_JOINT × {J-A}) — LARGEST single-rule production ship since S70 (+$98.67/1000h full grid)

**The story:** User reframed the strategic priority at start of session: "easy hands are easy to play — bleeding lives on weak hands where damage control is hard." (S87 user verbatim: "10-20 trash hands in a row, that's real bleeding.") Empirical look at S71 HIGH_ONLY drill data confirmed the framing — HIGH_ONLY × DS_NO_JOINT × {J-A} cells leak $76-125/1000h per cell vs v44, dwarfing the $6.43 MID-pair candidate from S86. But methodology: ALL HIGH_ONLY cells are prefix-silent (canonical_id ≥ 590K, prefix ends at 499K). User picked Plan A from a gated-pivot proposal — pre-drill addressability first, then commit.

**The bombshell:** The pre-drill ran v57 on the 756,000 J/Q/K/A × DS_NO_JOINT hands and revealed that **v57 leaks $98.67/1000h MORE than v44_dt would on this cell.** v52's HIGH_ONLY handler had been actively bleeding EV against the realistic-mixture opponent for 33+ sessions and was never caught because prefix grader is structurally silent for these cells. The layer-by-layer audit pinpointed v47 (Rules 13-16, Q-high DS chain) as the dominant bleed source ($82/1000h on the fallthrough subset), with v52's J-HIMID handler contributing $12.76 and v52-defensive-gated adding $3.84.

**The fix (Rule 21):** Surgical gate-out. v61 detects HIGH_ONLY × DS_NO_JOINT × max ∈ {J,Q,K,A} and returns strategy_v44_dt directly, bypassing the v47→v48→v52 chain on that 756,000-hand sub-population. Outside the gate v61 == v57 by construction.

**Full-grid grade — auto-fired SHIP:** Whole-grid lift **+$98.67/1000h** (cell hands: 756,000; total EV lift: 59,294 hand-EV units). Per-rank: J +$13.59 / Q +$19.24 / K +$28.95 / A +$36.89. Swap-right rate on changed hands: 62.3% (320,680 better / 194,439 worse / 240,881 same). Out-of-cell sanity: 0 v57≠v61 disagreements on 50K-hand random sample.

### Rule 21 design

Trigger (all must be true):
* HIGH_ONLY — no rank appears ≥2 times (no pair, trips, quads)
* max_rank ∈ {11,12,13,14} (J, Q, K, A)
* DS_NO_JOINT cell — at least one 4-singleton subset forms 2+2 suit pattern (n_DS > 0) AND at least one of those configurations leaves max card available for top (n_DS_with_max_top > 0) AND none of those configurations have a suit-matched middle pair (n_joint == 0)

Setting (when triggered):
* Whatever strategy_v44_dt picks. v61 is a "stop the bleed" rule — it removes a NET-NEGATIVE override, not adds a new positive setting. Future v62 could add value-extraction on top of v44_dt for these cells.

Plain-language: "Unpaired hand with a J-A as the top card, where a double-suit pattern exists in the singletons but not in a configuration that lets max-on-top combine with a suit-matched mid — let the ML model handle these. The chain of human-designed defensive rules (v47 Q-high DS, v52 J-HIMID) was actively making these hands worse than the ML baseline."
...
|---|---:|---:|
| Pre-S68 | $1,409 | baseline |
| Post-S70 (v56) | $348 | end of Path-B hybrid run |
| Post-S83 (v57) | $332 | −$16 (Rule 20) |
| S84-S86 | $332 | $0 (1 SHIP / 2 MIXED / 1 MIXED-by-methodology) |
| **Post-S87 (v61)** | **$234** | **−$98 (Rule 21 — biggest single-rule closure since v55)** |

Cumulative closure since pre-S68: **$1,175 of $1,409 = 83%** (was 76%).

**Score: $1,511.20/1000h on full grid, $776.88 on prefix (unchanged — rule fires entirely outside prefix coverage). Improvement: −$2,051 vs v8 baseline, −$99 vs v57.** Rule count: 20 → **21** (Rule 21 added).

**Methodology lesson #1 — prefix-silent cells can hide PRODUCTION REGRESSIONS, not just unship-able candidates.** The S86 prefix-coverage debate framed prefix-silence as an obstacle to SHIPPING new rules. S87 surfaced the inverse: rules ALREADY in production on prefix-silent cells can carry undetected bleed. The audit pattern (per-version layer attribution against v44_dt baseline) should be applied to every prefix-silent zone where the rule chain has overrides. There are likely more buried regressions — DS_NO_MAXTOP, MS_ONLY cells are next-largest candidates.

**Methodology lesson #2 — pre-committed thresholds plus aggregate-LLN arguments combine to ship $98 lifts without N=1000 validation.** The bleed is so large (LLN noise floor on 756K hands ≪ $1) that prefix validation is statistically unnecessary, even though structurally unavailable. This complements the gemini-3-pro argument from the S87 round-1 Socratic debate: N=200 aggregate uncertainty is tiny when the effect size dwarfs noise. The S84 two-grid SHIP standard remains correct as a default; the v61 ship is a justified exception under "effect size >> noise floor by orders of magnitude."

**Methodology lesson #3 — the user's strategic instinct as primary signal.** S86 ended on a methodology question. The user's response was not "answer the methodology question" but "step back — the bigger question is where the BLEEDING is." That reframe led directly to the audit and the $98 fix. Future sessions should preserve room for the user to redirect strategy at the framing level, not just at the methodology level.

**End of S87:** Rule chain ADVANCES to **v61_high_only_ds_no_joint_fix** ($1,511.20 full / $776.88 prefix unchanged). ML champion UNCHANGED at v44_dt ($1,081 full / $686 prefix). Rule count: **21.** v60 candidate from S86 remains DEFERRED (Option C N=1000 oracle infrastructure not built this session — deferred to S88 as focused build). S88 default: expand the audit to DS_NO_MAXTOP, MS_ONLY, and other prefix-silent HIGH_ONLY zones to find additional buried regressions.

---

## Session 88: v62 SHIPS Rule 22 (chain gate-out extension to DS_NO_MAXTOP + MS_ONLY + JOINT_HIGH) — back-to-back $98+ production ships; v47 confirmed net-negative across the entire HIGH_ONLY × {J-A} weak-hand zone (+$98.84/1000h full grid)

**The story:** S87's chain-audit pattern handed S88 a clear default — expand the audit to the next-largest prefix-silent HIGH_ONLY cells. Three target cells (all 100% prefix-silent, canonical_id ≥ 590,502): HIGH_ONLY × DS_NO_MAXTOP × {K,A} (133K hands), MS_ONLY × {J-A} (108K hands), JOINT_HIGH × {K,A} (117K hands). Total: 357,504 hands carrying $95.62/1000h v44_dt baseline leak.

**The bombshell (replay).** Pre-drill (50s compute) ran v61 (current production) on the 357K hands. **v61 leaks $98.84/1000h MORE than v44_dt on this cell sub-population** — virtually identical magnitude to S87's $98.67/1000h finding on DS_NO_JOINT. Same architectural pattern: the rule chain is actively making things worse than the ML baseline.

**The chain audit (74s compute).** Layer-by-layer attribution against v44_dt baseline:
- **v44→v47 = +$99.55/1000h (99.7% of bleed)**
- v47→v48 = -$0.82 (slightly helpful on MS_ONLY × J)
- v48→v52 = +$0.10 (zero)

99.7% from v44→v47 transition. **Same culprit as S87 (Rules 13-16, the Q-high DS chain).** v52 is essentially neutral on top of v47 on these cells.
...
**The fix (Rule 22):** Strict superset of v61's gate. v62 detects HIGH_ONLY × max ∈ {J,Q,K,A} × cell ∈ {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH} and returns strategy_v44_dt directly, bypassing the v47→v48→v52 chain. Outside the gate v62 == v61 == v57 by construction.

**Full-grid grade — auto-fired SHIP:** Whole-grid lift **+$98.84/1000h** (cell hands: 357,504; total EV lift: 59,395 hand-EV units). Per cell: DS_NO_MAXTOP +$52.88 / MS_ONLY +$31.51 / JOINT_HIGH +$14.45. Swap-right rate on changed hands: **76.8%** (188,810 better / 56,952 worse / 111,742 same — better than S87's 62.3%). Out-of-gate sanity: **0 v62≠v61 disagreements** on 50K-hand random sample.
...
* HIGH_ONLY — no rank appears ≥2 times (no pair, trips, quads)
* max_rank ∈ {11,12,13,14} (J, Q, K, A)
* cell ∈ {DS_NO_JOINT (S87), DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH} per the S71 cell taxonomy

Setting (when triggered):
* Whatever strategy_v44_dt picks. v62 is a "stop the bleed" rule — it removes a NET-NEGATIVE override, not adds a new positive setting. Future Rule 23 could add value-extraction on top of v44_dt for these cells; that requires Option C N=1000 oracle infrastructure (still deferred).

Plain-language: "Unpaired hand with a J-A as the top card, where the bottom-4 has SOME double-suit-ish structure but doesn't cleanly support max-on-top + suit-matched-mid — let the ML model handle it. The v47 Q-high DS chain has been making these hands WORSE than the ML baseline, not better."

### Cumulative two-track divergence closure (project = remaining gap to oracle)

| Session | rule-chain vs oracle gap | Δ this session |
|---|---:|---:|
| Pre-S68 | $1,409 | baseline |
| Post-S70 (v56) | $348 | end of Path-B hybrid run |
| Post-S83 (v57) | $332 | −$16 (Rule 20) |
| S84-S86 | $332 | $0 (1 SHIP / 2 MIXED / 1 MIXED-by-methodology) |
| Post-S87 (v61) | $234 | −$98 (Rule 21 — DS_NO_JOINT chain gate-out) |
| **Post-S88 (v62)** | **$135** | **−$99 (Rule 22 — chain gate-out extension)** |

Cumulative closure since pre-S68: **$1,273.84 of $1,409 = 90.4%** (was 83%).

**Score: $1,610.04/1000h on full grid, $776.88 on prefix (unchanged — rule fires entirely outside prefix coverage). Improvement: −$1,952 vs v8 baseline, −$99 vs v61.** Rule count: 21 → **22**.

**Methodology lesson #1 — the chain-audit pattern transfers across cells without modification.** S87's scripts (`drill_v57_high_only_addressability_S87.py`, `audit_v52_chain_bleed_S87.py`, `grade_v61_full_grid_S87.py`) became S88's three scripts via 1:1 templating: rebind target cells, point at v61 as the baseline instead of v57, keep everything else. Audit-pattern infrastructure cost is near-zero per cell × ranks group.

**Methodology lesson #2 — v47 is net-negative across the ENTIRE HIGH_ONLY × {J-A} weak-hand zone, not just DS_NO_JOINT.** Combined S87+S88 chain bleed: $98.67 + $98.84 = **$197.51/1000h recovered by gating out v47** on prefix-silent HIGH_ONLY × J-A cells. The original design intent of v47 (Rules 13-16, Q-high DS chain, S52) was "offensive value-add for high-card hands." Empirically, after 33+ sessions in production, it has been bleeding EV across the whole zone. The bleed went undetected because prefix grader is structurally blind to HIGH_ONLY hands (canonical_id ≥ 590K; prefix ends at 499K).

**Methodology lesson #3 — the EFFECT-SIZE-DOMINANCE rule replicates cleanly.** S87 shipped under the prefix-silent exception at $98.67. S88 ships at $98.84 under the same exception. Criterion is calibrated: effect ≫ noise floor by 20×+ AND mechanism is gate-out (not addition) → bypass two-grid standard with documentation.

**Methodology lesson #4 — the chain-audit lever now dominates value-extraction.** Two $98 ships in two sessions, both from the same architectural diagnosis (v47 net-negative on a prefix-silent cell). Combined recovered: $197.51/1000h. Compare to all rule-extraction ships from S71-S86: Rule 20 ($16.81 prefix) was the only SHIP across that 16-session window. The audit pattern has shipped 12× the cumulative value of incremental rule extraction.

**End of S88:** Rule chain ADVANCES to **v62_high_only_chain_fix** ($1,610.04 full / $776.88 prefix unchanged). ML champion UNCHANGED at v44_dt ($1,081 full / $686 prefix). Rule count: **22.** Audit candidates remaining for S89+: JOINT_MED + JOINT_LOW + NEITHER × {J-A}; HIGH_ONLY × max ∈ {7-T} (different firing mode); prefix-COVERED cells (LOW pair / two_pair / trips). v60 candidate from S86 remains DEFERRED — still waits on Option C N=1000 oracle infrastructure.

## Session 89: v63 SHIPS Rule 23 (chain gate-out closure on JOINT_MED + JOINT_LOW × {J-A}) — third consecutive chain-audit ship; HIGH_ONLY × {J-A} audit zone CLOSED (+$10.09/1000h full grid)

**The story:** S88's chain-audit pattern handed S89 a clear default — expand the audit to the remaining prefix-silent HIGH_ONLY × {J-A} cells (JOINT_MED, JOINT_LOW, NEITHER). Phase A query of S71 stats produced two surprises:

1. **NEITHER × {J-A} is structurally empty.** Combinatorial proof: NEITHER requires both n_DS = 0 AND n_ms_mid_with_max_top = 0. The latter requires "no two non-max cards share a suit." A HIGH_ONLY hand has 7 distinct-rank cards → 6 non-max cards. By pigeonhole on 4 suits, ≥ 2 must share a suit → cell ≠ NEITHER. So NEITHER × HIGH_ONLY is impossible for any max-rank.
2. **JOINT_MED + JOINT_LOW × {J-A} combined is 48,132 hands** — ~14× smaller than S87 and ~7× smaller than S88. The recoverable ceiling is correspondingly smaller; the resume prompt pre-committed a $5 SHIP threshold (vs S87/S88's $30) for exactly this reason.

NEITHER dropped from S89 target set with no audit needed (free win from a 5-minute structural check).

**The pre-drill (6s compute, 48K hands).** Re-evaluated v62 (production) on the target cells:

| cell | n | v44 leak $ | v62 leak $ | **Δ (chain bleed)** |
|---|---:|---:|---:|---:|
| JOINT_MED | 44,562 | $6.23 | $14.47 | **+$8.24** |
| JOINT_LOW | 3,570 | $0.13 | $1.98 | **+$1.85** |
| **TOTAL** | **48,132** | **$6.37** | **$16.45** | **+$10.09** |

**v62 leaks +$10.09/1000h MORE than v44_dt** on the 48K target hands. Smaller magnitude than S87/S88 (cells ~14× and ~7× smaller respectively) but mechanically identical. Override activity: 46-56% on JOINT_MED, 72-93% on JOINT_LOW.

**The chain audit (9s compute).** Layer-by-layer attribution against v44_dt baseline:
- **v44→v47 = +$9.68/1000h (96% of bleed)**
- v47→v48 = -$0.44 (slight improvement on MS_ONLY × J behavior leaking into audit)
- v48→v52 = +$0.85 (small v52-defensive-gated + v52-J-HIMID bleed)

**Third consecutive session with v47 (Rules 13-16, Q-high DS chain) as the dominant culprit.**
...
**The fix (Rule 23):** Strict superset of v62's gate. v63 detects HIGH_ONLY × max ∈ {J,Q,K,A} × cell ∈ {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH, JOINT_MED, JOINT_LOW} and returns strategy_v44_dt directly, bypassing the v47→v48→v52 chain. Outside the gate v63 == v62 == v57 by construction. v63 effectively gates **the entire HIGH_ONLY × {J-A} zone** because NEITHER × {J-A} is structurally empty.

**Full-grid grade — auto-fired SHIP:** Whole-grid lift **+$10.09/1000h** (cell hands: 48,132; total EV lift: 6,060 hand-EV units). Per cell: JOINT_MED +$8.24 / JOINT_LOW +$1.85. Per-hand effect: 48.8% same, 43.6% better, 7.6% worse. **Swap-right rate on changed hands: 85.2%** — HIGHEST across S87/S88/S89 (S87: 62.3%, S88: 76.8%, S89: 85.2%). Out-of-gate sanity: **0 v63≠v62 disagreements** on 50K-hand random sample.
...
* HIGH_ONLY — no rank appears ≥2 times (no pair, trips, quads)
* max_rank ∈ {11,12,13,14} (J, Q, K, A)
* cell ∈ {DS_NO_JOINT (S87), DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH (S88), JOINT_MED, JOINT_LOW (S89)} per the S71 cell taxonomy — i.e., ALL 6 non-empty HIGH_ONLY × {J-A} cells

Setting (when triggered):
* Whatever strategy_v44_dt picks. v63 is a "stop the bleed" rule — it removes a NET-NEGATIVE override, not adds a new positive setting. Future Rule 24 could add value-extraction on top of v44_dt for these cells; that requires Option C N=1000 oracle infrastructure (still deferred).

Plain-language: "Unpaired hand with a J-A as the top card — let the ML model handle it. The v47 Q-high DS chain has been making these hands WORSE than the ML baseline across the entire HIGH_ONLY × {J-A} zone, not just specific cells. With Rule 23, the gate-out is now complete for this zone."
...
| Post-S87 (v61) | $234 | −$98 (Rule 21 — DS_NO_JOINT chain gate-out) |
| Post-S88 (v62) | $135 | −$99 (Rule 22 — chain gate-out extension) |
| **Post-S89 (v63)** | **$125** | **−$10 (Rule 23 — chain gate-out CLOSURE)** |

Cumulative closure since pre-S68: **$1,283.93 of $1,409 = 91.1%** (was 90.4%).

**Score: $1,620.13/1000h on full grid, $776.88 on prefix (unchanged — rule fires entirely outside prefix coverage). Improvement: −$1,963 vs v8 baseline, −$10 vs v62.** Rule count: 22 → **23**.

**Methodology lesson #1 — the chain-audit pattern transferred 1:1 for a THIRD consecutive session.** S87 → S88 → S89: identical script structure, rebound parameters. Three sessions of audit work, three ships from the same diagnosis. Infrastructure cost is empirically near-zero per cell.

**Methodology lesson #2 — the EFFECT-SIZE-DOMINANCE rule generalizes across scales.** S87 ($98.67), S88 ($98.84), S89 ($10.09): three orders of magnitude apart, all three pass the "effect ≫ noise floor by 20×+" criterion. The per-session SHIP threshold ($30, $30, $5) is a per-cell calibration; the noise-floor multiple is what generalizes.

**Methodology lesson #3 — pre-Phase A structural feasibility check can close target cells for free.** NEITHER × {J-A} eliminated by a 5-minute combinatorial proof. Apply this check before any future chain-audit pivot to a new cell type.

**Methodology lesson #4 — per-hand swap-right rate is a useful secondary verdict signal.** S87 → S88 → S89 = 62.3% → 76.8% → 85.2%. Smaller, more uniform cells contain hands where v47's bias is more consistently wrong. Future audits should track this metric alongside whole-grid lift.

**Methodology lesson #5 — HIGH_ONLY × {J-A} audit closure is a methodology milestone.** Three sessions of chain-audit work converged on a defined and bounded zone. Within HIGH_ONLY × {J-A}, the chain-audit lever is now exhausted. S90+ must find new audit zones — HIGH_ONLY × max ≤ T (different firing mode, unknown bleed status), or prefix-COVERED cells.

**End of S89:** Rule chain ADVANCES to **v63_high_only_chain_fix_full** ($1,620.13 full / $776.88 prefix unchanged). ML champion UNCHANGED at v44_dt ($1,081 full / $686 prefix). Rule count: **23.** HIGH_ONLY × {J-A} chain-audit zone CLOSED. Audit candidates remaining for S90+: HIGH_ONLY × max ∈ {7-T} (different firing mode, v52-defensive-low rather than v52-fallthrough); prefix-COVERED cells (LOW pair / two_pair / trips). v60 candidate from S86 remains DEFERRED — still waits on Option C N=1000 oracle infrastructure.

## Session 90: v64 SHIPS Rule 24 (chain gate-out extension to HIGH_ONLY × max ∈ {8, 9, T}) — fourth consecutive chain-audit ship; structurally-non-empty HIGH_ONLY × max ≥ 8 audit COMPLETE; first project-level audit of v52-defensive-low (+$7.23/1000h full grid)

**The story:** S89's plan handed S90 the PRIMARY path — pivot to HIGH_ONLY × max ∈ {7-T}, a DIFFERENT firing mode (v52-defensive-low rather than v52-fallthrough that drove S87-S89). The open question: does v52-defensive-low, designed in S53 specifically for weak-high HIGH_ONLY hands, carry the same v47 chain bleed as the other v52 modes? Phase A produced surprises:

1. **HIGH_ONLY × max = 7 is structurally empty.** Combinatorial proof: HIGH_ONLY requires 7 distinct ranks, but only 6 ranks ≤ 7 exist (2-7). Aces always counted as 14 in the cell taxonomy, so wheel-style hands don't appear here.
2. **HIGH_ONLY × max ∈ {8, 9, T} × JOINT_HIGH is structurally empty.** JOINT_HIGH needs `best_ms_mid_high ≥ J=11`; mid cards have rank < max ≤ T = 10, so impossible.
3. **HIGH_ONLY × max ∈ {8, 9, T} × NEITHER is structurally empty.** Same pigeonhole argument as NEITHER × {J-A}.
4. **HIGH_ONLY × max=8 × JOINT_MED is structurally empty.** JOINT_MED needs `best_ms_mid_high ≥ 8`; mid cards have rank < max = 8.

After these four cell eliminations: 11 non-empty (cell × rank) combinations totaling **25,740 hands** across DS_NO_JOINT × {8,9,T} (16,200), DS_NO_MAXTOP × {8,9,T} (3,456), JOINT_MED × {9,T} (3,150), MS_ONLY × {8,9,T} (2,304), JOINT_LOW × {8,9,T} (630). All prefix-silent (cid_min ≥ 590,496).

**The pre-drill (3s compute, 25.7K hands).** Re-evaluated v63 (production) on the target cells:
...
**v63 leaks +$7.23/1000h MORE than v44_dt.** Hypothesis P1 (v52-defensive-low is well-designed → clean null) REJECTED. Hypothesis P2 (chain extends to max ≤ T) CONFIRMED. Override activity 68-89%.

**The chain audit (7s compute).** Layer-by-layer attribution against v44_dt:
- **v44→v47 = +$19.28/1000h** (4th consecutive session with v47 as the dominant regression introducer)
- v47→v48 = -$2.53 (small improvement)
- v48→v52 = **-$9.52** (v52-defensive-low partially recovers ~50% of v47's bleed but doesn't fully restore v44)

By v52 firing mode: **100% of 25,740 hands fire v52-defensive-low** (LOW_MAX_DEFENSIVE = {7, 8, 9, 10}). **This is the first project-level audit of v52-defensive-low's behavior.** Verdict: it is **partially effective** — the S53 design intent was correct (it recovers $9.52 of v47's $19.28 bleed) but the implementation leaves money on the table relative to ML.

**The fix (Rule 24):** Strict superset of v63's gate. v64 detects HIGH_ONLY × max ∈ {8,9,T,J,Q,K,A} × cell ∈ {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH, JOINT_MED, JOINT_LOW} and returns strategy_v44_dt directly, bypassing the v47→v48→v52 chain. v64 effectively gates **the entire structurally-non-empty HIGH_ONLY × max ≥ 8 zone**. (HIGH_ONLY × max = 7 is structurally empty by combinatorial necessity; no further chain-audit work remains in HIGH_ONLY.)

**Full-grid grade — auto-fired SHIP:** Whole-grid lift **+$7.23/1000h** (cell hands: 25,740). Per cell breakdown matched pre-drill prediction to $0.01 (high aggregate stability). Per-hand effect: 21.6% same, 55.2% better, 23.2% worse. **Swap-right rate on changed hands: 70.4%** — LOWER than S88/S89 (76.8%/85.2%) but higher than S87 (62.3%). The elevated 23.2% "worse" rate is the signature of v52-defensive-low's partial effectiveness — it actively wins on ~23% of these hands. Out-of-gate sanity: **0 v64≠v63 disagreements** on 50K-hand random sample.

### Rule 24 design

Trigger (all must be true):
* HIGH_ONLY — no rank appears ≥2 times (no pair, trips, quads)
* max_rank ∈ {8,9,10,11,12,13,14} (8, 9, T, J, Q, K, A)
* cell ∈ {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH (S88), JOINT_MED, JOINT_LOW} per the S71 cell taxonomy — i.e., ALL 6 non-empty HIGH_ONLY cell types

Setting (when triggered):
* Whatever strategy_v44_dt picks. Same "stop the bleed" mechanism as Rules 21+22+23 — removes a NET-NEGATIVE override, doesn't add a new positive setting.

Plain-language: "Unpaired hand with any high card 8-A — let the ML model handle it. The v47 Q-high DS chain has been making these hands WORSE than the ML baseline across the entire structurally-non-empty HIGH_ONLY zone, not just the J-A subset. v52-defensive-low (the S53 weak-high handler) catches about half of the bleed but doesn't go far enough."
...
| S84-S86 | $332 | $0 (1 SHIP / 2 MIXED / 1 MIXED-by-methodology) |
| Post-S87 (v61) | $234 | −$98 (Rule 21 — DS_NO_JOINT chain gate-out) |
| Post-S88 (v62) | $135 | −$99 (Rule 22 — chain gate-out extension {J-A}) |
| Post-S89 (v63) | $125 | −$10 (Rule 23 — chain gate-out CLOSURE {J-A}) |
| **Post-S90 (v64)** | **$117.84** | **−$7.23 (Rule 24 — chain gate-out EXTENSION to max ∈ {8,9,T})** |

Cumulative closure since pre-S68: **$1,291.16 of $1,409 = 91.6%** (was 91.1%).

**Score: $1,627.36/1000h on full grid, $776.88 on prefix (unchanged — rule fires entirely outside prefix coverage). Improvement: −$1,956 vs v8 baseline, −$7.23 vs v63.** Rule count: 23 → **24**.

**Methodology lesson #1 — the chain-audit pattern transferred 1:1 for a FOURTH consecutive session.** S87 → S88 → S89 → S90: identical script structure, rebound parameters. Four sessions of audit work, four ships from the same diagnosis. Infrastructure cost is empirically zero per cell.

**Methodology lesson #2 — the EFFECT-SIZE-DOMINANCE rule generalizes across four ships and three orders of magnitude.** S87 ($98.67), S88 ($98.84), S89 ($10.09), S90 ($7.23): all four pass the "effect ≫ noise floor by 20×+" criterion. The per-session SHIP threshold is a per-cell calibration; the noise-floor multiple is what generalizes. For S91+ prefix-COVERED audits, the two-grid SHIP standard kicks in instead — EFFECT-SIZE-DOMINANCE is not the only verdict mechanism.

**Methodology lesson #3 — pre-drill ↔ grader match to two decimal places is the load-bearing stability signal (NEW S90).** When pre-drill predicts $7.23 and grader confirms $7.23 to the penny across every cell, the noise floor is genuinely well below the effect size. This is the strongest statistical confirmation available without N=1000 validation.

**Methodology lesson #4 — per-hand swap-right rate < 65% is a "chain-layer-doing-real-work" flag (NEW S90).** S90's 70.4% (down from S89's 85.2%) accurately predicted v52-defensive-low's partial-effectiveness BEFORE the audit revealed the mechanism. Future audits: treat sub-65% swap-right rate as a flag to investigate the chain layer's design intent before defaulting to full gate-out.

**Methodology lesson #5 — v52-defensive-low is PARTIALLY effective (NEW S90 project finding).** First project-level audit of this firing mode. It recovers $9.52 of v47's $19.28 bleed but doesn't fully restore v44_dt levels. The S53 design intent (defensive-low for max ∈ {7-T}) was REAL and CORRECT; the implementation leaves money on the table relative to ML. Future v65 refinement opportunity: per-hand picker between v52-DL and v44 on the ~23% of S90 hands where v52-DL wins. Deferred.

**Methodology lesson #6 — Phase A structural feasibility check is fully transferable.** S90 eliminated 4 (cell × rank) combinations for free using pencil-and-paper combinatorial proofs. Apply this check before any future chain-audit pivot to a new cell type.

**End of S90:** Rule chain ADVANCES to **v64_high_only_chain_fix_zone** ($1,627.36 full / $776.88 prefix unchanged). ML champion UNCHANGED at v44_dt ($1,081 full / $686 prefix) — 18 sessions running. Rule count: **24.** Structurally-non-empty HIGH_ONLY × max ≥ 8 chain-audit zone COMPLETE. Audit candidates remaining for S91+: prefix-COVERED cells (LOW pair / two_pair / trips) — methodologically stronger with two-grid SHIP standard; v60 from S86 still UNSHIPPED pending Option C N=1000 oracle infrastructure; v52-defensive-low per-hand-picker refinement deferred.

## Session 93: v65 SHIPS Rule 25 (MID pair × PMID_DS_NOMAXTOP × max_sing ≤ Q chain extension) — unblocks 7-session-parked v60 candidate via NEW Option C N=1000 sparse-grid infrastructure (engine `--id-list-file`); first SHIP by two-grid standard on prefix-uncovered cells; production +$6.43/1000h on N=200, +$6.34/1000h on N=1000

**The story:** S92 declared the chain-audit methodology arc COMPLETE and promoted Option C (build N=1000 oracle generator infrastructure) from TERTIARY to PRIMARY for S93, with the explicit goal of unblocking v60 — a candidate parked since S86 as MIXED-by-methodology because its target cell (MID × PMID_DS_NOMAXTOP, cid_min 593,072) sat entirely outside the prefix N=1000 grid's [0, 500K) coverage and could not be evaluated under the S84 two-grid SHIP standard.

S93 executed that plan in four phases:

**Phase A — Rust engine modification.** Added a `solve_grid_ids` function to `engine/src/oracle_grid.rs` (sibling of `solve_grid_range` but each item carries its own canonical_id) and a `--id-list-file` CLI option to the `OracleGrid` subcommand in `engine/src/main.rs`. The id-list file is plain text (one decimal canonical_id per line; blank lines and `#` comments ignored); the engine sorts + dedupes the ids, processes only those rows, and writes a SPARSE TWOG file whose header `canonical_total` is the id-list length so the existing resume + header-mismatch guards still work. Output rows carry their own canonical_id, so the file is interpretable by downstream tools as a sparse subset of the full grid. EVs at the same `--samples` / `--seed` / `--opponent` are bit-identical to corresponding rows of the full sequential grid, because the per-hand seed in `solve_grid_one` is `base_seed + canonical_id × φ`.

**Phase B — correctness test (bit-exact vs prefix grid).** Wrote `test_id_list_correctness_S93.py`: sampled 100 ids spread across the prefix range [0, 500K), invoked the engine with `--id-list-file` at the default params (samples=1000, seed=0xC0FFEE, opp=Realistic), and compared per-hand 105-EV vectors against the existing prefix N=1000 grid. **100/100 rows bit-identical, max abs EV diff = 0.0.** Engine throughput: ~25 hands/s parallel at N=1000 on this Mac.

**Phase C-1 — id list prep.** `prepare_v60_id_list_S93.py` walked the MID × PMID_DS_NOMAXTOP cell (114,048 hands from `drill_pair_v44_per_hand_structural.parquet`), computed v57 and v60 picks at each of gates 10/11/12, identified the 32,304 canonical_ids where v60-gate12 changes v57's pick, wrote them to a text file, and recomputed the S86 N=200 baselines: gate 10 +$1.63 / gate 11 +$4.85 / gate 12 **+$6.43**. **Exact match to S86's published numbers** — confirms the cell hand list and v57/v60 strategy implementations are unchanged.

**Phase C-2 — engine sparse run.** Invoked the engine on the 32,304-id list at N=1000. **Wall: 1,277 s (21:17) at 25.3 hands/s** with 8.0s block flush cadence (block-size 200). Output: `data/session93/v60_n1000_sparse.bin`, 13.7 MB, 32,304 sparse records.

**Phase C-3 — two-grid grade (pre-committed thresholds).** `grade_v60_id_list_n1000_S93.py` LOCKED thresholds in code BEFORE reading the sparse grid: SHIP requires BOTH N=200 lift ≥ $5 AND N=1000 lift ≥ $5 (each on whole-grid scale); NULL requires both ≤ $1; MIXED otherwise. Per-gate results on the same changed-hand subsets:
...
**Gate 12 SHIPS by two-grid standard.** The two grids' independent MC estimates agree to within $0.09/1000h ($0.02 at gate 10). Per-hand sign-agreement between N=200 and N=1000 deltas is only 77.8% — typical MC noise at the per-hand level — but aggregated over 32,304 hands the variance collapses and both grids land at essentially the same whole-grid lift. **This is the first project SHIP via the two-grid bar on a prefix-uncovered cell.**

**Phase C+ — build + grade v65 (production composition).** Built `strategy_v65_mid_pair_chain_extend.py`: composes v64's HIGH_ONLY chain-audit gate (Rules 21-24) with v60-gate12's MID pair rule. The two firing zones are DISJOINT by construction (HIGH_ONLY has no pair; v60's zone requires exactly one MID pair), so composition is purely additive — v64's lift over v57 + v60-gate12's lift over v57 = v65's lift over v57. `grade_v65_full_grid_S93.py` confirmed empirically: **v65 whole-grid N=200 lift over v64 = +$6.43/1000h** (matches S86 baseline exactly); v64==v57 on 100% of the 32,304 changed hands (confirms v65 = v64 + v60 logic everywhere); v65==v64 on 50,000/50,000 out-of-cell random sample (0 disagreements). **Production: $1,627.36 → $1,633.79/1000h on full grid.** Prefix score UNCHANGED at $776.88 (rule fires entirely outside prefix coverage).
...
* top = leftover singleton (highest non-max in the DS partition)

Plain-language: "If you have a MID pair (8-T) with max kicker ≤ Q, AND there's a double-suited bot configuration available BUT only if you DON'T put your max kicker on top — drop the max kicker into the bot, take the next-best kicker on top, keep the pair in the mid. The double-suited bot is worth more than the marginal extra top equity from a Q-or-below max card."
...
|---|---:|---:|
| Pre-S68 | $1,409 | baseline |
| Post-S90 (v64) | $117.84 | — (S91, S92 contribute $0) |
| **Post-S93 (v65)** | **$111.41** | **−$6.43 (Rule 25 — MID pair PMID_DS_NOMAXTOP × max_sing ≤ Q)** |

Cumulative closure since pre-S68: **$1,297.59 of $1,409 = 92.09%** (was 91.6%).

**Score: $1,633.79/1000h on full grid, $776.88 on prefix (unchanged). Improvement: −$1,950 vs v8 baseline, −$6.43 vs v64.** Rule count: 24 → **25**.

**Methodology lesson #1 — Option C N=1000 sparse infrastructure works at production quality (NEW S93).** The engine `--id-list-file` mode produced bit-identical EVs to the corresponding rows of the prefix N=1000 grid on the 100-sample correctness test. Throughput (~25 hands/s parallel) is acceptable for cell-scale validations (32K hands ≈ 22 min). The infrastructure is now available for ALL future "rule cell entirely outside prefix" candidates — most notably it permits future smaller-effect rules to be validated against the two-grid SHIP standard rather than landing in MIXED-by-methodology limbo.

**Methodology lesson #2 — MIXED-by-methodology candidates are recoverable if the only blocker was prefix coverage (NEW S93).** v60 was parked for 7 sessions (S86 → S92) carrying a +$6.43 N=200 SHIP-signal that couldn't be confirmed because the prefix grader was structurally silent. With the right infrastructure, the SHIP was a single engine run away. Future MIXED-by-methodology candidates with the same shape should be queued for Option C validation rather than written off.

**Methodology lesson #3 — pre-committed two-grid thresholds are robust at low effect sizes (NEW S93).** Per-hand MC noise at N=200 vs N=1000 produced only 77.8% sign-agreement on the 32,304 changed hands (high disagreement!), but aggregate lifts agreed to $0.09/1000h. The per-hand noise floor matters for per-hand picker design; the aggregate cell-level effect is what the two-grid bar measures, and it survives high per-hand variance cleanly.

**Methodology lesson #4 — disjoint firing zones make production composition trivially safe (NEW S93).** v64 fires on HIGH_ONLY (no pair); v60 fires on MID pair (exactly one pair of rank 8-T). Empirical confirmation: v64==v57 on 100% of v60's changed hands, and v65==v64 on 50K out-of-cell random samples. When zones are demonstrably disjoint, a composed strategy can ship with confidence from the cell-level grader alone, without a separate full whole-grid regrade.

**Methodology lesson #5 — chain-audit-arc-complete does NOT mean rule-arc-complete (NEW S93).** S92 closed the chain-audit lever after 4 ships + 2 NULLs. S93's ship is NOT a chain-audit ship — it's a rule-EXTRACTION ship (Option D-revised pattern) on a within-v44_dt residual leak. The infrastructure track (Option C) unlocked the ship by making the parked candidate gradeable. **Project still has live levers — they're just different ones than the chain-audit track that dominated S87-S92.**

**End of S93:** Rule chain ADVANCES to **v65_mid_pair_chain_extend** ($1,633.79 full / $776.88 prefix). ML champion UNCHANGED at v44_dt ($1,081 full / $686 prefix) — 21 sessions running. Rule count: **25.** Cumulative closure: 91.6% → 92.09% of original $1,409. Two-track divergence: $117.84 → $111.41/1000h. Combined S87-S93 ship recovery: $214.83 (chain-audit S87-S90) + $6.43 (Rule 25 S93) = **$221.26/1000h**. New levers available: Option C lets us now validate other parked candidates and probe smaller-effect rules (e.g., gate=11 at $+4.85/+$4.77 — currently MIXED, conceivably ships under a relaxed two-grid bar; two_pair LAYOUT_A_SS at $35.22/1000h in 437K hands). S94 default plan: rule-extraction on within-v44_dt residual (two_pair LAYOUT_A_SS or LOW pair PMID_DS_MAXTOP refinement), OR validate other parked MIXED candidates via Option C, OR headline-goal recalibration. The chain-audit lever remains exhausted on current architecture.

## Session 94: STRUCTURAL NULL on within-v44_dt rule-extraction (Option D-revised) applied to two_pair LAYOUT_A_SS + 9 adjacent residual cells — defines the rule-extraction lever's structural saturation boundary

**Production v65 UNCHANGED.** S94 executed the S93-defined PRIMARY path verbatim: rule-extraction on two_pair LAYOUT_A_SS at $35.22/1000h on 437,580 hands — the largest unaddressed within-v44_dt residual cell. The plan was to identify a Rule-20-style structural-feature trigger (analogous to S83's Rule 20 / S86+S93's Rule 25), pre-drill on N=200, run Option C N=1000 sparse grid, and ship strategy_v66 if the candidate cleared the two-grid SHIP bar.

**Phase A characterization** found v44_dt is structurally saturated on this cell at the (hi_pair, lo_pair) bucket level. Of 78 (hi, lo) buckets with 1000+ hands, **75 have matching v44/oracle modal layouts** — v44 is picking the right major layout choice. The largest single off-diagonal pivot (v44=Layout B → oracle=Layout C, $10.47/1000h on 23,326 hands) was probed for a sub-bucket trigger across 691 (hi, lo, max_sing) sub-buckets. **ZERO sub-buckets reach the Rule 20 anchor of ≥70% predictivity** (and zero reach Rule 25's 62% anchor); the top sub-bucket is at 50.0% on only 4 hands.

**Cross-cell saturation audit** extended the analysis to all 10 within-v44_dt residual cells (6 two_pair + 4 trips). Aggregate result: bucket-level layout-flip ceiling **$5.08/1000h** across all cells — under the $5 SHIP bar even at 100% trigger accuracy. Maximum sub-bucket predictivity across all 10 cells: 57.5% (trips B_DS_AVAIL_HKR), still below Rule 25's 62% anchor.

**Intra-Layout-A bot_suit follow-up on trips B_DS_AVAIL_LKR** (the only cell × structure-layer showing genuine Rule-20-style signal). Within (Layout-A agree, v44 picks non-DS bot, oracle picks DS bot): $19.49/1000h aggregate on 25,905 hands. Two narrow suit-feature sub-buckets — (kickers_max_suit_count, n_kickers_in_trip_suits, n_b_ds_routings) = (2, 4, 1) and (3, 4, 1) — reach 100% and 98.4% trigger predictivity respectively, but together carry only $0.53/1000h. The broader sub-buckets that carry the material wg ($2.42 and $4.70 each) drop to 28-48% predictivity. **Sub-buckets at ≥70% predictivity total 8,160 hands at $5.89/1000h** — borderline at the $5 SHIP bar even at perfect trigger, most likely lands MIXED in real grading. **Logged as S95 PRIMARY candidate; requires Rule-20-style picking-logic engineering ("force-best-DS-bot-within-Layout-A" pick).**

**Score: $1,633.79/1000h on full grid. Improvement: $0 vs v65. v44_dt UNCHANGED for 22nd consecutive session.**

**Methodology lesson #1 — Option D-revised has a structural saturation boundary (NEW S94).** Just as the chain-audit lever has a saturation boundary (S91 POPULATION-DIVERGENCE / S92 STRUCTURAL NULLs closed it), the rule-extraction lever applied to within-v44_dt residuals has a STRUCTURAL TRIGGER-PREDICTIVITY BOUNDARY. At the (cell × structural feature) level, no within-v44_dt residual sub-bucket on the 10 cells reaches Rule 25's 62% predictivity threshold. Chain-audit + bucket-level rule-extraction levers are BOTH characterized as saturated on the current ML architecture.

**Methodology lesson #2 — trigger predictivity is the operational definition of "rule extractable" (NEW S94).** Rule 20 trigger predictivity ≥ 89% (S83). Rule 25 trigger predictivity = 62% (S86 + S93). Below ~62%, a deterministic-pick rule cannot ship cleanly because the false-positive cost (forcing the wrong pick on hands where v44_dt is correct) dominates the true-positive gain. **Predictivity is the gating signal; leak magnitude alone is insufficient.** Two_pair LAYOUT_A_SS has $35.22/1000h of leak but no trigger above 29.3% — leak alone does not imply extractability.

**Methodology lesson #3 — intra-layout improvements are a distinct lever sub-class within Option D-revised (NEW S94).** Bucket-level layout-flip rules capture only $5.08/1000h across all 10 within-v44_dt residual cells. The Rule-20-style mechanism is INTRA-LAYOUT (within the modal layout choice, flip a sub-structural feature like bot_suit). Trips B_DS_AVAIL_LKR intra-Layout-A bot_suit is the first identified candidate of this sub-class on a within-v44_dt residual cell.

**Methodology lesson #4 — hand-specific residual dominates structural residual on these cells (NEW S94).** Two_pair LAYOUT_A_SS's $35.22 leak distributes across 702 sub-buckets with no concentration (top-30 sum $3.82, top-100 sum $11.18). The residual leak is not waiting for a structural rule to capture it — it's hand-specific variance requiring either oracle-quality picks (richer ML champion) or a different lever entirely.

**End of S94:** Rule chain UNCHANGED at **v65_mid_pair_chain_extend** ($1,633.79 full / $776.88 prefix). ML champion UNCHANGED at v44_dt ($1,081 full / $686 prefix) — **22 sessions running**. Rule count: **25** (UNCHANGED). Cumulative closure UNCHANGED at 92.09% of original $1,409. Two-track divergence UNCHANGED at $111.41/1000h. Combined S87-S93 ship recovery still $221.26/1000h (S94 contributes $0). The bucket-level rule-extraction lever joins the chain-audit lever in the "characterized as saturated on current architecture" category. S95 PRIMARY: trips B_DS_AVAIL_LKR intra-Layout-A bot_suit candidate (borderline 20-40% ship probability). S95 SECONDARY: headline-goal recalibration (well-motivated now by S91 + S92 + S94 saturation findings).

## Session 95: MIXED on trips B_DS_AVAIL_LKR intra-Layout-A force-best-DS-bot candidate (Rule 26) — NEW methodology lesson: picker alignment is the SECOND necessary condition for rule extraction alongside S94's trigger-predictivity boundary

**Production v65 UNCHANGED.** S95 executed the S94-defined PRIMARY path: design strategy_v66 with a Rule-20-analog "force-best-DS-bot-within-Layout-A" picker for trips, multi-gate pre-drill on N=200, Option C N=1000 sparse-grid grading on the WIDE (superset) population. Verdict: MIXED on all 3 gates. NARROW closest to ship at N=200 +$4.59 / N=1000 +$4.75 (|Δ|=$0.16, 93.9% sign-agree), still $0.25 below the $5 SHIP bar on each grid.

**Phase A re-confirmed the S94 finding from per-hand parquet.** Note: the resume prompt's NARROW sub-buckets (n=210 + n=370 = 580 hands, $0.53 wg) were transcription typos; the S94 audit JSON shows n=1,618 + n=6,542 = 8,160 hands at 90.4% mean predictivity / $5.89 wg ceiling. Three pre-committed trigger gates locked:
* **NARROW** [(2,4,1)+(3,4,1)]: 8,160 hands, mean P=90.4%, $5.89 wg ceiling
* **MEDIUM** [+ (2,4,3)]: 15,072 hands, mean P=77.7%, $10.07 wg ceiling
* **WIDE** [+ (2,3,1)]: 44,537 hands, mean P=56.8%, $22.64 wg ceiling

**Phase B's picker-criterion catastrophe + recovery.** strategy_v66's initial picker carried over Rule 20 / Rule 25's `bot_pair_high desc, bot_pair_2nd desc, top_rank desc` criterion. N=200 pre-drill: ALL three gates strongly NEGATIVE — NARROW $−3.22, MEDIUM $−6.25, WIDE $−8.84. Diagnostic showed exact-pick match with oracle was only 7.5% on NARROW fired+oracle-DS hands (3.5% on (2,4,1), 8.7% on (3,4,1)) despite 88.7% direction-predictivity. The rule's picker was choosing a different DS config than oracle in the high-predictivity buckets. An 8-criterion picker sweep (`picker_sweep_v66_S95.py`) found **TOP_HIGH then pair_tops** as the correct criterion: NARROW $+4.59 (85.2% sr), MEDIUM $+2.95 (65.9%), WIDE $+0.36 (57.2%) — a $7.81/1000h swing on NARROW from picker change alone. Per-sub-bucket exploration under TOP_HIGH variants: no slice clears the $5 SHIP bar at N=200; best individual slice (3,4,1) at $+3.50/82.0%; best cumulative slice NARROW = (2,4,1)+(3,4,1) at $+4.59/85.2%.

**Phase C Option C N=1000 grading (51,531 ids, ~31.7 min engine wall at 27.1 hands/s):**
...
Two-grid noise is small ($0.16 on NARROW); the verdict is an honest miss, not noise-driven. **Per the pre-committed two-grid standard (SHIP both ≥ $5, NULL both ≤ $1, otherwise MIXED), the candidate cannot ship.**

**Score: $1,633.79/1000h on full grid. Improvement: $0 vs v65. v44_dt UNCHANGED for 23rd consecutive session.**

**Methodology lesson #1 — Picker alignment is the SECOND necessary condition for rule extraction (NEW S95).** S94 established trigger predictivity ≥ 62-70% as necessary for "rule extractable". S95 adds: even when direction predictivity is 90%+, the rule's deterministic picker must align with oracle's specific pick within the predicted direction. Rules 20 and 25 had ≥89% picker alignment by happy accident (their `bot_pair_high` criterion = oracle's). The trips analog under the same criterion got 3-8% alignment in the highest-P sub-buckets. **Both conditions (trigger predictivity ≥ 62%, picker alignment ≥ ~70%) must be verified empirically for each new cell.**

**Methodology lesson #2 — Picker criteria are cell-specific, not transferable (NEW S95).** Rule 20 / 25 use `bot_pair_high` because the bot's pair value dominates top's single-card value for hands with a pair. Trips invert this — bot's strength is the trip card itself, top wants the highest kicker. **Never assume a successful rule's picker criterion will carry over;** validate empirically. The 8-criterion sweep is a 3-min diagnostic that swung NARROW lift by $7.81/1000h.

**Methodology lesson #3 — Resume-prompt transcription typo (operational).** S94's resume prompt and SESSION_94_REPORT.md listed NARROW sub-buckets at 580 hands / $0.53 wg — actually 8,160 hands / $5.89 wg per the S94 audit JSON. Always cross-check resume-prompt numbers against the JSON of record at session start when numbers look anomalous.

**Methodology lesson #4 — Option D-revised rule-extraction lever now exhausted across both sub-classes.** S94 closed bucket-level layout-flip rules ($5.08/1000h ceiling). S95 closes intra-layout structural-feature rules on the strongest remaining candidate. With both sub-classes saturated on the current ML architecture, S96 PRIMARY promotes to headline-goal recalibration.

**End of S95:** Rule chain UNCHANGED at **v65_mid_pair_chain_extend** ($1,633.79 full / $776.88 prefix). ML champion UNCHANGED at v44_dt ($1,081 full / $686 prefix) — **23 sessions running**. Rule count: **25** (UNCHANGED). Cumulative closure UNCHANGED at 92.09% of original $1,409. Two-track divergence UNCHANGED at $111.41/1000h. Combined S87-S93 ship recovery still $221.26/1000h (S94 + S95 contribute $0). Both rule-extraction sub-classes characterized as saturated. S96 PRIMARY: headline-goal recalibration. S96 SECONDARY: A3 ML retrain (~70h wall). S96 TERTIARY: v52-defensive-low partial-effectiveness exploit.

## Session 96: Headline-goal recalibration (doc-only) — 95% match% formally retired second time; $/1000h on the production grid codified as the project's headline metric; CLAUDE.md + checklist.md zombie-95% framing replaced

**Production v65 UNCHANGED. No code touched. No engine runs.** S96 is a documentation-only session that closes a multi-session loop the project has been carrying since Decision 033 (S16, 2026-04-27). Decision 033 retired the *shape-agreement* form of the 95% target; the *match%* form survived in load-bearing docs (`CLAUDE.md`, `checklist.md`) and re-emerged in session-report framing (S79, S81, S82, S85, S95). S96 audits the zombie framing, drafts the replacement headline, and applies the edits.

**Phase A audit found three load-bearing 95% match% references still alive:**
...
Decision 033's "Consequence #1" had explicitly noted *"CLAUDE.md headline section may need an update reflecting the new goal framing (deferred to Session 17 — not blocking)"* — 79 sessions later, never done. Every recent session report had been quietly working around the stale framing.

**Saturation evidence stack motivating the second-stage retirement (cited in Decision 131):**

| Lever | Where it sits | Anchor decision |
|---|---|---|
| ML feature engineering | EXHAUSTED at v44 saturating regime; redundancy ceiling at 2.25M leaves | Decision 113 (S78) |
| Label-quality A-path | FORMALLY CLOSED at v44 capacity (Lens-3 OOS NULL on all variants) | Decision 117 (S82) |
| Chain-audit (HIGH_ONLY × {J-A}/{8-T}) | $214.83/1000h SHIPPED then COMPLETE (S91 POPULATION-DIVERGENCE NULL; S92 STRUCTURAL NULL) | Decisions 122-127 (S87-S92) |
| Rule extraction bucket-level | SATURATED — $5.08/1000h ceiling at 100% trigger across all 10 within-v44_dt residual cells | Decision 129 (S94) |
| Rule extraction intra-layout | SATURATED — strongest candidate (trips B_DS_AVAIL_LKR) MIXED at $+4.59/$+4.75 (NARROW $0.25 short of $5 SHIP bar on each grid) | Decision 130 (S95) |
| A3 ML retrain | Only remaining lever with $50+ recovery potential; ~70h compute; credibility-low prior | DEPRIORITIZED |
| v52-DL partial-effectiveness exploit | Speculative; smaller magnitude than A3 | DEFERRED |

**Plus S79's empirical finding that oracle labels self-disagree at 32% N=200↔N=1000** — meaning any match% above ~68% on N=200 oracle is partially memorizing label noise. v44 sits at 67% on N=1000 Lens-3 OOS; the 95% target was set before this measurement and bakes in the assumption that the answer key is deterministic, which it provably isn't.

**Phase B drafted the new headline metric:**

> **PRIMARY HEADLINE METRIC:** Production strategy $/1000h on the full grid (N=200, RealisticHumanMixture, 6M canonical hands). Currently **$1,633.79**.
>
> **SECONDARY (diagnostic):** remaining gap to oracle ceiling (**$111.41**), two-track divergence (production vs v44_dt alone, **$552.79** — positive = rule layer adds value over ML), rule count (**25** — memorability cost).
>
> **OPERATIONAL SHIP STANDARD** (unchanged): pre-committed two-grid bar — both N=200 full grid AND N=1000 (prefix or sparse) lifts ≥ $5/1000h on the changed-hand cohort, with |Δ| ≤ ~$1 and sign-agreement ≥ 70%.
>
> **Match% retained as diagnostic only** — still informative for debugging, comparable across sessions, but not the headline.

**Phase C applied the doc edits:**
1. **CLAUDE.md** — lines 12 + 20 rewritten to drop 95%+/95-99% framing in favor of "$/1000h on the production grid" headline. Added a Decisions-033+131 pointer paragraph under Primary Goal so future readers have the trail.
2. **checklist.md** — lines 158-159 (70%+ agreement task and 95%+ agreement task) struck through with `[~]` markers + decision pointers; agreement-analysis line deprioritized.
3. **DECISIONS_LOG.md** — Decision 131 appended (full record of options considered, choice rationale, six "why" bullets, six consequences, two "what this does/does not change" lists).
4. **CURRENT_PHASE.md** — rewritten for S97 — A3 ML retrain promoted to PRIMARY (the only remaining lever with $50+ recovery potential, with honest expected-NULL prior); v52-DL exploit SECONDARY; v44_RULE13 fallthrough replacement TERTIARY.
5. **STRATEGY_GUIDE.md** — this entry (Part 1) + front-matter "Last updated" refreshed.
6. **MASTER_HANDOFF_01.md** + **sprints/SPRINT_INDEX.md** — S96 entries appended.

**Score: $1,633.79/1000h on full grid. Improvement: $0 vs v65. v44_dt UNCHANGED for 24th consecutive session.**

**Methodology lesson #1 — Doc drift compounds across sessions; periodic audits catch it (NEW S96).** Decision 033 retired the *shape-agreement* 95% target 79 sessions ago and explicitly flagged a needed CLAUDE.md update as "not blocking" → deferred to Session 17 → never done. The match% form re-emerged organically as the project pivoted to ML champions, and the zombie framing then lurked in load-bearing docs through ~30 sessions of saturation evidence accumulating. **"Not blocking" doc updates compound. Annual or post-pivot audits of CLAUDE.md / checklist.md headlines against actual project state are worth running.** Cost: ~1 session of grep + careful editing. Benefit: future MIXED verdicts (like S95's borderline $4.59/$4.75) are interpreted against the actual operational standard, not a defunct target.

**Methodology lesson #2 — Headline-goal recalibration is a one-off; saturation closures across all major levers are the legitimate trigger (NEW S96).** This recalibration is not a "lowering the bar to ship more easily" move — the per-candidate $5/1000h SHIP standard is unchanged. It is a "calling the project's overall finish-line where the evidence already pointed" move. Three saturation closures (Decisions 113, 117, 129+130) over 18 sessions form the legitimate evidentiary basis. Cascade recalibrations should require this kind of multi-lever closure pattern, not a single NULL session.

**Methodology lesson #3 — $/1000h survives label noise where match% does not (NEW S96).** Match% on an oracle with 32% self-disagreement at N=200↔N=1000 is fundamentally noisy as a per-hand metric. $/1000h averaged across 6M canonical hands × 105 settings × N=200 (or sparse N=1000) is robust by central-limit-theorem to that same noise. The two-grid SHIP standard's $5 bar at |Δ| ≤ $1 already encodes the noise floor empirically. **Pick a metric that the data quality supports; the original 95% match% target was conceived before the data quality was characterized.**

**End of S96:** Rule chain UNCHANGED at **v65_mid_pair_chain_extend** ($1,633.79 full / $776.88 prefix). ML champion UNCHANGED at v44_dt ($1,081 full / $686 prefix) — **24 sessions running**. Rule count: **25** (UNCHANGED). Cumulative closure UNCHANGED at 92.09% of original $1,409. Two-track divergence UNCHANGED at $111.41/1000h. Combined S87-S93 ship recovery still $221.26/1000h (S94 + S95 + S96 contribute $0 production). **Headline metric formally recalibrated to $/1000h on the production grid (Decision 131); 95% match% retired second-stage.** S97 PRIMARY: A3 ML retrain (the only remaining lever with $50+ recovery potential, ~70h wall, credibility-low expected outcome — operator authorization required to launch). S97 SECONDARY: v52-defensive-low partial-effectiveness exploit. S97 TERTIARY: v44_RULE13 fallthrough replacement (mostly absorbed by v54/v55/v56 already).
...
- **Rule 20 — LOW pair defensive PMID-DS swap (Session 83)** → `analysis/scripts/strategy_v57_lo_pair_defensive.py` — trigger: pair_rank ∈ {2-7} AND PMID_DS_NOMAXTOP cell AND max_sing ≤ Q (12). Setting: mid=both pair cards, bot=4-singleton subset forming best DS config (must include max_sing per cell), top=leftover non-max singleton. Fires on 12,096 prefix hands / 72,576 full-grid hands (changes 6,048 / 42,288 picks respectively at the Q gate; 72.8% swap-right rate within changed). **+$16.47/1000h whole-grid (full) + +$16.81/1000h prefix — grader-confirmed both grids with pre-committed SHIP/NULL/MIXED thresholds in code.** First rule extraction in the under-rule-covered weak-hand zone; validates Option D-revised rule-layer track as alive while ML-cascade track remains exhausted per Decisions 113 + 117.
- **Rule 21 — HIGH_ONLY × DS_NO_JOINT × {J-A} chain gate-out (Session 87)** → `analysis/scripts/strategy_v61_high_only_ds_no_joint_fix.py` — trigger: HIGH_ONLY (no pair) AND max_rank ∈ {J,Q,K,A} AND DS_NO_JOINT cell (n_DS > 0 AND n_DS_with_max_top > 0 AND n_joint == 0). Setting: return strategy_v44_dt directly, bypass v47→v48→v52 chain. Stops the $98.67/1000h v47-chain bleed on 756,000 hands. **+$98.67/1000h whole-grid (full) / $0 prefix (rule fires outside prefix coverage; SHIPPED under EFFECT-SIZE-DOMINANCE rule — effect ≫ noise floor by 20×+, mechanism is gate-out not addition, per Decision 122).** Largest single-rule production ship since S70.
- Rule 22 — HIGH_ONLY × {J-A} × {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH} chain gate-out extension (Session 88) → `analysis/scripts/strategy_v62_high_only_chain_fix.py` — trigger: HIGH_ONLY (no pair) AND max_rank ∈ {J,Q,K,A} AND cell ∈ {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH} per the S71 cell taxonomy. Setting: return strategy_v44_dt directly, bypass v47→v48→v52 chain. Strict superset of Rule 21's gate. Stops another $98.84/1000h v47-chain bleed on 357,504 additional hands (DS_NO_MAXTOP × {K,A} = 133K × +$52.88; MS_ONLY × {J-A} = 108K × +$31.51; JOINT_HIGH × {K,A} = 117K × +$14.45). Per-hand effect: 31.3% same, 52.8% better, 15.9% worse; swap-right rate on changed hands = 76.8%. **+$98.84/1000h whole-grid (full) / $0 prefix (rule fires outside prefix coverage; SHIPPED under EFFECT-SIZE-DOMINANCE rule per Decision 123).** Combined S87+S88 chain-audit recovery: $197.51/1000h. **Superseded by v63 (Session 89) — Rule 23 closes out the HIGH_ONLY × {J-A} zone.**
- Rule 23 — HIGH_ONLY × {J-A} × {all 6 non-empty cells} chain gate-out CLOSURE (Session 89) → `analysis/scripts/strategy_v63_high_only_chain_fix_full.py` — trigger: HIGH_ONLY (no pair) AND max_rank ∈ {J,Q,K,A} AND cell ∈ {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH, JOINT_MED, JOINT_LOW} per the S71 cell taxonomy. Setting: return strategy_v44_dt directly, bypass v47→v48→v52 chain. Strict superset of Rule 22's gate; closes the HIGH_ONLY × {J-A} zone (NEITHER × {J-A} is structurally empty by combinatorial proof — pigeonhole on 6 non-max cards across 4 suits). Stops the final $10.09/1000h v47-chain bleed on 48,132 additional hands (JOINT_MED × {J-A} = 44,562 × +$8.24; JOINT_LOW × {J-A} = 3,570 × +$1.85). Per-hand effect: 48.8% same, 43.6% better, 7.6% worse; swap-right rate on changed hands = **85.2% (HIGHEST across S87/S88/S89)**. **+$10.09/1000h whole-grid (full) / $0 prefix (rule fires outside prefix coverage; SHIPPED under EFFECT-SIZE-DOMINANCE rule at calibrated $5 SHIP threshold per Decision 124).** Combined S87+S88+S89 chain-audit recovery: **$207.60/1000h**. **Superseded by v64 (Session 90) — Rule 24 extends gate to HIGH_ONLY × max ∈ {8, 9, T}.**
- Rule 24 — HIGH_ONLY × max ∈ {8,9,T} × {5 non-empty cells} chain gate-out EXTENSION (Session 90) → `analysis/scripts/strategy_v64_high_only_chain_fix_zone.py` — trigger: HIGH_ONLY (no pair) AND max_rank ∈ {8,9,10,11,12,13,14} AND cell ∈ {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH, JOINT_MED, JOINT_LOW} per the S71 cell taxonomy. Setting: return strategy_v44_dt directly, bypass v47→v48→v52 chain. Strict superset of Rule 23's gate; extends to max ≤ T territory. Effective coverage: 11 non-empty (cell × rank) combinations at max ≤ T (the structurally-impossible HIGH_ONLY × max=7 / max ∈ {8,9,T} × JOINT_HIGH / max ∈ {8,9,T} × NEITHER / max=8 × JOINT_MED combinations are filtered out by the cell classifier). Stops a $7.23/1000h v44→v52 chain regression on 25,740 additional hands (DS_NO_JOINT × {8,9,T} = 16,200 × +$3.75; JOINT_MED × {9,T} = 3,150 × +$1.35; DS_NO_MAXTOP × {8,9,T} = 3,456 × +$0.92; MS_ONLY × {8,9,T} = 2,304 × +$0.90; JOINT_LOW × {8,9,T} = 630 × +$0.31). Per-hand effect: 21.6% same, 55.2% better, 23.2% worse; swap-right rate on changed hands = **70.4%**. The elevated 23.2% "worse" rate is the signature of v52-defensive-low's partial effectiveness — first project-level audit of this firing mode (LOW_MAX_DEFENSIVE = {7,8,9,10}). Verdict: v52-defensive-low is PARTIALLY effective (recovers $9.52 of v47's $19.28 bleed but doesn't fully restore v44_dt). S53 design intent was correct but conservative. **+$7.23/1000h whole-grid (full) / $0 prefix (rule fires outside prefix coverage; SHIPPED under EFFECT-SIZE-DOMINANCE rule at $5 SHIP threshold per Decision 125).** Pre-drill ↔ grader match to $0.01 across every cell (high aggregate stability). Combined S87+S88+S89+S90 chain-audit recovery: **$214.83/1000h** across four consecutive sessions. **Structurally-non-empty HIGH_ONLY × max ≥ 8 audit zone COMPLETE.** **Superseded by v65 (Session 93) — Rule 25 adds MID pair × PMID_DS_NOMAXTOP × max_sing ≤ Q extension on top of v64's HIGH_ONLY zone.**
- **Rule 25 — MID pair × PMID_DS_NOMAXTOP × max_sing ≤ Q chain extension (Session 93, CURRENT PRODUCTION)** → `analysis/scripts/strategy_v65_mid_pair_chain_extend.py` — trigger: exactly one pair AND pair_rank ∈ {8,9,10} (MID pair) AND PMID_DS_NOMAXTOP cell (n_PBOT_DS == 0 AND n_PMID_DS > 0 AND n_PMID_DS_w_maxtop == 0) AND max_sing (max non-pair singleton rank) ≤ Q (12) AND v57's current pick is PMID_tmax-style (top = max_sing position AND mid = pair positions). Setting: mid = pair, bot = 4 singletons forming the best DS configuration (the cell guarantees PMID_DS exists with max NOT on top, so max_sing goes into bot), top = leftover non-max singleton. Plain-language: "MID pair (8-T) with max kicker ≤ Q AND a DS bot is available — drop the max kicker into bot, take the next-best kicker on top, keep the pair in mid; double-suited bot is worth more than the marginal extra top equity from a Q-or-below max." Stops a +$6.43/1000h leak on 32,304 changed hands. Per-hand effect: 62.0% v65 better, 37.6% worse, 0.5% same; swap-right rate on changed hands = **62.0%**. **+$6.43/1000h whole-grid N=200 (full) / $0 prefix (rule fires entirely outside prefix coverage; cell cid_min 593,072 > prefix 500K boundary).** Validated under the two-grid SHIP standard via NEW Option C N=1000 sparse-grid infrastructure: N=1000 lift = +$6.34/1000h, |N=200 − N=1000| = $0.09 (S93 retroactive validation; both grids ≥ $5). **First project SHIP via two-grid bar on a prefix-uncovered cell.** Unblocks the v60 candidate parked since S86 as MIXED-by-methodology. Out-of-cell sanity: 0 v65 ≠ v64 disagreements on 50,000 random sample (composition with v64 is safe; firing zones disjoint by construction).
- **Combined chain (25 rules, CURRENT PRODUCTION)** → `analysis/scripts/strategy_v65_mid_pair_chain_extend.py` (wraps v64 with Rule 25 — disjoint firing zone vs v64; v64 wraps v57 with Rule 24, strict superset of Rule 23's gate, covers all 11 non-empty HIGH_ONLY × {8,9,T} (cell × rank) combinations + 6 non-empty HIGH_ONLY × {J-A} cells; v57 wraps v56 with Rule 20; v56 is the trips → v55 → v54 → v53 → v52 hybrid chain)
...
>
> **Production rule chain (end of Session 93): v65_mid_pair_chain_extend** —
> stacks atop v64 via Rule 25, a MID pair × PMID_DS_NOMAXTOP × max_sing ≤ Q
> rule that fires on a DISJOINT hand zone from v64's HIGH_ONLY gate (v64
> requires no pair; Rule 25 requires exactly one MID pair, rank 8-T). For
> hands matching all four conditions — exactly one pair AND pair_rank ∈
> {8,9,T} AND PMID_DS_NOMAXTOP cell AND max_sing ≤ Q AND v57's current
> pick is PMID_tmax-style — force the best PMID_tnomax_DS setting (mid =
> pair; bot = 4 singletons forming DS with max_sing in bot; top = next-
> best non-max singleton). Else fall through to v64 (HIGH_ONLY chain
> bypass) → v57 (LOW pair defensive + Rule 20) → v56 → … . Score:
> **$1,633.79/1000h full grid / $776.88 prefix** (prefix unchanged — the
> rule fires entirely outside prefix coverage, cell cid_min 593,072
> > prefix 500K boundary) — beats v14 by **+$2,472/1000h** on the full
> grid. Lives at
> `analysis/scripts/strategy_v65_mid_pair_chain_extend.py`. **Total
> project rule count: 25** (Rule 25 added in S93 via NEW Option C
> N=1000 sparse-grid infrastructure, unblocking v60 parked since S86;
> +$6.43/1000h N=200 + +$6.34/1000h N=1000 — first project SHIP via
> two-grid bar on a prefix-uncovered cell). S93 is the sixth strategy-
> of-record change in 11 sessions (S83 Rule 20, S87 Rule 21, S88 Rule
> 22, S89 Rule 23, S90 Rule 24, S93 Rule 25).
>
> **ML champion (not human-memorizable): v44_dt (end of Session 58,
> UNCHANGED through S93 — 21 consecutive sessions)** —
> ~2,250,000-leaf DecisionTreeRegressor (depth=36, min_samples_leaf=1),
> 107 features including all the gated families through v43 plus 4 new
> `ho_v4_*_g` features for high_only quality discrimination. Score:
> **$1,081/1000h full grid / $686 prefix** — beats v14 by **+$1,952/1000h**
> on the full grid and **+$1,351/1000h** on the prefix N=1000. Lives at
> `analysis/scripts/strategy_v44_dt.py` + `data/v44_dt_model.npz`.
> v44_dt is now invoked inside v55 for two_pair AND inside v54 for pair
> PBOT cells, so v44's quality directly drives the production rule chain.
>
> The cumulative 9-ship ML arc since v32: v32 → v34 → v36 → v39 → v40 →
> v41 → v42 → v43 → v44 = **−$634/1000h on the full grid**. Each ship is
...
>
> The production rule chain at v65 now outperforms v44_dt on the full
> grid by **+$552.79/1000h** (v65 $1,633.79 vs v44_dt $1,081). The
> remaining gap to the oracle ceiling (the "two-track divergence" in
> the project's historical framing) is **$111.41/1000h**, down from
> $1,409 pre-S68 — **cumulative closure: 92.09%.** Closures by session:
> S68 (+$382 via v54 pair-PBOT routing), S69 (+$634 via v55 two_pair
> routing), S70 (+$45 via v56 trips routing), S83 (+$16 via v57 Rule 20
> LOW-pair defensive PMID-DS swap), S87 (+$98.67 via v61 Rule 21
> DS_NO_JOINT chain gate-out), S88 (+$98.84 via v62 Rule 22 chain
> gate-out extension), S89 (+$10.09 via v63 Rule 23 chain gate-out
> closure {J-A}), S90 (+$7.23 via v64 Rule 24 chain gate-out
> extension to max ≤ T), **S93 (+$6.43 via v65 Rule 25 MID pair
> PMID_DS_NOMAXTOP × max_sing ≤ Q — first SHIP under the two-grid bar
> on a prefix-uncovered cell, enabled by NEW Option C N=1000 sparse
> oracle generator infrastructure)**. The S68-S70 arc is architectural
> routing; S83 is rule extraction; S87-S90 are CHAIN AUDIT recoveries
> on prefix-silent HIGH_ONLY cells (**$214.83/1000h** across four
> consecutive sessions, **CHAIN-AUDIT METHODOLOGY ARC COMPLETE** per
> S92 — applied across all 4 major hand-categories with 4 ships + 2
> NULLs at well-characterized boundaries); S93 is rule extraction on
> a within-v44_dt residual, unblocked by infrastructure. Combined
> S87-S93 production-chain recovery: **$221.26/1000h**. New levers
> opened in S93: Option C lets us now validate other parked candidates
> at N=1000 quality (gate=11 of v60 currently MIXED at +$4.85/+$4.77;
> two_pair LAYOUT_A_SS at $35.22/1000h on 437K hands) and probe
> smaller-effect rules that previously couldn't clear the two-grid
> bar. v52-defensive-low partial-effectiveness exploit (S90 finding)
> remains deferred.
