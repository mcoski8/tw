...
- **Rule 22 — HIGH_ONLY × {J-A} × {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH} chain gate-out extension (Session 88, CURRENT PRODUCTION)** → `analysis/scripts/strategy_v62_high_only_chain_fix.py` — trigger: HIGH_ONLY (no pair) AND max_rank ∈ {J,Q,K,A} AND cell ∈ {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH} per the S71 cell taxonomy. Setting: return strategy_v44_dt directly, bypass v47→v48→v52 chain. Strict superset of Rule 21's gate. Stops another $98.84/1000h v47-chain bleed on 357,504 additional hands (DS_NO_MAXTOP × {K,A} = 133K × +$52.88; MS_ONLY × {J-A} = 108K × +$31.51; JOINT_HIGH × {K,A} = 117K × +$14.45). Per-hand effect: 31.3% same, 52.8% better, 15.9% worse; swap-right rate on changed hands = 76.8%. **+$98.84/1000h whole-grid (full) / $0 prefix (rule fires outside prefix coverage; SHIPPED under EFFECT-SIZE-DOMINANCE rule per Decision 123).** Combined S87+S88 chain-audit recovery: $197.51/1000h.
- **Combined chain (22 rules, CURRENT PRODUCTION)** → `analysis/scripts/strategy_v62_high_only_chain_fix.py` (wraps v57 with Rule 22 — strict superset of Rule 21's gate; v57 itself wraps v56 with Rule 20; v56 is the trips → v55 → v54 → v53 → v52 hybrid chain)
...
>
> **Production rule chain (end of Session 88): v62_high_only_chain_fix** —
> stacks atop v57 via Rule 22, a strict superset of Rule 21's gate-out:
> for HIGH_ONLY (no-pair) hands with max ∈ {J,Q,K,A} in cells
> {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH} per the S71 taxonomy,
> bypass the v47→v48→v52 chain and return strategy_v44_dt directly. Else
> route to v57_lo_pair_defensive (which adds Rule 20 atop v56 etc.).
> Score: **$1,610.04/1000h full grid / $776.88 prefix** (prefix unchanged
> — the rule fires entirely outside prefix coverage) — beats v14 by
> **+$2,449/1000h** on the full grid. Lives at
> `analysis/scripts/strategy_v62_high_only_chain_fix.py`. **Total project
> rule count: 22** (Rule 22 added in S88 — chain gate-out extension to
> three additional prefix-silent HIGH_ONLY cells, recovering another
> $98.84/1000h after S87's $98.67). S88 is the third strategy-of-record
> change in 6 sessions (S83 added Rule 20, S87 added Rule 21, S88 added
> Rule 22).
>
...
>
> The production rule chain at v62 now outperforms v44_dt on the full
> grid by **+$529/1000h** (v62 $1,610.04 vs v44_dt $1,081). The remaining
> gap to the oracle ceiling (the "two-track divergence" in the project's
> historical framing) is **$135/1000h**, down from $1,409 pre-S68 —
> **cumulative closure: 90.4%.** Closures by session:
...
> gate-out extension). The S68-S70 arc is architectural routing (route
> categories through v44); S83 is rule extraction; S87-S88 are CHAIN
> AUDIT recoveries — finding and removing buried v47 regressions on
> prefix-silent HIGH_ONLY cells. The chain-audit lever has now shipped
> $197.51/1000h across two sessions, dominating value-extraction
> ($16.81 from Rule 20 across the prior 16-session window).
