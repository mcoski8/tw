...
- **Rule 14 — A-high no-pair, A-on-top + DS/SS HIMID (Session 50, CURRENT PRODUCTION) — LARGEST SINGLE-RULE LIFT IN PROJECT HISTORY** → `analysis/scripts/strategy_v45_rule14_Ahigh_DS.py` — trigger: cat=high_only AND max=A AND DS-bot OR SS-bot achievable with A on top. Setting builder: TOP=A always, try DS-bot first then SS-bot fallback, in both cases use HIMID tie-break (mid = 2 highest non-A cards). Fires on 93.6% of A-high no-pair (= 10.3% of grid). **+$131/1000h whole-grid (full) + $0 prefix (UNCHANGED — high_only has zero prefix coverage).** pct_opt full: +0.71%. high_only category $4,082 → $3,439 (−$643, 16% reduction); high_only pct_opt 19.8% → 23.3% (+3.5%). p90 regret IMPROVED 0.785→0.745. Beats v33's Rule 6 (+$113, S37) which had held the single-rule record for 13 sessions. Cumulative v44 → v45 = −$131 full / $0 prefix; v39 → v45 = −$260 full / −$185 prefix. Mechanism: HIMID, not HIBOT — with A on top, the next-best 2 cards (K+Q) belong in MID for Hold'em scoring, lower 4 cards form bot's Omaha play (suit > rank).
...
- **Combined chain (14 rules, CURRENT PRODUCTION)** → `analysis/scripts/strategy_v45_rule14_Ahigh_DS.py` (wraps v44 with Rule 14 — A-high no-pair + DS/SS HIMID; LARGEST single-rule lift in project history)
