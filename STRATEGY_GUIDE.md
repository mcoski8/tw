...
- **Rule 16 — Q-high no-pair, Q-on-top + DS/SS HIMID (Session 52, CURRENT PRODUCTION)** → `analysis/scripts/strategy_v47_rule16_Qhigh_DS.py` — same structure as Rules 14/15. Trigger: cat=high_only AND max=Q AND DS-bot OR SS-bot achievable with Q on top. Setting builder: TOP=Q always, try DS-bot first then SS-bot fallback, HIMID tie-break. Fires on 97.2% of Q-high no-pair (= 2.4% of grid). **+$19/1000h whole-grid (full) + $0 prefix (UNCHANGED).** pct_opt full: +0.06%; high_only sub-category +0.3%. high_only $3,187 → $3,096 (−$91, −2.9%). Cumulative v46 → v47 = −$19 full / $0 prefix; v39 → v47 = −$330 full / −$185 prefix. Known coverage gap: oracle picks Q-on-top only 49% of Q-high (Q is genuinely borderline) — Rule 16 v2 candidate would address the 51% non-Q-on-top sub-zone (incl 16% defensive 2-on-top).
...
- **Combined chain (16 rules, CURRENT PRODUCTION)** → `analysis/scripts/strategy_v47_rule16_Qhigh_DS.py` (wraps v46 with Rule 16 — Q-high no-pair + DS/SS HIMID; completes the A/K/Q-high HIMID family)
