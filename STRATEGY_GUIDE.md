...
- **Rule 23 — HIGH_ONLY × {J-A} × {all 6 non-empty cells} chain gate-out CLOSURE (Session 89, CURRENT PRODUCTION)** → `analysis/scripts/strategy_v63_high_only_chain_fix_full.py` — trigger: HIGH_ONLY (no pair) AND max_rank ∈ {J,Q,K,A} AND cell ∈ {DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH, JOINT_MED, JOINT_LOW} per the S71 cell taxonomy. Setting: return strategy_v44_dt directly, bypass v47→v48→v52 chain. Strict superset of Rule 22's gate; closes the HIGH_ONLY × {J-A} zone (NEITHER × {J-A} is structurally empty by combinatorial proof — pigeonhole on 6 non-max cards across 4 suits). Stops the final $10.09/1000h v47-chain bleed on 48,132 additional hands (JOINT_MED × {J-A} = 44,562 × +$8.24; JOINT_LOW × {J-A} = 3,570 × +$1.85). Per-hand effect: 48.8% same, 43.6% better, 7.6% worse; swap-right rate on changed hands = **85.2% (HIGHEST across S87/S88/S89)**. **+$10.09/1000h whole-grid (full) / $0 prefix (rule fires outside prefix coverage; SHIPPED under EFFECT-SIZE-DOMINANCE rule at calibrated $5 SHIP threshold per Decision 124).** Combined S87+S88+S89 chain-audit recovery: **$207.60/1000h** — bigger than every value-extraction ship from S71-S86 combined. **HIGH_ONLY × {J-A} audit zone now CLOSED.**
- **Combined chain (23 rules, CURRENT PRODUCTION)** → `analysis/scripts/strategy_v63_high_only_chain_fix_full.py` (wraps v57 with Rule 23 — strict superset of Rule 22's gate, covers all 6 non-empty HIGH_ONLY × {J-A} cells; v57 itself wraps v56 with Rule 20; v56 is the trips → v55 → v54 → v53 → v52 hybrid chain)
...
>
> **Production rule chain (end of Session 89): v63_high_only_chain_fix_full** —
> stacks atop v57 via Rule 23, a strict superset of Rule 22's gate-out
> that CLOSES the HIGH_ONLY × {J-A} zone:
> for HIGH_ONLY (no-pair) hands with max ∈ {J,Q,K,A} in any of the 6
> non-empty cells (DS_NO_JOINT, DS_NO_MAXTOP, MS_ONLY, JOINT_HIGH,
> JOINT_MED, JOINT_LOW) per the S71 taxonomy, bypass the v47→v48→v52
> chain and return strategy_v44_dt directly. Else route to
> v57_lo_pair_defensive (which adds Rule 20 atop v56 etc.). NEITHER ×
> {J-A} is structurally empty (combinatorial proof, no audit needed).
> Score: **$1,620.13/1000h full grid / $776.88 prefix** (prefix unchanged
> — the rule fires entirely outside prefix coverage) — beats v14 by
> **+$2,459/1000h** on the full grid. Lives at
> `analysis/scripts/strategy_v63_high_only_chain_fix_full.py`. **Total
> project rule count: 23** (Rule 23 added in S89 — chain gate-out
> closure on the last two prefix-silent HIGH_ONLY × {J-A} cells,
> recovering another $10.09/1000h after S87's $98.67 and S88's $98.84).
> S89 is the fourth strategy-of-record change in 7 sessions (S83 added
...
>
> The production rule chain at v63 now outperforms v44_dt on the full
> grid by **+$539/1000h** (v63 $1,620.13 vs v44_dt $1,081). The remaining
> gap to the oracle ceiling (the "two-track divergence" in the project's
> historical framing) is **$125/1000h**, down from $1,409 pre-S68 —
> **cumulative closure: 91.1%.** Closures by session:
...
> closure). The S68-S70 arc is architectural routing (route categories
> through v44); S83 is rule extraction; S87-S88-S89 are CHAIN AUDIT
> recoveries — finding and removing buried v47 regressions on
> prefix-silent HIGH_ONLY cells. The chain-audit lever has now shipped
> **$207.60/1000h across three sessions**, dominating value-extraction
> ($16.81 from Rule 20 across the prior 16-session window). **HIGH_ONLY
> × {J-A} chain-audit zone is now CLOSED** — all 6 non-empty cells
> gated; NEITHER × {J-A} structurally empty.
